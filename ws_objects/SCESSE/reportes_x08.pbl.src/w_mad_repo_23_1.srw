$PBExportHeader$w_mad_repo_23_1.srw
$PBExportComments$Ventana para el Reporte de Grupos Cancelables
forward
global type w_mad_repo_23_1 from w_master_main_rep
end type
type uo_lb_division from uo_pfc_list_box_multiple within w_mad_repo_23_1
end type
type tab_1 from tab within w_mad_repo_23_1
end type
...
type dw_depto_0 from datawindow within w_mad_repo_23_1
end type
type dw_depto_5 from datawindow within w_mad_repo_23_1
end type
type dw_depto_4 from datawindow within w_mad_repo_23_1
end type
type dw_depto_3 from datawindow within w_mad_repo_23_1
end type
type dw_depto_2 from datawindow within w_mad_repo_23_1
end type
type dw_depto_1 from datawindow within w_mad_repo_23_1
end type
type st_2 from statictext within w_mad_repo_23_1
end type
type ddlb_tipo_imp from dropdownlistbox within w_mad_repo_23_1
end type
end forward
...
integer width = 3822
integer height = 2892
string title = "Reporte de Grupos Cancelables"
...
global w_mad_repo_23_1 w_mad_repo_23_1
...
type st_sistema from w_master_main_rep`st_sistema within w_mad_repo_23_1
end type

type p_ibero from w_master_main_rep`p_ibero within w_mad_repo_23_1
end type
...
integer height = 1192
integer taborder = 63
string dataobject = "dw_repo_mad_23_1_gz"
...
if (em_2.text="") then
	messagebox("Información", "Por Favor Especifique un limite mínimo de Alumnos",Exclamation!)
	Plan_GO = 0
end if
...
if ( Total > 0 and Plan_GO = 1)  then
   // Se limpian los datawindows
	...
	FOR contador=1 TO Total
		Textito=lb_4.text(contador)
		Departamentos[contador] = long(Textito)
  	NEXT
...
End If
   ...
   Valor_Minimo = integer(em_2.text)
...
//**--**--**--**--**--**--**--**--**--**--**--**--**--**--**--**--**--**--**--**--**		
	...
	dw_2x.retrieve(agrupa,Departamentos,Salones,periodo_x,Anio_X,ls_array_nivel[],Valor_Minimo)	
	dw_2z.retrieve(agrupa,Departamentos,Salones,periodo_x,Anio_X,ls_array_nivel[],Valor_Minimo,ii_tipo_imp)
	
		if (cbx_2x.checked=FALSE) then
	...
	// Filtramos los grupos ZZ
	if cbx_sin_gpos_zz.checked then
		dw_2z.SetFilter("grupos_gpo not in ('ZZ')")  
	else
		dw_2z.SetFilter(ls_filtro_vacio)
	end if
	
	dw_2z.Filter()
	
	ll_cont = dw_2z.rowcount() 
	if ll_cont > 0 then
...
integer height = 392
integer taborder = 31
string dataobject = "dw_repo_mad_23_1_gx"
boolean livescroll = true
end type
...
dw_2x.Object.DataWindow.Zoom = 78
end event
