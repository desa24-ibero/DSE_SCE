$PBExportHeader$w_grupos_cupo_coord2.srw
forward
global type w_grupos_cupo_coord2 from window
end type
type cbx_evaluar from checkbox within w_grupos_cupo_coord2
end type
type st_1 from statictext within w_grupos_cupo_coord2
end type
type cb_porcentaje_uso from commandbutton within w_grupos_cupo_coord2
end type
type dw_profesor_auxiliar from uo_dw_captura within w_grupos_cupo_coord2
end type
type uo_3 from uo_nombre_profesor within w_grupos_cupo_coord2
end type
type dw_sdw from datawindow within w_grupos_cupo_coord2
end type
type dw_horario from uo_dw_captura within w_grupos_cupo_coord2
end type
type st_periodo from statictext within w_grupos_cupo_coord2
end type
type cb_1 from commandbutton within w_grupos_cupo_coord2
end type
type uo_2 from uo_per_ani within w_grupos_cupo_coord2
end type
type dw_1 from uo_dw_captura within w_grupos_cupo_coord2
end type
type uo_1 from uo_grupos_cupo_coord2 within w_grupos_cupo_coord2
end type
end forward
...
global w_grupos_cupo_coord2 w_grupos_cupo_coord2
...
public function boolean wf_valida_salon_en_linea ();// Se verifica si existe algún registro de horario con clase aula diferente a 3 "OTRO" 
...
on w_grupos_cupo_coord2.create
...
on w_grupos_cupo_coord2.destroy
...
uo_1.BackColor = rgb(192,192,192)
gi_anio=li_anio
...
STRING ls_desc_tipo ,ls_comentario
...
//if ll_num_horarios =0 and li_tipo<>1 and li_tipo<>2 and li_tipo<>4 then
...
//Busca si el salon ya se asignó en otro horario o para otro grupo
	if	ls_clase_aula <> "TALLER" then
		// Se valida si el salón requiere horario. 
...
//Cada vez que cargue un alumno los datawindows de grupos y horario solo permitiran modificar el cupo
this.dataobject = 'd_grupos_imp_nvo_cupo_coord'
this.SetTransObject(gtr_sce)

dw_horario.dataobject = 'd_gpo_imp_horario_cupo_coord'
...
//	END IF 			
	...
	IF data = '4' THEN 
		...
		IF THIS.GETITEMNUMBER(row, "tipo") = 1 THEN 
			MESSAGEBOX("Aviso", "Un grupo de asesorias no puede impartirse en Línea. ") 
			THIS.settext('') 
			THIS.SETITEM(row, 'modalidad', le_null) 
			RETURN 1 
...
IF ISNULL(le_modalidad) OR le_modalidad = 0 THEN 
	MESSAGEBOX("Aviso", "No se ha seleccionado una modalidad para el grupo")	 
	RETURN -1  
END IF 
...
if ModifiedCount()+DeletedCount() > 0  or ModifiedCount()= 0 Then
	...
	IF li_respuesta = 1 THEN 		
		...
		li_idioma = dw_1.GETITEMNUMBER(1, "idioma") 
		IF ISNULL(li_idioma) THEN li_idioma = 1 
			IF li_idioma <> 1 AND NOT ISNULL(li_idioma) THEN 
				SELECT idioma
				INTO :ls_idioma
				FROM idioma 
...
//Solo si se modificaron los horarios, se actualiza el datawindow de horarios, derecho-uso y grupos
			if li_hor_modified +li_hor_deleted > 0 then
				...
				///////////////////////////////////////////////////////////////////////////////////////////////////////////////	
				INTEGER li_tipo 
				LONG ll_cve_prof, le_valida_audiovisual
...
//						MESSAGEBOX("Aviso", iuo_grupos_multiplantel.is_ocupado)
			...
			if wf_actualiza_sdu()=1 and dw_1.Update()=1 and dw_horario.Update()=1 then
					
					//wf_inserta_horario_profesor( )
					//COMMIT USING gtr_sce; 
...
//Si NO se modificaron los horarios, solo se actualiza el datawindow de grupos
				if dw_1.Update()=1 then
					
					// Se actualiza el horario de profesor y la bandera de "Evaluar profesor" 
					IF wf_inserta_horario_profesor() < 0 THEN 
						// NOTA: EL ROLLBACK SE HACE EN EL OBJETO DE SERVICIOS
						messagebox("Información","No se han guardado los cambios.", Information!)
						RETURN -1
...
//Deshabilita la edición de horarios
	m_grupos_impartidos_nvo_cupo_coor2.m_registro.m_nuevohorario.enabled = false
	m_grupos_impartidos_nvo_cupo_coor2.m_registro.m_borrahorario.enabled = false
	m_grupos_impartidos_nvo_cupo_coor2.m_registro.m_borrahorarioactual.enabled = false
...
if ll_row>0 then
	...
	if li_cond_gpo = 0 then
		m_grupos_impartidos_nvo_cupo_coor2.m_registro.m_nuevo.enabled = false
		m_grupos_impartidos_nvo_cupo_coor2.m_registro.m_actualiza.enabled = false

		m_grupos_impartidos_nvo_cupo_coor2.m_registro.m_cargaregistro.enabled = false
		m_grupos_impartidos_nvo_cupo_coor2.m_registro.m_nuevohorario.enabled = false
		m_grupos_impartidos_nvo_cupo_coor2.m_registro.m_borrahorario.enabled = false
		m_grupos_impartidos_nvo_cupo_coor2.m_registro.m_borrahorarioactual.enabled = false

		m_grupos_impartidos_nvo_cupo_coor2.m_registro.enabled = false
		this.enabled = false
		dw_horario.enabled = false
		dw_profesor_auxiliar.enabled = false
	else
		if not( m_grupos_impartidos_nvo_cupo_coor2.m_registro.enabled = false or &
			m_grupos_impartidos_nvo_cupo_coor2.m_registro.visible = false) then
...
//		this.SetTabOrder ("cve_asimilada" , 33 )
//		this.SetTabOrder ("gpo_asimilado" , 36 )		
//con el objeto de evitar empalmes de horario, debe deshabilitarse el cambio de asimilantes		
		this.SetTabOrder ("cve_asimilada" , 0 )
		this.SetTabOrder ("gpo_asimilado" , 0 )		
		this.SetTabOrder ("cve_profesor" , 0 )
		this.SetTabOrder ("cupo" , 0 )	
...
//Se deshabilita el profesor para grupos asimilados		
		uo_3.enabled = false
...
//Se permite habilitar el profesor para grupos normales		
		uo_3.enabled = true
		uo_3.em_cve_profesor.text= string(ll_cve_profesor)
		uo_3.em_cve_profesor.Event Activarbusq()		
...
on uo_1.destroy
call uo_grupos_cupo_coord2::destroy
end on
