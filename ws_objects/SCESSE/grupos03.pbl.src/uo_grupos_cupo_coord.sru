$PBExportHeader$uo_grupos_cupo_coord.sru
forward
global type uo_grupos_cupo_coord from userobject
end type
type cbx_descuenta_sdu_se from checkbox within uo_grupos_cupo_coord
end type
type rb_editar from radiobutton within uo_grupos_cupo_coord
end type
type rb_insertar from radiobutton within uo_grupos_cupo_coord
end type
type dw_1 from datawindow within uo_grupos_cupo_coord
end type
end forward
...
global uo_grupos_cupo_coord uo_grupos_cupo_coord
...
on uo_grupos_cupo_coord.create
...
on uo_grupos_cupo_coord.destroy
...
if isvalid(w_grupos_cupo_coord) then
	w_ventana  =  w_grupos_cupo_coord
end if
...
CHOOSE CASE ls_columna
	CASE "gpo"
		...
		if ls_gpo = "ZZ" then
			MessageBox("Grupo Invalido","No se permite modificar los grupos ZZ",StopSign!)
			this.SetText(string(is_gpo))
			SetColumn("gpo")
			ib_editando= false
			return 1
