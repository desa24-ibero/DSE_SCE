$PBExportHeader$w_mad_repo_20.srw
$PBExportComments$Ventana para el Reporte de Grupos Impartidos
forward
global type w_mad_repo_20 from w_master_main_rep
end type
type uo_lb_division from uo_pfc_list_box_multiple within w_mad_repo_20
end type
type cbx_15 from checkbox within w_mad_repo_20
end type
type cbx_14 from checkbox within w_mad_repo_20
end type
type cbx_13 from checkbox within w_mad_repo_20
end type
type cbx_12 from checkbox within w_mad_repo_20
end type
type cbx_11 from checkbox within w_mad_repo_20
end type
type tab_1 from tab within w_mad_repo_20
end type
...
type dw_depto_0 from datawindow within w_mad_repo_20
end type
type dw_depto_5 from datawindow within w_mad_repo_20
end type
type dw_depto_4 from datawindow within w_mad_repo_20
end type
type dw_depto_3 from datawindow within w_mad_repo_20
end type
type dw_depto_2 from datawindow within w_mad_repo_20
end type
type dw_depto_1 from datawindow within w_mad_repo_20
end type
type st_2 from statictext within w_mad_repo_20
end type
type ddlb_tipo_imp from dropdownlistbox within w_mad_repo_20
end type
end forward
...
integer width = 3867
integer height = 2808
string title = "Reporte de Grupos Impartidos"
...
global w_mad_repo_20 w_mad_repo_20
...
this.Control[iCurrent+2]=this.cbx_15
this.Control[iCurrent+3]=this.cbx_14
this.Control[iCurrent+4]=this.cbx_13
this.Control[iCurrent+5]=this.cbx_12
this.Control[iCurrent+6]=this.cbx_11
this.Control[iCurrent+7]=this.tab_1
this.Control[iCurrent+8]=this.dw_depto_0
this.Control[iCurrent+9]=this.dw_depto_5
this.Control[iCurrent+10]=this.dw_depto_4
this.Control[iCurrent+11]=this.dw_depto_3
this.Control[iCurrent+12]=this.dw_depto_2
this.Control[iCurrent+13]=this.dw_depto_1
this.Control[iCurrent+14]=this.st_2
this.Control[iCurrent+15]=this.ddlb_tipo_imp
end on

on w_mad_repo_20.destroy
call super::destroy
...
type st_sistema from w_master_main_rep`st_sistema within w_mad_repo_20
integer x = 704
integer y = 92
end type

type p_ibero from w_master_main_rep`p_ibero within w_mad_repo_20
integer x = 14
integer y = 8
end type
...
event clicked;If (this.checked = TRUE) and (cbx_12.checked = TRUE) and (cbx_13.checked = TRUE)and (cbx_11.checked = TRUE) then
  	cbx_15.checked = TRUE
	cbx_15. EVENT clicked()
end if
end event
...
event clicked;If (this.checked = TRUE) and (cbx_12.checked = TRUE) and (cbx_11.checked = TRUE)and (cbx_14.checked = TRUE) then
  	cbx_15.checked = TRUE
	cbx_15. EVENT clicked()
end if
end event
...
event clicked;If (this.checked = TRUE) and (cbx_11.checked = TRUE) and (cbx_13.checked = TRUE)and (cbx_14.checked = TRUE) then
	cbx_15.checked = TRUE
	cbx_15. EVENT clicked()
end if
end event
...
event clicked;If (this.checked = TRUE) and (cbx_12.checked = TRUE) and (cbx_13.checked = TRUE)and (cbx_14.checked = TRUE) then
	cbx_15.checked = TRUE
	cbx_15. EVENT clicked()
end if
end event
...
this.Control[]={this.cbx_sin_gpos_zz,&
this.gb_5,&
this.uo_nivel,&
this.gb_4,&
this.hsb_1,&
...
Depto_A=integer(Depto)
  ...
  SELECT coordinaciones.cve_coordinacion, coordinaciones.coordinacion, departamentos.cve_division  
    ...
    FROM departamentos, coordinaciones
	 WHERE departamentos.cve_depto = coordinaciones.cve_depto AND
...
if Bandera = 1 then
	messagebox("Atención","La Coordinacion que desea introducir "+&
...
if ((cbx_1.checked = true or cbx_2.checked = true or cbx_3.checked = true or cbx_4.checked = true)&
	and (cbx_11.checked = true or cbx_12.checked = true or cbx_13.checked = true or cbx_14.checked = true)) then
   Plan_GO = 1
else
	Plan_GO = 0
end if
...
if ( Total > 0 and Plan_GO = 1)  then
   ...
   // Se Obtienen los tipos de Salon
	...
	contador1  = upperbound (Categoria[])
   ...
   if (cbx_4.checked = true) THEN
	...
	if (cbx_11.checked = true) THEN
		  Categoria[contador1+1]=2
	END IF	
	contador1  = upperbound (Categoria[])
	if (cbx_12.checked = true) THEN
		  Categoria[contador1+1]=3
	END IF	
	contador1  = upperbound (Categoria[])
	if (cbx_13.checked = true) THEN
		  Categoria[contador1+1]=4
	END IF	
	contador1  = upperbound (Categoria[])
	if (cbx_14.checked = true) THEN
		  Categoria[contador1+1]=6
	END IF	
	contador1  = upperbound (Categoria[])
	if (cbx_15.checked = true) THEN
		  Categoria[contador1+1]=0
	END IF	
	contador1  = upperbound (Categoria[])
...
If 	dw_2x.retrieve(agrupa,Departamentos,Salones,periodo_x[],Anio_X,ls_array_nivel[],Categoria,ii_tipo_imp)	<= 0 Then
...
//Originalmente utilizado para desplegar la informacion en pantalla
//	dw_2z.retrieve(agrupa,Departamentos,Salones,periodo_x,Anio_X,Nivel_X,Categoria)
...
integer height = 832
integer taborder = 61
string dataobject = "dw_repo_mad_20_gz"
boolean hscrollbar = true
boolean vscrollbar = true
...
integer taborder = 31
boolean bringtotop = true
string dataobject = "dw_repo_mad_20_gx"
boolean minbox = true
boolean hscrollbar = true
boolean vscrollbar = true
boolean resizable = true
