$PBExportHeader$w_mad_repo_31.srw
$PBExportComments$Ventana para el Reporte de Plan Grupos
forward
global type w_mad_repo_31 from w_master_main_rep
end type
type tab_1 from tab within w_mad_repo_31
end type
...
type dw_depto_0 from datawindow within w_mad_repo_31
end type
type dw_depto_5 from datawindow within w_mad_repo_31
end type
type dw_depto_4 from datawindow within w_mad_repo_31
end type
type dw_depto_3 from datawindow within w_mad_repo_31
end type
type dw_depto_2 from datawindow within w_mad_repo_31
end type
type dw_depto_1 from datawindow within w_mad_repo_31
end type
end forward
...
integer width = 4014
integer height = 2804
string title = "Reporte de Grupos Sesionados"
...
global w_mad_repo_31 w_mad_repo_31
...
type st_sistema from w_master_main_rep`st_sistema within w_mad_repo_31
end type

type p_ibero from w_master_main_rep`p_ibero within w_mad_repo_31
end type
...
IF ( Total > 0 and Plan_GO = 1)  THEN
	...
	IF dw_2x.retrieve(Departamentos,Salones,le_periodo,Anio_X,ls_array_nivel[], ii_tipo_imp) <= 0 Then
...
IF (rb_1x.checked =TRUE) THEN 
	// Ordenamiento por Cve_Materia 
   	dw_2x.setsort("cve_mat A, gpo A, fecha_duracion A, horario_cve_dia A ")
	dw_2x.sort()
	...
	IF (cbx_2x.checked=TRUE) THEN 
		...
		dw_2x.setsort("materias_cve_coordinacion A, cve_mat A, gpo A, fecha_duracion A, horario_cve_dia A ")
		dw_2x.sort()
		// Se recalcula el grupo
		dw_2x.GroupCalc ( )
	END IF
...
IF (rb_2x.checked=TRUE) THEN 
	...
	dw_2x.setsort("grupos_sesionado A, cve_mat A, gpo A, fecha_duracion A, horario_cve_dia A ")
	dw_2x.sort()
	// Se recalcula el grupo
	dw_2x.GroupCalc ( )
	...
	IF (cbx_2x.checked=TRUE) THEN 
		// Se ordenan por departamento y por clave del profesor
	   	dw_2x.setsort("materias_cve_coordinacion A, grupos_sesionado A, cve_mat A, gpo A, fecha_duracion A, horario_cve_dia A ")
		dw_2x.sort()
		// Se recalcula el grupo
		dw_2x.GroupCalc ( )
	END IF
...
IF (rb_3x.checked=TRUE) THEN 
	...
	dw_2x.setsort("grupos_fecha_inicio A, grupos_fecha_fin A, fecha_duracion A, cve_mat A, gpo A, horario_cve_dia A ")
	dw_2x.sort()
	// Se recalcula el grupo
	dw_2x.GroupCalc ( )
	...
	IF (cbx_2x.checked=TRUE) THEN 
		// Se ordenan por departamento y por nombre del profesor
      	dw_2x.setsort("materias_cve_coordinacion A, grupos_fecha_inicio A, grupos_fecha_fin A, fecha_duracion A, cve_mat A, gpo A, horario_cve_dia A ")
		dw_2x.sort()
		// Se recalcula el grupo
		dw_2x.GroupCalc ( )
	END IF
...
integer taborder = 31
boolean bringtotop = true
string dataobject = "dw_repo_mad_31_gx"
boolean hscrollbar = true
boolean vscrollbar = true
boolean resizable = true
