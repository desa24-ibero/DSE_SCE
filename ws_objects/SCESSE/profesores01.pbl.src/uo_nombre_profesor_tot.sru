$PBExportHeader$uo_nombre_profesor_tot.sru
forward
global type uo_nombre_profesor_tot from userobject
end type
type st_1 from statictext within uo_nombre_profesor_tot
end type
type em_digito from editmask within uo_nombre_profesor_tot
end type
type st_2 from statictext within uo_nombre_profesor_tot
end type
type r_2 from rectangle within uo_nombre_profesor_tot
end type
type dw_nombre_alumno from datawindow within uo_nombre_profesor_tot
end type
type r_3 from rectangle within uo_nombre_profesor_tot
end type
type cbx_nuevo from checkbox within uo_nombre_profesor_tot
end type
type r_1 from rectangle within uo_nombre_profesor_tot
end type
type em_cve_profesor from editmask within uo_nombre_profesor_tot
end type
end forward
...
global uo_nombre_profesor_tot uo_nombre_profesor_tot
...
on uo_nombre_profesor_tot.create
...
on uo_nombre_profesor_tot.destroy
destroy(this.st_1)
...
if cbx_nuevo.checked = false then
	...
	if gtr_sce.sqlerrtext = "Select returned more than one row" then
		...
		openwithparm(w_profesoresnombre_tot,origen)
		if w_profesoresnombre_tot.dw_nombre.retrieve(ls_nombre,ls_apat,ls_amat)  = 0 then
...
event getfocus;if isvalid(w_profesoresnombre_tot) then
	close(w_profesoresnombre_tot)
end if
end event
