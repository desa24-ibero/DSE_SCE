$PBExportHeader$w_reporte_asistencia_semanal.srw
forward
global type w_reporte_asistencia_semanal from w_ancestral
end type
type gb_1 from groupbox within w_reporte_asistencia_semanal
end type
type dw_reporte_asistencia from uo_dw_reporte within w_reporte_asistencia_semanal
end type
type st_periodo from statictext within w_reporte_asistencia_semanal
end type
type st_anio from statictext within w_reporte_asistencia_semanal
end type
type cb_guardartotales from commandbutton within w_reporte_asistencia_semanal
end type
type dw_coordinacion from datawindow within w_reporte_asistencia_semanal
end type
type vsb_coordinacion from vscrollbar within w_reporte_asistencia_semanal
end type
type rb_todas from radiobutton within w_reporte_asistencia_semanal
end type
type rb_semanas from radiobutton within w_reporte_asistencia_semanal
end type
type gb_2 from groupbox within w_reporte_asistencia_semanal
end type
type rb_materia from radiobutton within w_reporte_asistencia_semanal
end type
type rb_profesor from radiobutton within w_reporte_asistencia_semanal
end type
type em_num_semana from editmask within w_reporte_asistencia_semanal
end type
type st_1 from statictext within w_reporte_asistencia_semanal
end type
type st_2 from statictext within w_reporte_asistencia_semanal
end type
end forward
...
em_num_semana em_num_semana
st_1 st_1
st_2 st_2
end type
global w_reporte_asistencia_semanal w_reporte_asistencia_semanal

type variables
int ii_anio, ii_periodo, ii_cve_coordinacion, ii_num_semana
uo_periodo_servicios iuo_periodo_servicios
end variables

on w_reporte_asistencia_semanal.create
int iCurrent
call super::create
...
this.em_num_semana=create em_num_semana
...
this.Control[iCurrent+13]=this.em_num_semana
this.Control[iCurrent+14]=this.st_1
this.Control[iCurrent+15]=this.st_2
end on

on w_reporte_asistencia_semanal.destroy
call super::destroy
...
dw_reporte_asistencia.DataObject = "d_reporte_asistencia_por_prof_semanal"
...
this.Title = "Reporte de asistencia semanal generado por "+ls_descripcion_usuario+" de la coordinacion "+string(ii_cve_coordinacion)

if f_obten_periodo(ii_periodo, ii_anio, 1) = 1 then
	dw_reporte_asistencia.DataObject = "d_reporte_asistencia_por_prof_semanal"
	dw_reporte_asistencia.SetTransObject(gtr_sce)
	if ii_periodo <> li_periodo_mi or ii_anio <>li_anio_mi then
		dw_reporte_asistencia.DataObject = 'd_reporte_asistencia_por_prof_hist_seman'		
		dw_reporte_asistencia.SetTransObject(gtr_sce)
	end if
...
// Se modifica la decodificación de el periodo del computed field.
iuo_periodo_servicios.f_modifica_decodificacion(dw_reporte_asistencia, "compute_1", "L", "a_periodo")
...
type p_uia from w_ancestral`p_uia within w_reporte_asistencia_semanal
end type
...
string dataobject = "d_reporte_asistencia_por_prof_semanal"
...
event carga;int li_cve_coordinacion, li_num_semana
string ls_num_semana

ls_num_semana=em_num_semana.text
if not isnumber(ls_num_semana) then
	Goto Error
else
	li_num_semana= integer(ls_num_semana)
	if li_num_semana <0 or li_num_semana >53 then
		Goto Error
	else
		ii_num_semana= li_num_semana		
	end if	
end if
if ii_cve_coordinacion = 9999 then
...
return retrieve(ii_periodo, ii_anio, li_cve_coordinacion, ii_num_semana)

Error:
	MessageBox("Semana Inválida","Favor de escribir un número de semana válido",StopSign!)
	return 0
...
if rb_profesor.checked then	
	dw_reporte_asistencia.DataObject = "d_reporte_asistencia_prof_sem_semanal"	
end if
...
if ii_periodo<> li_periodo_mi or ii_anio<>li_anio_mi then
	if rb_profesor.checked then
		dw_reporte_asistencia.DataObject = 'd_reporte_asist_prof_sem_hist_semanal'				
	end if
	dw_reporte_asistencia.SetTransObject(gtr_sce)
end if
...
dw_reporte_asistencia.DataObject = "d_reporte_asistencia_semanal"
dw_reporte_asistencia.SetTransObject(gtr_sce)

if ii_periodo<> li_periodo_mi or ii_anio<>li_anio_mi then
	dw_reporte_asistencia.DataObject = 'd_reporte_asistencia_hist_semanal'		
	dw_reporte_asistencia.SetTransObject(gtr_sce)
end if
