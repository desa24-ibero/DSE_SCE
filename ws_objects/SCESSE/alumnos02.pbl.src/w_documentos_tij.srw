$PBExportHeader$w_documentos_tij.srw
forward
global type w_documentos_tij from w_master_main
end type
type r_1 from rectangle within w_documentos_tij
end type
type dw_doc from uo_master_dw within w_documentos_tij
end type
type dw_telefono from uo_master_dw within w_documentos_tij
end type
type st_replica from statictext within w_documentos_tij
end type
type dw_adeuda from uo_master_dw within w_documentos_tij
end type
type r_2 from rectangle within w_documentos_tij
end type
type uo_nombre from uo_nombre_alumno_2013 within w_documentos_tij
end type
type st_1 from statictext within w_documentos_tij
end type
type em_campo from editmask within w_documentos_tij
end type
type rb_alumno from radiobutton within w_documentos_tij
end type
type st_2 from statictext within w_documentos_tij
end type
type dw_recibo_documentos from datawindow within w_documentos_tij
end type
end forward
...
global w_documentos_tij w_documentos_tij
...
this.Control[iCurrent+12]=this.dw_recibo_documentos
end on

on w_documentos_tij.destroy
call super::destroy
...
event ue_imprime;call super::ue_imprime;Int		le_flag
Int		li_contador
Int		li_cve_flag_documento
Long	ll_cuenta

IF dw_adeuda.RowCount () = 0 THEN Return

FOR li_contador = 1 TO dw_adeuda.RowCount ()
	li_cve_flag_documento = dw_adeuda.Object.cve_flag_documento [ li_contador ]
	
	IF li_cve_flag_documento <> 3 THEN
		MessageBox ( "Aviso" , "No es posible imprimir el recibo de documentos, hay documentos que aún no están aprobados" )
		Return
	END IF
NEXT

ll_cuenta = Long ( uo_nombre.of_obten_cuenta ( ) )
dw_recibo_documentos.Reset ( )
dw_recibo_documentos.SetTransObject ( gtr_sce )
dw_recibo_documentos.Retrieve ( ll_cuenta )
...
type st_sistema from w_master_main`st_sistema within w_documentos_tij
end type

type p_ibero from w_master_main`p_ibero within w_documentos_tij
end type
...
integer height = 1312
integer taborder = 60
string dataobject = "dw_documentos_adeuda_tij"
...
string title = "none"
string dataobject = "dw_recibo_documentos_tij"
boolean resizable = true
