$PBExportHeader$w_reporte_titulados.srw
forward
global type w_reporte_titulados from window
end type
type tab_1 from tab within w_reporte_titulados
end type
type tabpage_general from userobject within tab_1
end type
type uo_nivel_general from uo_nivel_rbutton within tabpage_general
end type
type dw_3 from datawindow within tabpage_general
end type
type uo_nivel_old from uo_mostrar_grado within tabpage_general
end type
...
type tabpage_general from userobject within tab_1
uo_nivel_general uo_nivel_general
dw_3 dw_3
uo_nivel_old uo_nivel_old
rb_1 rb_1
rb_2 rb_2
rb_3 rb_3
...
type uo_individual_nivel from uo_nivel_rbutton within tabpage_individual
end type
type uo_nivel_2_old from uo_mostrar_grado within tabpage_individual
end type
...
type tabpage_individual from userobject within tab_1
uo_individual_nivel uo_individual_nivel
uo_nivel_2_old uo_nivel_2_old
lb_1 lb_1
dw_2x dw_2x
cb_5x cb_5x
rb_3x rb_3x
...
type uo_opcion_nivel from uo_nivel_rbutton within tabpage_opcion
end type
type uo_nivel_3_old from uo_mostrar_grado within tabpage_opcion
end type
...
type tabpage_opcion from userobject within tab_1
uo_opcion_nivel uo_opcion_nivel
uo_nivel_3_old uo_nivel_3_old
dw_3w dw_3w
rb_1w rb_1w
rb_2w rb_2w
rb_3w rb_3w
...
global w_reporte_titulados w_reporte_titulados
...
tab_1.tabpage_general.uo_nivel_general.f_genera_nivel( "V", "L", "L", gtr_sce, "S", "N")  
tab_1.tabpage_individual.uo_individual_nivel.f_genera_nivel( "V", "L", "L", gtr_sce, "S", "N")    
tab_1.tabpage_opcion.uo_opcion_nivel.f_genera_nivel( "V", "L", "L", gtr_sce, "S", "N")    
...
this.uo_nivel_general=create uo_nivel_general
this.dw_3=create dw_3
this.uo_nivel_old=create uo_nivel_old
...
this.Control[]={this.uo_nivel_general,&
this.dw_3,&
this.uo_nivel_old,&
this.rb_1,&
this.rb_2,&
...
destroy(this.uo_nivel_general)
destroy(this.dw_3)
destroy(this.uo_nivel_old)
destroy(this.rb_1)
destroy(this.rb_2)
destroy(this.rb_3)
...
ls_nivel = uo_nivel_general.cve_nivel 
IF ls_nivel = "A" THEN ls_nivel = "X" 
...
if isdate(em_1.text) and isdate(em_2.text) then
	if date(em_2.text)>=date(em_1.text) then
			...
			ll_rows = dw_1.retrieve(date(em_1.text),date(em_2.text),carrerax, ls_nivel)
			if ll_rows >0 then
				ldttm_fecha_hoy = fecha_servidor(gtr_sce)
				ls_fecha_hoy = string(ldttm_fecha_hoy,"dd/mm/yyyy")
				dw_1.object.fecha_hoy.text = ls_fecha_hoy				
			elseif ll_rows=-1 then
				MessageBox("Error de consulta", "No es posible consultar titulados",StopSign!)
			end if
...
if (rb_1.checked=TRUE) then
   dw_1.setsort("alumnos_nombre_completo A, carreras_carrera A")
	dw_1.sort()
   if (cbx_2.checked=TRUE) then
      dw_1.setsort("carreras_carrera A, alumnos_nombre_completo A")
		dw_1.sort()
	end if	
end if
...
integer height = 1068
integer taborder = 5
string dataobject = "d_reporte_titulados_carrera"
end type
...
this.uo_individual_nivel=create uo_individual_nivel
this.uo_nivel_2_old=create uo_nivel_2_old
...
this.Control[]={this.uo_individual_nivel,&
this.uo_nivel_2_old,&
this.lb_1,&
this.dw_2x,&
this.cb_5x,&
...
destroy(this.uo_individual_nivel)
destroy(this.uo_nivel_2_old)
destroy(this.lb_1)
destroy(this.dw_2x)
destroy(this.cb_5x)
destroy(this.rb_3x)
...
on uo_individual_nivel.destroy
call uo_nivel_rbutton::destroy
end on
...
if (rb_1x.checked=TRUE) then
   dw_1x.setsort("alumnos_nombre_completo A, carreras_carrera A")
	dw_1x.sort()
   if (cbx_2x.checked=TRUE) then
      dw_1x.setsort("carreras_carrera A, alumnos_nombre_completo A")
		dw_1x.sort()
	end if	
end if
...
ls_nivel = uo_individual_nivel.cve_nivel 
IF ls_nivel = "A" THEN ls_nivel = "X" 
...
if Total > 0 then
   contador=1
	...
	NEXT
	ll_rows = dw_1x.retrieve(Number, ls_nivel)
	if ll_rows >0 then
		ldttm_fecha_hoy = fecha_servidor(gtr_sce)
		ls_fecha_hoy = string(ldttm_fecha_hoy,"dd/mm/yyyy")
		dw_1x.object.fecha_hoy.text = ls_fecha_hoy				
	elseif ll_rows=-1 then
		MessageBox("Error de consulta", "No es posible consultar titulados",StopSign!)
	end if
...
integer height = 1088
integer taborder = 4
string dataobject = "d_reporte_titulados_carr_ind"
end type
...
this.uo_opcion_nivel=create uo_opcion_nivel
this.uo_nivel_3_old=create uo_nivel_3_old
...
this.Control[]={this.uo_opcion_nivel,&
this.uo_nivel_3_old,&
this.dw_3w,&
this.rb_1w,&
this.rb_2w,&
...
destroy(this.uo_opcion_nivel)
destroy(this.uo_nivel_3_old)
destroy(this.dw_3w)
destroy(this.rb_1w)
destroy(this.rb_2w)
destroy(this.rb_3w)
...
//	ls_nivel = "X"
//end if
ls_nivel = uo_opcion_nivel.cve_nivel 
IF ls_nivel = "A" THEN ls_nivel = "X" 
...
if isdate(em_1w.text) and isdate(em_2w.text) then
	if date(em_2w.text)>=date(em_1w.text) then
			...
			ll_rows= dw_1w.retrieve(date(em_1w.text),date(em_2w.text),carrera2,opcion, ls_nivel)
			if ll_rows >0 then
				ldttm_fecha_hoy = fecha_servidor(gtr_sce)
				ls_fecha_hoy = string(ldttm_fecha_hoy,"dd/mm/yyyy")
				dw_1w.object.fecha_hoy.text = ls_fecha_hoy				
			elseif ll_rows=-1 then
				MessageBox("Error de consulta", "No es posible consultar titulados",StopSign!)
			end if
...
if (rb_1w.checked=TRUE) then
   dw_1w.setsort("alumnos_nombre_completo A, carreras_carrera A")
	dw_1w.sort()
   if (cbx_2w.checked=TRUE) then
      dw_1w.setsort("carreras_carrera A, alumnos_nombre_completo A")
		dw_1w.sort()
	end if	
end if
...
integer height = 972
integer taborder = 1
string dataobject = "d_reporte_titulados_carr_opc"
end type
