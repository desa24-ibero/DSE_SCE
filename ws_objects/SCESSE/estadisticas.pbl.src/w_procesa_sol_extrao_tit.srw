$PBExportHeader$w_procesa_sol_extrao_tit.srw
forward
global type w_procesa_sol_extrao_tit from window
end type
type uo_1 from uo_per_ani_ets within w_procesa_sol_extrao_tit
end type
type p_1 from picture within w_procesa_sol_extrao_tit
end type
type st_1 from statictext within w_procesa_sol_extrao_tit
end type
type dw_nivel from datawindow within w_procesa_sol_extrao_tit
end type
type cb_pagos from commandbutton within w_procesa_sol_extrao_tit
end type
type dw_extrao_tit from datawindow within w_procesa_sol_extrao_tit
end type
type cb_procesar_autorizaciones from commandbutton within w_procesa_sol_extrao_tit
end type
type cbx_cursativas from checkbox within w_procesa_sol_extrao_tit
end type
type rb_posgrado from radiobutton within w_procesa_sol_extrao_tit
end type
type rb_licenciatura from radiobutton within w_procesa_sol_extrao_tit
end type
type uo_coordinacion from uo_dgmu_coordinaciones within w_procesa_sol_extrao_tit
end type
type gb_nivel from groupbox within w_procesa_sol_extrao_tit
end type
type st_3 from statictext within w_procesa_sol_extrao_tit
end type
type dw_estadisticas from datawindow within w_procesa_sol_extrao_tit
end type
type cb_1 from commandbutton within w_procesa_sol_extrao_tit
end type
type em_anio from editmask within w_procesa_sol_extrao_tit
end type
type rb_otonio from radiobutton within w_procesa_sol_extrao_tit
end type
type rb_verano from radiobutton within w_procesa_sol_extrao_tit
end type
type rb_primavera from radiobutton within w_procesa_sol_extrao_tit
end type
type dw_1x from datawindow within w_procesa_sol_extrao_tit
end type
type gb_8 from groupbox within w_procesa_sol_extrao_tit
end type
type gb_20 from groupbox within w_procesa_sol_extrao_tit
end type
type gb_2 from groupbox within w_procesa_sol_extrao_tit
end type
type gb_1 from groupbox within w_procesa_sol_extrao_tit
end type
end forward
...
global w_procesa_sol_extrao_tit w_procesa_sol_extrao_tit
...
on w_procesa_sol_extrao_tit.create
...
on w_procesa_sol_extrao_tit.destroy
...
//Se genera la conexion a utilizar para ejecutar los stored procedures
li_chk	=	f_conecta_pas_parametros_sp_bd(itr_parametros_iniciales,itr_trans_sp,is_controlescolar_cnx,gs_usuario,gs_password,1)
...
if ls_nivel = 'P' then
	dw_estadisticas.dataobject = 	'd_solicitudes_extrao_tit_pos2'	
	dw_extrao_tit.dataobject = 	'd_extrao_tit_lic'
else
	dw_estadisticas.dataobject = 	'd_solicitudes_extrao_tit_lic2'
	dw_extrao_tit.dataobject = 	'd_extrao_tit_lic'
...
if isvalid(itr_trans_sp) then
	DESTROY itr_trans_sp;
end if
f_obten_titulo(w_principal)
...
fontfamily fontfamily = swiss!
string facename = "Tahoma"
string text = "Procesar Pagos"
end type

event clicked;integer li_confirmacion
long ll_rows, ll_row, ll_cuenta
integer li_autorizado, li_pagado, li_update
integer li_num_horas, li_cve_concepto, li_sit_saldo_concepto, li_sit_asocia_pagos
decimal ld_codigo_sp
string ls_mensaje_sp, ls_mensaje_sp2, ls_order_num
integer li_periodo, li_anio, li_resultado, li_cursativa , li_asociado
long ll_cve_mat
...
for ll_row=1  to ll_rows
	/*statementblock*/
	ll_cuenta = dw_estadisticas.GetItemNumber(ll_row, 'examen_extrao_titulo_sol_cuenta')
	ll_cve_mat= dw_estadisticas.GetItemNumber(ll_row, 2)
	
	li_periodo = dw_estadisticas.GetItemNumber(ll_row, 'examen_extrao_titulo_sol_periodo')
	li_anio = dw_estadisticas.GetItemNumber(ll_row, 'examen_extrao_titulo_sol_anio')

	li_cve_concepto = dw_estadisticas.GetItemNumber(ll_row, 'examen_extrao_titulo_sol_cve_concepto')
	li_num_horas = dw_estadisticas.GetItemNumber(ll_row, 'examen_extrao_titulo_sol_num_horas')
	...
	ls_order_num =  dw_estadisticas.GetItemString(ll_row, 'examen_extrao_titulo_sol_order_num')
	
	ll_cve_mat = dw_estadisticas.GetItemNumber(ll_row, 'examen_extrao_titulo_sol_cve_mat')
	li_asociado = dw_estadisticas.GetItemNumber(ll_row, 'examen_extrao_titulo_sol_asociado')
	if isnull(li_asociado) then li_asociado=0
//	li_cursativa mat_prerrequisito_cursativa o mat_prerreq_pos_cursativa
	li_cursativa = dw_estadisticas.GetItemNumber(ll_row, 19)


//Si la materia no es cursativa en automático la autoriza
	if li_cursativa = 0 then
		dw_estadisticas.SetItem(ll_row, 'examen_extrao_titulo_sol_autorizado', 1)			
	end if


	//Antes de 2011-10-12 Solo si el trámite NO se encuentra pagado, procesa para ver si tiene el saldo y genera la asociación
	//Solo si el trámite Se encuentra Autorizado, procesa para ver si tiene el saldo y genera la asociación
	if li_autorizado=1 and li_pagado= 1 then
//		li_sit_saldo_concepto =itr_trans_sp.f_sit_saldo_pago_concepto(ll_cuenta, li_periodo, li_anio, li_cve_concepto, &
//										li_num_horas, ld_codigo_sp, ls_mensaje_sp)

		li_sit_saldo_concepto = f_sit_saldo_concepto(ll_cuenta, li_periodo, li_anio, li_cve_concepto, &
										li_num_horas, itr_trans_sp, ld_codigo_sp, ls_mensaje_sp)


//Si hay un error																				
		if li_sit_saldo_concepto = -1 then
			MessageBox("Error de Ejecución","No es posiblr ejecutarr", StopSign!)
//Si se tiene el saldo			
		elseif  ld_codigo_sp = 1 then
			if li_asociado= 0 then
				li_sit_asocia_pagos = f_sit_asocia_pagos(ll_cuenta, li_periodo, li_anio, li_cve_concepto, &
										li_num_horas, itr_trans_sp, li_resultado, ls_mensaje_sp2, ls_order_num, ll_cve_mat)
				if li_resultado =-1 then
					MessageBox("Error","No es posible asociar los pagos del alumno con cuenta["+string(ll_cuenta)+"] y materia ["+string(ll_cve_mat)+"]~n"+ls_mensaje_sp2, StopSign!)
				ELSEif li_resultado =1 then
						//PAGADO = 1
						dw_estadisticas.SetItem(ll_row, 'examen_extrao_titulo_sol_pagado',1)
						//Si se ejecuto correctamente, la marca como asociada
						dw_estadisticas.SetItem(ll_row, 'examen_extrao_titulo_sol_asociado', 1)			
				end if			
			end if
		end if
	end if
...
//Debido a que el sp de sp_sit_aplica_asocia_pagos realiza el update a la columna de asociado esto se vuelve innecesario
//li_update = dw_estadisticas.Update()
//if li_update = 1 then
//	commit using gtr_sce;
//	MessageBox("Información","Las solicitudes fueron Procesadas", Information!)
//else
//	rollback using gtr_sce;
//	MessageBox("Error","No es posible almacenar las Solicitudes Autorizadas", StopSign!)
//	return
//end if
	MessageBox("Información","Las solicitudes fueron Procesadas Exitosamente", Information!)
...
string dataobject = "d_solicitudes_extrao_tit_lic2"
boolean maxbox = true
boolean hscrollbar = true
boolean vscrollbar = true
boolean resizable = true
