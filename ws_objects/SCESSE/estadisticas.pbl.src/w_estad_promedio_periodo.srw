$PBExportHeader$w_estad_promedio_periodo.srw
forward
global type w_estad_promedio_periodo from window
end type
type uo_periodo from uo_periodo_variable_tipos within w_estad_promedio_periodo
end type
type uo_carrera from uo_carreras within w_estad_promedio_periodo
end type
type uo_coordinacion from uo_coordinaciones within w_estad_promedio_periodo
end type
type gb_carreras from groupbox within w_estad_promedio_periodo
end type
type gb_coordinacion from groupbox within w_estad_promedio_periodo
end type
type st_3 from statictext within w_estad_promedio_periodo
end type
type dw_estadisticas from datawindow within w_estad_promedio_periodo
end type
type cb_1 from commandbutton within w_estad_promedio_periodo
end type
type em_anio from editmask within w_estad_promedio_periodo
end type
type rb_otonio from radiobutton within w_estad_promedio_periodo
end type
type rb_verano from radiobutton within w_estad_promedio_periodo
end type
type rb_primavera from radiobutton within w_estad_promedio_periodo
end type
type dw_1x from datawindow within w_estad_promedio_periodo
end type
type gb_8 from groupbox within w_estad_promedio_periodo
end type
type gb_11 from groupbox within w_estad_promedio_periodo
end type
type gb_20 from groupbox within w_estad_promedio_periodo
end type
type gb_1 from groupbox within w_estad_promedio_periodo
end type
end forward
...
string title = "Estadística de Promedios por Periodo"
...
global w_estad_promedio_periodo w_estad_promedio_periodo

type variables
int periodo_x
end variables

on w_estad_promedio_periodo.create
...
on w_estad_promedio_periodo.destroy
...
string dataobject = "d_promedio_periodo"
...
//dw_estadisticas.Retrieve(li_periodo, li_anio, ll_cve_carrera, ll_cve_coordinacion)
...
FOR le_index = 1 TO UPPERBOUND(le_periodo[])
	...
	//Necesario por la creacion de la tabla temporal
	gtr_sce.Autocommit= true
	le_resultado = lds_paso.Retrieve(le_periodo[le_index], li_anio, ll_cve_carrera, ll_cve_coordinacion, ls_tipo_periodo[le_index])
	gtr_sce.Autocommit= false

	IF le_resultado < 0 THEN 
		MESSAGEBOX("Error", "Se produjo un error al recuperar la información.") 
		RETURN -1
	ELSEIF le_resultado = 0 THEN 
		CONTINUE
	ELSE
		lds_paso.ROWSCOPY(1, lds_paso.ROWCOUNT(), PRIMARY!, lds_temporal, lds_temporal.ROWCOUNT() + 1, PRIMARY!) 
	END IF
