...
if f_conecta_con_parametros_bd(gtr_sce, itr_esta, 3 )=0 then
	messagebox("Atención...", "Problemas al conectarse a la bd de estacionamiento")
	return -1
end if 

if f_conecta_pas_parametros_bd(gtr_sce, itr_sit, 2, gs_usuario, gs_password )=0 then
...
int li_detenido, li_cve_tramite, li_baja_total_alumno, li_update
...
//Baja Total y estatus Pendiente
	if li_cve_tramite = 4 and li_cve_estatus_solicitud_tramite= 1 and il_num_intentos >= ll_num_intentos then
		...
		li_baja_total_alumno = f_baja_total_alumno(ll_cuenta, ltr_sit, ltr_esta, itr_mssqlserver,ls_mensaje_bt )
...
//Comentado 2010-11-08 para permitir la ejecución cuando falla por un solo alumno			
//			if f_trans_sql_syb_detenido_update(0,ls_error) <> 1 then
...
//				return -1
//			end if
//			MessageBox("ERROR", "No es posible efectuar la baja total", StopSign!)	
