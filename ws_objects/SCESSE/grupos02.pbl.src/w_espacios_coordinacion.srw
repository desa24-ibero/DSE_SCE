$PBExportHeader$w_espacios_coordinacion.srw
forward
global type w_espacios_coordinacion from window
end type
type lb_tipo_grupo_clase from listbox within w_espacios_coordinacion
end type
type st_tpo_gpo_clase from statictext within w_espacios_coordinacion
end type
type ddlb_coordinaciones from dropdownlistbox within w_espacios_coordinacion
end type
type st_coordinacion from statictext within w_espacios_coordinacion
end type
type ddlb_departamentos from dropdownlistbox within w_espacios_coordinacion
end type
type st_departamento from statictext within w_espacios_coordinacion
end type
type ddlb_divisiones from dropdownlistbox within w_espacios_coordinacion
end type
type st_division from statictext within w_espacios_coordinacion
end type
type st_horario_vespertino from statictext within w_espacios_coordinacion
end type
type st_horario_matutino from statictext within w_espacios_coordinacion
end type
type st_tamanio_bloque_hrs from statictext within w_espacios_coordinacion
end type
type st_horario_vesp from statictext within w_espacios_coordinacion
end type
type st_horario_mat from statictext within w_espacios_coordinacion
end type
type cb_imprimir from commandbutton within w_espacios_coordinacion
end type
type cbx_mostrar_porcentajes from checkbox within w_espacios_coordinacion
end type
type cb_consultar from commandbutton within w_espacios_coordinacion
end type
type cbx_10_o_menos_inscritos from checkbox within w_espacios_coordinacion
end type
type ddlb_horario from dropdownlistbox within w_espacios_coordinacion
end type
type st_horario from statictext within w_espacios_coordinacion
end type
type cb_1 from commandbutton within w_espacios_coordinacion
end type
type ddlb_periodo from dropdownlistbox within w_espacios_coordinacion
end type
type st_4 from statictext within w_espacios_coordinacion
end type
type st_3 from statictext within w_espacios_coordinacion
end type
type em_anio from editmask within w_espacios_coordinacion
end type
type tab_1 from tab within w_espacios_coordinacion
end type
...
type st_clase_aula from statictext within w_espacios_coordinacion
end type
type st_dia from statictext within w_espacios_coordinacion
end type
type ddlb_dias from dropdownlistbox within w_espacios_coordinacion
end type
type gb_3 from groupbox within w_espacios_coordinacion
end type
type gb_1 from groupbox within w_espacios_coordinacion
end type
type gb_2 from groupbox within w_espacios_coordinacion
end type
type gb_4 from groupbox within w_espacios_coordinacion
end type
type gb_5 from groupbox within w_espacios_coordinacion
end type
type ddlb_clase_aula from dropdownlistbox within w_espacios_coordinacion
end type
end forward
...
string title = "Indicadores del Plan Grupos"
...
global w_espacios_coordinacion w_espacios_coordinacion
...
Int		ii_anio
Int		ii_periodo
String	is_tipo_horario, is_turno, is_coordinacion, is_inscritos_minimos
Int		ii_inscritos
...
Int     ii_horario_matutino
Int     ii_horario_vespertino
long   il_inscritos_minimos
...
DECLARE cur_coordinaciones CURSOR FOR
	SELECT	coordinacion
	FROM		coordinaciones WHERE cve_coordinacion <> 9999
...
if is_coordinacion<> "Todos" then
	ddlb_coordinaciones.SelectItem ( is_coordinacion , 0 )
end if
...
on w_espacios_coordinacion.create
...
on w_espacios_coordinacion.destroy
...
Int 	li_obten_parametros_indicadores
...
elseif ii_cve_coordinacion = -1 then
	...
	ddlb_coordinaciones.enabled = false
	ddlb_coordinaciones.visible = true
end if

li_obten_parametros_indicadores = f_obten_parametros_indicadores(1, il_inscritos_minimos)
IF li_obten_parametros_indicadores<>0 THEN
	MessageBox("Error en lectura de parametros_indicadores", "Los inscritos mínimos serán [10]",StopSign!)
	il_inscritos_minimos = 10
END IF

is_inscritos_minimos = '['+string(il_inscritos_minimos)+ '] o menos alumnos inscritos'
cbx_10_o_menos_inscritos.text = is_inscritos_minimos
...
//si es coordinación desaparece el reporte condensado, de lo contrario muéstralo
if ii_cve_coordinacion <>0 then
	tab_1.tabpage_condensado.visible = false
	tab_1.tabpage_condensado.enabled = false	
end if
...
ls_tipo_grupo_clase = SelectedItem() 
...
String	ls_tipo_grupo_clase, ls_turno
Int		li_indice
...
IF cbx_10_o_menos_inscritos.Checked THEN
	ii_inscritos = il_inscritos_minimos	
	Tab_1.Tabpage_coordinacion.dw_coordinacion.Object.t_inscritos.Text = is_inscritos_minimos
	Tab_1.tabpage_condensado.dw_reporte_condensado.Object.t_inscritos.Text = is_inscritos_minimos
ELSE
	ii_inscritos = 9999
	Tab_1.Tabpage_coordinacion.dw_coordinacion.Object.t_inscritos.Text = ''
	Tab_1.tabpage_condensado.dw_reporte_condensado.Object.t_inscritos.Text = ''
...
IF is_tipo_horario = 'MATUTINO' THEN
	...
	ls_turno= 'TURNO MATUTINO'
END IF

IF is_tipo_horario = 'VESPERTINO' THEN
	li_horario_vespertino = 9999
	li_horario_matutino = 0
	ls_turno= 'TURNO VESPERTINO'
END IF

IF is_tipo_horario = 'TODOS' THEN
	li_horario_matutino = 9999
	li_horario_vespertino = 9999
	ls_turno= 'AMBOS TURNOS'
END IF

ii_horario_matutino = li_horario_matutino
ii_horario_vespertino = li_horario_vespertino
is_turno = ls_turno
...
IF Tab_1.Tabpage_coordinacion.dw_coordinacion.Retrieve ( ii_anio , ii_periodo , ii_cve_dia ,  li_horario_matutino, li_horario_vespertino, ii_cve_coordinacion, ii_inscritos ) = 0 THEN
...
IF Tab_1.tabpage_condensado.dw_reporte_condensado.Retrieve ( ii_anio , ii_periodo , ii_cve_dia , ii_clase_aula , li_cve_tipo_grupo_clase [  ] ,li_horario_matutino, li_horario_vespertino, ii_inscritos) = 0 THEN
...
event clicked;
//Copia el valor leido de los parámetros o 9999 para traer a todos
IF Checked THEN
	ii_inscritos = il_inscritos_minimos	
...
DECLARE  lsp_genera_grupos_clase PROCEDURE FOR dbo.sp_genera_bloques_grupos
@periodo= :ii_periodo, 
@anio = :ii_anio, 
...
integer taborder = 30
string title = "none"
string dataobject = "d_espacios_coordinacion"
boolean hscrollbar = true
boolean vscrollbar = true
boolean resizable = true
boolean livescroll = true
...
event retrieveend;long ll_row, ll_row_count, ll_cve_tipo_grupo_clase, ll_clase_aula, ll_cve_dia, ll_num_horas, ll_num_grupos, ll_periodo,ll_anio, ll_total_horas, ll_total_grupos
long ll_total_horas_bloque, ll_total_grupos_bloque, ll_total_horas_sin_aula, ll_total_grupos_sin_aula
real lr_porcentaje_horas, lr_porcentaje_grupos
...
//	 ll_cve_tipo_grupo_clase = this.GetItemNumber(ll_row, 'cve_tipo_grupo_clase')
//	 ll_clase_aula = this.GetItemNumber(ll_row, 'clase_aula') 
	 ll_cve_dia = this.GetItemNumber(ll_row, 'cve_dia') 
	 ll_periodo = this.GetItemNumber(ll_row, 'grupo_clase_periodo') 
	 ll_anio = this.GetItemNumber(ll_row, 'grupo_clase_anio') 
	 ll_num_horas = this.GetItemNumber(ll_row, 'num_horas') 	
	 
	 
	 //Cuenta los grupos existenes para ese tipo de bloque, día y clase de aula. COLUMNA: num_grupos
	select count(*) 
 	into :ll_num_grupos
	from v_bloques
	where
			( v_bloques.cve_dia =:ll_cve_dia) AND					
			( v_bloques.periodo =  :ll_periodo) AND					 		
			( v_bloques.anio =  :ll_anio)  AND
			(	/* Filtro para Horario ( Matutino, Vespertino, o TODOS ) */
				(	/* Filtro para Horario Vespertino ( Se activa si argumento = 0 ) */
					v_bloques.hora_inicio >= ( Select hora_inicio_vespertino from parametros_grupo_clase where cve_parametros_grupo_clase = 1 AND 0 = :ii_horario_matutino ) OR
					:ii_horario_matutino = 9999
				) AND
				(	/* Filtro para Horario Matutino ( Se activa si argumento = 0 ) */
	
					v_bloques.hora_inicio < ( Select hora_inicio_vespertino from parametros_grupo_clase where cve_parametros_grupo_clase = 1 AND 0 = :ii_horario_vespertino ) OR
					:ii_horario_vespertino = 9999
				)
			) AND
			( cve_coordinacion = :ii_cve_coordinacion  )  AND	/* Filtro para Coordinación */ 
			( num_inscritos <= :ii_inscritos ) 								/* Filtro para número de Inscritos */
	...
	//Suma la cantidad de horas para esa clase de aula (o todas)  y tipo de bloque. COLUMNA: total_horas
	select sum(num_horas) 
	into 	 :ll_total_horas
	from grupo_clase
	where
			( periodo =  :ll_periodo) AND					 		
			( anio =  :ll_anio) 	AND
			(	/* Filtro para Horario ( Matutino, Vespertino, o TODOS ) */
				(	/* Filtro para Horario Vespertino ( Se activa si argumento = 0 ) */
					grupo_clase.hora_inicio >= ( Select hora_inicio_vespertino from parametros_grupo_clase where cve_parametros_grupo_clase = 1 AND 0 = :ii_horario_matutino ) OR
					:ii_horario_matutino = 9999
				) AND
				(	/* Filtro para Horario Matutino ( Se activa si argumento = 0 ) */
	
					grupo_clase.hora_inicio < ( Select hora_inicio_vespertino from parametros_grupo_clase where cve_parametros_grupo_clase = 1 AND 0 = :ii_horario_vespertino ) OR
					:ii_horario_vespertino = 9999
				)
			) AND
			( cve_coordinacion = :ii_cve_coordinacion ) AND	/* Filtro para Coordinación */ 
	...
	//Cuenta la cantidad de grupos existenes para ese tipo de bloque y clase de aula (sin importar el día). COLUMNA: total_grupos
	select count(*) 
 	into :ll_total_grupos
	from v_bloques
	where
			( periodo =  :ll_periodo) AND					 		
			( anio =   :ll_anio)   AND
			(	/* Filtro para Horario ( Matutino, Vespertino, o TODOS ) */
				(	/* Filtro para Horario Vespertino ( Se activa si argumento = 0 ) */
					hora_inicio >= ( Select hora_inicio_vespertino from parametros_grupo_clase where cve_parametros_grupo_clase = 1 AND 0 = :ii_horario_matutino ) OR
					:ii_horario_matutino = 9999
				) AND
				(	/* Filtro para Horario Matutino ( Se activa si argumento = 0 ) */
	
					hora_inicio < ( Select hora_inicio_vespertino from parametros_grupo_clase where cve_parametros_grupo_clase = 1 AND 0 = :ii_horario_vespertino ) OR
					:ii_horario_vespertino = 9999
				)
			) AND
			( cve_coordinacion = :ii_cve_coordinacion  ) AND	/* Filtro para Coordinación */ 
	...
	//Cuenta la cantidad de grupos existenes para ese día (sin importar la clase de aula). COLUMNA: total_grupos_sin_aula
	select count(*) 
 	into :ll_total_grupos_sin_aula
	from v_bloques
	where
			( cve_dia = :ll_cve_dia) AND
			( periodo =  :ll_periodo) AND					 		
			( anio =   :ll_anio)   AND
			(	/* Filtro para Horario ( Matutino, Vespertino, o TODOS ) */
				(	/* Filtro para Horario Vespertino ( Se activa si argumento = 0 ) */
	...
	lr_porcentaje_horas = 0
	lr_porcentaje_grupos = 0
	//Si el divisor es distinto de Cero, calcula el porcentaje de horas correspondiente. COLUMNA: porcentaje_horas
	if ll_total_horas<>0 then 
		lr_porcentaje_horas = ll_num_horas /ll_total_horas
	end if
	
	//Si el divisor es distinto de Cero, calcula el porcentaje de grupos correspondiente. COLUMNA: porcentaje_grupos
	if ll_total_grupos<>0 then 
		lr_porcentaje_grupos = ll_num_grupos / ll_total_grupos
	end if

	//Suma la cantidad de horas para ese tipo de bloque. COLUMNA: total_horas_bloque
	select sum(num_horas) 
	into 	 :ll_total_horas_bloque
	from grupo_clase
	where
			( periodo =  :ll_periodo) AND					 		
			( anio =  :ll_anio) 	AND	
	...
	//Suma la cantidad de horas para ese día (sin importar el tipo de bloque). COLUMNA: total_horas_sin_aula
	select sum(num_horas) 
	into 	 :ll_total_horas_sin_aula
	from grupo_clase
	where
			( cve_dia = :ll_cve_dia)   AND
			( periodo =  :ll_periodo) AND					 		
	...
	//Cuenta la cantidad de grupos existenes para ese tipo de bloque sin importar el día. COLUMNA: total_grupos_bloque
	select count(*) 
 	into :ll_total_grupos_bloque
	from v_bloques
	where
			( periodo =  :ll_periodo) AND					 		
			( anio =   :ll_anio)   AND
	...
	this.SetItem(ll_row, 'num_grupos',ll_num_grupos) 	
	
	this.SetItem(ll_row, 'total_horas',ll_total_horas) 	
	this.SetItem(ll_row, 'total_grupos',ll_total_grupos) 	

	this.SetItem(ll_row, 'total_horas_sin_aula',ll_total_horas_sin_aula) 	
	this.SetItem(ll_row, 'total_grupos_sin_aula',ll_total_grupos_sin_aula) 	


	this.SetItem(ll_row, 'total_horas_bloque',ll_total_horas_bloque) 	
	this.SetItem(ll_row, 'total_grupos_bloque',ll_total_grupos_bloque) 	

	this.SetItem(ll_row, 'porcentaje_horas',lr_porcentaje_horas) 	
	this.SetItem(ll_row, 'porcentaje_grupos',lr_porcentaje_grupos) 	

	this.SetItem(ll_row, 'turno',is_turno) 	
...
integer taborder = 30
string title = "none"
string dataobject = "d_bloques_por_dia"
boolean hscrollbar = true
boolean vscrollbar = true
boolean resizable = true
boolean livescroll = true
...
for ll_row=1 to ll_row_count
	...
	where
			( v_bloques.cve_dia =:ll_cve_dia) AND					
			( v_bloques.clase_aula =  :ll_clase_aula) AND					 		
			( v_bloques.cve_tipo_grupo_clase = :ll_cve_tipo_grupo_clase) AND
			( v_bloques.periodo =  :ll_periodo) AND					 		
			( v_bloques.anio =  :ll_anio)  AND
			(	/* Filtro para Horario ( Matutino, Vespertino, o TODOS ) */
				(	/* Filtro para Horario Vespertino ( Se activa si argumento = 0 ) */
	...
	from grupo_clase
	where
			( clase_aula =  :ll_clase_aula OR 9999=:ll_clase_aula) AND					 		
			( cve_tipo_grupo_clase = :ll_cve_tipo_grupo_clase  ) AND
			( periodo =  :ll_periodo) AND					 		
			( anio =  :ll_anio) 	AND
			(	/* Filtro para Horario ( Matutino, Vespertino, o TODOS ) */
				(	/* Filtro para Horario Vespertino ( Se activa si argumento = 0 ) */
