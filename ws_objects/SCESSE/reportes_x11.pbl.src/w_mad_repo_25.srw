$PBExportHeader$w_mad_repo_25.srw
$PBExportComments$Ventana para los Planes (nuevo..sta fe II), Altas Bajas,Cambios e Impresión)
forward
global type w_mad_repo_25 from Window
end type
type st_1 from statictext within w_mad_repo_25
end type
type vsb_dw_certificado from vscrollbar within w_mad_repo_25
end type
type dw_1 from datawindow within w_mad_repo_25
end type
end forward
...
global w_mad_repo_25 w_mad_repo_25

on w_mad_repo_25.create
...
control_escolar.toolbarsheettitle="Planes"
...
w_mad_repo_25.dw_1.event actualiza ( )
...
int Height=208
int TabOrder=20
string DataObject="dw_repo_mad_25"
end type
		...
		then
			...
			if update(true) = 1 then		
         	...
         	dw_1.sort()
				...
				"Algunos datos están incorrectos, favor de corregirlos. O bien trata de dar de Alta a un Plan ya actualizado")
			end if
	else
...
FOR Contador=1 TO VarX
     if Contador <> this.object.cve_plan[Contador] then
		EXIT
     end if
NEXT
...
Object.cve_plan[RowCount()]=Contador
