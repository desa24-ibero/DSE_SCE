$PBExportHeader$w_enlace_mat_sce_ant.srw
forward
global type w_enlace_mat_sce_ant from window
end type
type dw_5 from datawindow within w_enlace_mat_sce_ant
end type
type cb_1 from commandbutton within w_enlace_mat_sce_ant
end type
type dw_horarios_remedial_esp from datawindow within w_enlace_mat_sce_ant
end type
type dw_horarios_remedial_mat from datawindow within w_enlace_mat_sce_ant
end type
type st_5 from statictext within w_enlace_mat_sce_ant
end type
type st_4 from statictext within w_enlace_mat_sce_ant
end type
type st_fin from statictext within w_enlace_mat_sce_ant
end type
type st_ini from statictext within w_enlace_mat_sce_ant
end type
type dw_1 from uo_dw_reporte within w_enlace_mat_sce_ant
end type
type uo_1 from uo_ver_per_ani within w_enlace_mat_sce_ant
end type
type em_max from editmask within w_enlace_mat_sce_ant
end type
type em_min from editmask within w_enlace_mat_sce_ant
end type
type gb_1 from groupbox within w_enlace_mat_sce_ant
end type
type gb_2 from groupbox within w_enlace_mat_sce_ant
end type
type gb_4 from groupbox within w_enlace_mat_sce_ant
end type
type st_1 from statictext within w_enlace_mat_sce_ant
end type
type tab_1 from tab within w_enlace_mat_sce_ant
end type
...
type gb_3 from groupbox within w_enlace_mat_sce_ant
end type
end forward
...
global w_enlace_mat_sce_ant w_enlace_mat_sce_ant
...
event enlace();Long		ll_rows, ll_periodo_materias, ll_anio_materias, ll_valida_carrera, ll_valida_gpos_inex, &
			ll_cuantos_web, ll_valor, ll_cuantos		/* OSS. 03-ene-2012 */
String		ls_periodo_materias, ls_anio_materias
Integer	li_actualiza_insc_sem_ant, li_result, li_Respuesta /* OSS. 15-Sep-2011 */
...
IF wf_verifica_carreras (gi_anio, gi_periodo) = -1 OR wf_verifica_carreras (gi_anio, gi_periodo) = 1 THEN
	RETURN
END IF

//Validamos que la carrera de admision sea la misma de control escolar de los aspirantes
IF wf_verifica_paq_gpos_inexist (gi_anio, gi_periodo) = -1 OR wf_verifica_paq_gpos_inexist (gi_anio, gi_periodo) = 1 THEN
	RETURN
END IF
	...
	DO WHILE gtr_sadm.sqlcode <> 100
		...
		DECLARE cur_materias_repetidas CURSOR FOR
			SELECT count ( h.cve_dia )	,
					   ...
					   h.hora_inicio,
					   h.hora_final
			FROM paquetes_materias pm, horario h
...
on w_enlace_mat_sce_ant.create
...
on w_enlace_mat_sce_ant.destroy
