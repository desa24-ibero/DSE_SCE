$PBExportHeader$uo_gpo_division_servicios.sru
forward
global type uo_gpo_division_servicios from nonvisualobject
end type
end forward

global type uo_gpo_division_servicios from nonvisualobject
end type
global uo_gpo_division_servicios uo_gpo_division_servicios
...
FOR le_vuelta = 1 TO 2 
	...
	FOR le_pos = 1 TO ids_aspiran_inscribe.ROWCOUNT() 
		...
		FOR le_pos_paq = 1 TO lds_grupos_paquete.ROWCOUNT() 
			...
			ELSE														
				le_pos_gpo = ids_grupos_cupo_div.FIND("cve_mat = " + STRING(ll_cve_mat) + " AND gpo = '" + ls_gpo + "' AND cupo > inscritos ", 1, ids_grupos_cupo_div.ROWCOUNT() + 1) 	 
			END IF 	
		...
		NEXT  // Ciclo sobre el paquete 
	
	NEXT // Ciclo sobre los aspirantes 

NEXT  // Ciclo de segunda vuelta para cupos llenos 
...
on uo_gpo_division_servicios.create
call super::create
TriggerEvent( this, "constructor" )
end on

on uo_gpo_division_servicios.destroy
