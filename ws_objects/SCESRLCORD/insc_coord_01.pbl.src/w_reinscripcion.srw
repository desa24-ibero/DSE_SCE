$PBExportHeader$w_reinscripcion.srw
forward
global type w_reinscripcion from window
end type
type p_uia from picture within w_reinscripcion
end type
type st_1 from statictext within w_reinscripcion
end type
type dw_reinsc_mat from datawindow within w_reinscripcion
end type
type dw_horario_mat from datawindow within w_reinscripcion
end type
type dw_materias from datawindow within w_reinscripcion
end type
type dw_plan from datawindow within w_reinscripcion
end type
type uo_nombre from uo_nombre_alumno within w_reinscripcion
end type
type dw_ext_h from datawindow within w_reinscripcion
end type
type dw_prerre from datawindow within w_reinscripcion
end type
type dw_cursada from datawindow within w_reinscripcion
end type
type dw_labs from datawindow within w_reinscripcion
end type
type dw_comprobante from datawindow within w_reinscripcion
end type
type r_1 from rectangle within w_reinscripcion
end type
type dw_grupos_disp from datawindow within w_reinscripcion
end type
type dw_mat_integ from datawindow within w_reinscripcion
end type
type mensaje from structure within w_reinscripcion
end type
end forward
...
string title = "Sistema de Reinscripción para Coordinaciones"
string menuname = "m_reinscripcion"
...
global w_reinscripcion w_reinscripcion
...
public subroutine wf_establece_integracion (boolean ab_es_coordinacion_integ_ru)
							...
							if band_bloq	=	0	then
								if revisa_hora_insc(cta) = 0 then //Función que revisa si el alumno está en la hora correcta de inscripción.
									...
									if gi_nivel_usuario > 10 then	 
										...
										elseif nivel = 'L' Or nivel = 'T' then
...
cont = 0
intento:
if gi_nivel_usuario = 1 or gi_nivel_usuario = 3 then//Selecciona si es un super usuario o no
	niv = 0
...
//Linea peligrosa para coordinaciones 10 Abril 2007	
...
elseif niv = 0 then
	if nivel = 'L' Or nivel = 'T' then
		...
		messagebox("No pertenece a Licenciatura ni a Técnico Superior Universitario","El alumno con número de cuenta "+string(numcta)+"-"+obten_digito(numcta)+" no pertenece a licenciatura. ~rNo puede inscribirse",stopsign!)			
		return 1
	end if
elseif niv = 1 then
	if nivel = 'P' then
...
c= 0
	...
	intento:	 
				 ...
				 revisa_rows()				  
				  ...
				  if sqlca.sqlcode = 100 then//Este if se ejecuta si el alumno no ha llevado esa materia
						...
						if (	(	enlace = 'Y'	) or (isnull(enlace) and cont	=	1) or op3 = 1	)	then	//Si el	alumno no 
							...
							messagebox("Faltan prerrequisitos", "El alumno con número de cuenta "+string(cuenta)+" no ha cursado todos ~rlos prerrequisitos para la materia "+string(mat)+".",Exclamation!)
							return 1	
						end if
		...
		elseif respuesta < 0 then
				commit;
				messagebox("Error de Comunicación","Error con la consulta de prerrequisitos BD. Favor de intentar nuevamente", None!)
...
//Regresa 0 si existe o 1 en caso contrario
 ...
 if gpo = 'ZZ' then
	messagebox("Grupo de Intercambio","Los grupos ZZ (Intercambio Estudiantil) solo pueden registrarse en Servicios Escolares. ~nFavor de verificar.",Exclamation!)
	return 1	
 end if
...
cont = 0
intento:
if gi_nivel_usuario =1 or gi_nivel_usuario =3  then
...
//Los coordinadores no realizaran el bloqueo por invasor de horario, pero si enviarán un aviso...
...
public function integer gpo_lleno (long mat, string gpo);//  Antonio Pica Ruiz
//	Función que verifica si un grupo esta lleno o no 
// Regresa 0 si hay cupo y 1 en caso contrario
// ***IMPORTANTE***
// ***DEBIDO A QUE ESTA APLICACION ESTA PENSADA PARA COORDINADORES, SIEMPRE PERMITIRÁ EXCESO DE CUPO.***
 long /*año,*/cup,insc
...
//*** AQUI SE OBLIGA AL EXCESO DE CUPO POR SALON PARA COORDINADORES
...
if gi_nivel_usuario < 10 then 
	...
	SELECT pregunta_nip INTO :li_pregunta_nip FROM activacion_su;
	if sqlca.sqlcode <> 0 then
...
if gi_nivel_usuario >= 10 then 
	SELECT revisa_grupos_bloqueados INTO :i_revisa_grupos_bloqueados FROM activacion_su;
	if sqlca.sqlcode <> 0 then
		messagebox("Error de Comunicación","Error con la consulta de activacion_su BD. Favor de intentar nuevamente", None!)
		return 1
	end if
...
DO UNTIL intent = 11		
		intent++
//Se comenta este código 2009-05-12 Antonio Pica
...
cont = 0
  ...
  SELECT area_mat.cve_area  
    ...
    WHERE area_mat.cve_mat = :mat   
	 	AND area_mat.cve_area  IN (8061, 8062, 8063, 8064, 2200, 2201, 2202, 2203, 2204, 2205, 335,336); //Revisión del area de la materia a inscribir
	 
	revisa_rows()			//verifica que el error no se deba a exceso de renglones
	 
	 if sqlca.sqlcode = 0 then
		commit;
		if (area < 2201 or area > 2205) and (area < 8061 or area > 8064) and (area < 335 or area > 336) then
			return 0
...
if cuenta > 0 then
	if gi_nivel_usuario < 10 then 
		...
		SELECT revisa_teoria_lab INTO :li_revisa_teoria_lab FROM activacion_su;
		if sqlca.sqlcode <> 0 then
			messagebox("Error de Comunicación","Error con la consulta de activacion_su BD. Favor de intentar nuevamente", None!)
			return 1
		end if
...
public subroutine wf_establece_integracion (boolean ab_es_coordinacion_integ_ru);//wf_establece_integracion
//Habilita o deshabilita los menús correspondientes a las opciones de integración
//
//Recibe
//	ab_es_coordinacion_integ_ru		boolean
//
boolean lb_bandera = false

if ab_es_coordinacion_integ_ru then
	lb_bandera = true
else
	lb_bandera = false	
end if 
...
if isvalid(m_preinscripcion) then
	m_preinscripcion.m_integracin.visible = lb_bandera
	m_preinscripcion.m_integracin.m_temafundamental.visible = lb_bandera
	m_preinscripcion.m_integracin.m_temai.visible = lb_bandera
	m_preinscripcion.m_integracin.m_temaii.visible = lb_bandera
	m_preinscripcion.m_integracin.m_temaiii.visible = lb_bandera
	m_preinscripcion.m_integracin.m_temaiv.visible = lb_bandera
	
	m_preinscripcion.m_integracin.ToolbarItemVisible = lb_bandera
	m_preinscripcion.m_integracin.m_temafundamental.ToolbarItemVisible	 = lb_bandera
	m_preinscripcion.m_integracin.m_temai.ToolbarItemVisible	 = lb_bandera
	m_preinscripcion.m_integracin.m_temaii.ToolbarItemVisible	 = lb_bandera
	m_preinscripcion.m_integracin.m_temaiii.ToolbarItemVisible	 = lb_bandera
	m_preinscripcion.m_integracin.m_temaiv.ToolbarItemVisible	 = lb_bandera

	m_preinscripcion.m_integracin.enabled = lb_bandera

elseif isvalid (m_reinscripcion) then
	m_reinscripcion.m_integracin.visible = lb_bandera
	m_reinscripcion.m_integracin.m_temafundamental.visible = lb_bandera
	m_reinscripcion.m_integracin.m_temai.visible = lb_bandera
	m_reinscripcion.m_integracin.m_temaii.visible = lb_bandera
	m_reinscripcion.m_integracin.m_temaiii.visible = lb_bandera
	m_reinscripcion.m_integracin.m_temaiv.visible = lb_bandera
	
	m_reinscripcion.m_integracin.ToolbarItemVisible = lb_bandera
	m_reinscripcion.m_integracin.m_temafundamental.ToolbarItemVisible = lb_bandera
	m_reinscripcion.m_integracin.m_temai.ToolbarItemVisible = lb_bandera
	m_reinscripcion.m_integracin.m_temaii.ToolbarItemVisible = lb_bandera
	m_reinscripcion.m_integracin.m_temaiii.ToolbarItemVisible = lb_bandera
	m_reinscripcion.m_integracin.m_temaiv.ToolbarItemVisible = lb_bandera

	m_reinscripcion.m_integracin.enabled = lb_bandera
...
on w_reinscripcion.create
if this.MenuName = "m_reinscripcion" then this.MenuID = create m_reinscripcion
...
event open;//Antonio Pica Ruiz		10 Abril 2007
...
f_obten_titulo(w_insc_coord)
wf_establece_integracion(gb_es_coordinacion_integ_ru)
...
string text = "Verano  2009 01/06/2009"
boolean focusrectangle = false
end type
...
if rowcount()	>	0 then
	...
	if getitemnumber(getrow(),1) > 0 and getitemstring(getrow(),2) <> " " then
		...
		if revisa_per_insc_mat(long(uo_nombre.em_cuenta.text),cvmat,gpo) = 0 then //Revisa que la materia a dar de baja sea de este periodo (Altas o Insc)
			//if actualiza_alumnos_baja(cvmat,gpo) = 0 then	//Actualización de alumnos inscritos		
				//deleterow(getrow())
...
if rowcount() > 0  then
	...
	if cvmat <> 0 then
		if dw_grupos_disp.retrieve(cvmat,G_per,G_anio) = 0 then
			...
			dw_grupos_disp.visible = False
			timer(0)
			messagebox("No hay grupos disponibles","La materia " +string(cvmat)+" no tiene grupos disponibles.",Information!)
		else
			commit;
...
if columna = "mat_inscritas_cve_mat" then //Si la columna es clave de materia	
	if cvmat > 0 then
		if f_materia_valida (cvmat,gi_cve_coordinacion ) then  //Revisa que la materia pertenezca a la coordinacion del usuario
			...
			setpointer(Arrow!)
			borra_materia()
			messagebox("La materia no está permitida","La materia con clave "+string(cvmat)+" no pertenece a su coordinación. Favor de verificar.",Exclamation!)		
			return 1	
		end if
	end if
	setpointer(Arrow!)
...
string dataobject = "dw_grupos_libres"
boolean controlmenu = true
boolean maxbox = true
boolean vscrollbar = true
boolean resizable = true
...
string dataobject = "dw_materias_integracion"
boolean controlmenu = true
boolean maxbox = true
boolean vscrollbar = true
boolean resizable = true
