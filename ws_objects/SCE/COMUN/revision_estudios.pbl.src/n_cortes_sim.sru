...
Select  TOP 1 historico_re.calificacion
Into :ls_calificacion
From historico_re 
Where historico_re.cuenta = :al_cuenta 
And	historico_re.cve_carrera = :al_cve_carrera
And	historico_re.cve_plan = :al_cve_plan
And	historico_re.cve_mat in (	SELECT am.cve_mat FROM area_mat am 
									WHERE am.cve_area IN(SELECT pe2.cve_area_servicio_social 
																	FROM plan_estudios pe2
																	WHERE historico_re.cve_carrera = pe2.cve_carrera 
																	AND historico_re.cve_plan = pe2.cve_plan)
