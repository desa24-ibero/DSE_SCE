...
public function integer of_actualizacion_publicacion_actas (integer an_periodo, integer an_anio, integer an_cve_tipo_examen, datawindow adw_object, ref transaction atr_origen, ref transaction atr_destino)
...
public function integer of_update_datastore2 (u_datastore ads_origen, u_datastore ads_destino, transaction atr_origen, transaction atr_destino)
public function integer of_actualizacion_borra_inserta (long al_array_cuentas[], string as_dataobject, transaction atr_origen, transaction atr_destino)
end prototypes
...
FOR ll_indice_array = 1 TO ll_tamanio_array
	...
	if ll_rows_origen >= 1 then		
		...
		elseif ll_rows_destino> 0 then		
			ll_actualiza_datastore = of_update_datastore2(lds_origen, lds_destino, atr_origen, atr_destino)			
		elseif ll_rows_destino = -1 then		
			MessageBox("Error","Error en Destino",StopSign!)
			return ll_rows_destino
		end if
...
public function integer of_actualizacion_publicacion_actas (integer an_periodo, integer an_anio, integer an_cve_tipo_examen, datawindow adw_object, ref transaction atr_origen, ref transaction atr_destino);//Funcion
//of_actualizacion_objeto_replica
//		as_objeto		string
//		atr_origen		transaction
//		atr_destino		transaction
...
//
//	lds_objeto_replica = create u_datastore
//	lds_objeto_replica.dataobject = adw_object
//	lds_objeto_replica.SetTransObject(atr_origen)
//	
// 
//	ll_rows_objeto_replica  =  lds_objeto_replica.Retrieve(as_objeto)
...
public function integer of_update_datastore2 (u_datastore ads_origen, u_datastore ads_destino, transaction atr_origen, transaction atr_destino);//Funcion
//of_update_datastore
//		ads_origen	u_datastore
//		ads_destino	u_datastore
//		atr_origen	transaction
...
//			Obtiene el numero de columnas del datastore
			ll_num_columnas = UpperBound(la_valores[1])
			FOR li_columna_actual = 1 TO ll_num_columnas 
				ls_columna = ads_destino.DESCRIBE("#" + STRING(li_columna_actual) + ".Name") 
				ls_llave = ads_destino.DESCRIBE(ls_columna + ".Key")
				IF UPPER(ls_llave) = "NO" THEN 
					li_res_item_status = ads_destino.SetItemStatus(ll_row_actual, li_columna_actual,Primary!, DataModified!)				
				END IF 	
			NEXT
//			li_res_item_status = ads_destino.SetItemStatus(ll_row_actual, 0,Primary!, DataModified!)
...
public function integer of_actualizacion_borra_inserta (long al_array_cuentas[], string as_dataobject, transaction atr_origen, transaction atr_destino);//Funcion
//of_actualizacion_incremental
//		al_array_cuentas[]	long
//		as_dataobject		string
//		atr_origen			transaction
...
FOR ll_indice_array = 1 TO ll_tamanio_array
	...
	if ll_rows_origen >= 1 then		
		...
		// Se eliminan los registros del destino 
		FOR ll_pos_act = 1 TO ll_rows_destino 
			lds_destino.DELETEROW(1)  
		NEXT 
		lds_destino.UPDATE() 
		
		// Se pasan los registros del origen al destino 
		lds_origen.ROWSCOPY(1, lds_origen.ROWCOUNT(), PRIMARY!, lds_destino, 1, PRIMARY!) 
		IF lds_destino.UPDATE() < 0 THEN 
			ROLLBACK USING atr_destino; 
			MessageBox("Error","Error en Destino",StopSign!)
		END IF 
		COMMIT USING atr_destino; 
