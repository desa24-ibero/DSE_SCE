$PBExportHeader$uo_periodo_variable_chk.sru
forward
global type uo_periodo_variable_chk from userobject
end type
type dw_periodos from datawindow within uo_periodo_variable_chk
end type
end forward
...
global uo_periodo_variable_chk uo_periodo_variable_chk
...
FOR le_row = 1 TO iuo_periodo_servicios.ids_periodos.ROWCOUNT()
	le_row_ins = dw_periodos.insertrow(0)  
	// Se inserta el Id del periodo
	dw_periodos.SETITEM(le_row_ins, "periodo", iuo_periodo_servicios.ids_periodos.GETITEMNUMBER(le_row, "periodo"))	
NEXT
...
//ELSE
//	//dw_periodos.Modify("periodo.Width='" + STRING(400 * iuo_periodo_servicios.ids_periodos.ROWCOUNT()) + "'")	
...
//ELSE
//	li_Height = 80
//	li_Width = 400 * iuo_periodo_servicios.ids_periodos.ROWCOUNT() 
//END IF
//
...
FOR le_pos = 1 TO dw_periodos.ROWCOUNT() 
	...
	le_pos_array ++
	le_periodos[le_pos_array] = dw_periodos.GETITEMNUMBER(le_pos, "periodo")
...
on uo_periodo_variable_chk.create
this.dw_periodos=create dw_periodos
this.Control[]={this.dw_periodos}
end on

on uo_periodo_variable_chk.destroy
...
integer taborder = 10
string title = "none"
string dataobject = "d_selecciona_periodo_or"
