$PBExportHeader$n_transfiere_sybase_sql_2.sru
forward
global type n_transfiere_sybase_sql_2 from n_base
end type
end forward

global type n_transfiere_sybase_sql_2 from n_base
end type
global n_transfiere_sybase_sql_2 n_transfiere_sybase_sql_2
...
public function integer of_actualizacion_objeto_replica_tipo (string as_objeto, ref transaction atr_origen, ref transaction atr_destino)
public function integer of_actualizacion_delete_insert_tipo (string as_dataobject, transaction atr_origen, transaction atr_destino)
end prototypes
...
public function integer of_actualizacion_objeto_replica_tipo (string as_objeto, ref transaction atr_origen, ref transaction atr_destino);//Funcion
//of_actualizacion_objeto_replica
//		as_objeto		string
//		atr_origen		transaction
//		atr_destino		transaction
...
if li_replica_activa = 1 then
	...
	if ll_rows_objeto_replica >= 1 then	
		...
		FOR  ll_row_objeto_replica=1 to ll_rows_objeto_replica
			...
			ls_datawindow= lds_objeto_replica.GetItemString(ll_row_objeto_replica, "datawindow")	
			if len(ls_datawindow)>0 then
				ll_actualiza_datastore = this.of_actualizacion_delete_insert_tipo(ls_datawindow,atr_origen, atr_destino)
			end if				
		NEXT
...
public function integer of_actualizacion_delete_insert_tipo (string as_dataobject, transaction atr_origen, transaction atr_destino);//Funcion
//of_actualizacion_delete_insert
//		as_dataobject		string
//		atr_origen			transaction
//		atr_destino			transaction
...
lds_destino.SetTransObject(atr_destino)
	...
	ll_rows_origen  =  lds_origen.Retrieve(gs_tipo_periodo)
	//Si existen registros en el origen procede
	if ll_rows_origen >= 1 then		
		ll_rows_destino =  lds_destino.Retrieve(gs_tipo_periodo)
		//Sin importar si existen registros en el destino, borra e inserta
		if ll_rows_destino>= 0 then		
			ll_actualiza_datastore = of_delete_insert_datastore(lds_origen, lds_destino, atr_origen, atr_destino)
		elseif ll_rows_destino = -1 then		
...
on n_transfiere_sybase_sql_2.create
call super::create
end on

on n_transfiere_sybase_sql_2.destroy
call super::destroy
end on
