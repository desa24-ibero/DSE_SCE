$PBExportHeader$uo_genera_xml_cuenta.sru
forward
global type uo_genera_xml_cuenta from nonvisualobject
end type
end forward

global type uo_genera_xml_cuenta from nonvisualobject
end type
global uo_genera_xml_cuenta uo_genera_xml_cuenta

type variables

uds_datastore ids_archivo 
uds_datastore ids_archivo_dato 
uds_datastore ids_archivo_query 
//uds_datastore ids_archivo_query_sumuia
uds_datastore ids_archivo_seccion 
uds_datastore ids_archivo_xml 
uds_datastore ids_archivo_xml_seccion 
uds_datastore ids_archivo_xml_seccion2
uds_datastore ids_archivo_datos
...
public subroutine genera_xml (string as_cuenta)
public function integer of_calcula_titulo (long al_cuenta, long al_cve_carrera, long al_cve_plan)
public function string of_obten_ruta_archivo (string as_id_doc)
public function string of_obten_nivel_alumno (long al_cuenta)
end prototypes
...
ids_archivo.DATAOBJECT = "dw_archivo_por_cuenta"
...
public subroutine genera_xml (string as_cuenta);INTEGER le_pos_archivo
STRING ls_archivo
...
string ls_cuenta, ls_nivel, ls_nombre_nivel
string  ls_ruta_nivel

// Se llenan los Datastores con las estructuras de los XML
carga_estructuras() 

ls_nivel = of_obten_nivel_alumno(long(as_cuenta))
...
//	IF ls_archivo = "template" THEN 
	...
	ids_archivo_datos.SETFILTER("#1 = '" + as_cuenta + "'")	
	ids_archivo_datos.FILTER() 
	...
	DO WHILE ll_pos_datos <= ll_ttl_datos 
		...
		//Se asume que el la columna 1 siempre estará el número de cuenta convertido a string
		ls_cuenta = ids_archivo_datos.GetItemString(ll_pos_datos,1)		
		// Se abre archivo por cuenta
		IF ls_nivel = 'L' THEN
		...
		//Crea el directorio si no existe
		 ls_ruta_nivel=ls_ruta +ls_nombre_nivel +ls_fecha_ejec + "\"

		If not DirectoryExists ( ls_ruta_nivel ) Then
			CreateDirectory ( ls_ruta_nivel )
		end if

		
		
		ll_archivo = FileOpen(ls_ruta_nivel+ ls_cuenta+ "_" +  ls_fecha_ejec + ".xml", LineMode!, Write! , LockWrite! , Replace!, EncodingUTF8!) 
				...
				DO 
					...
					FOR ll_pos_xml_ciclo = 1 TO ids_archivo_xml_seccion.ROWCOUNT() 
						...
						ELSE
							...
							// Se revisa si la línea del archivo lleva algún dato 
...
//							END IF
			...
			//////////////////////////////////////////////////////////////////////////////////
			// Se cierra y se abre un nuevo archivo 
			//Cierra el archivo por el número de cuenta actual
...
//		ls_cuenta = ids_archivo_datos.GetItemString(ll_pos_datos,1)		
//		// Se abre archivo por cuenta
//		ll_archivo = FileOpen(ls_ruta +ls_cuenta+ "_" +  ls_fecha_ejec + ".xml", LineMode!, Write! , LockWrite! , Replace!, EncodingUTF8!) 
//		
//		ll_pos_xml = 1 
//			
//		/* XML */
...
public function integer of_calcula_titulo (long al_cuenta, long al_cve_carrera, long al_cve_plan);//of_calcula_titulo
// Recibe
//		al_cuenta			long
//		al_cve_carrera		long
//		al_cve_plan			long
//Ejecuta el stored procedure que complementa la información generada en la base de datos que no sale directamente del query

string ls_mensaje_sql
integer li_codigo_sql, li_resultado
datetime ldttm_fecha_inicio, ldttm_fecha_terminacion


DECLARE sp_calcula_titulo PROCEDURE FOR sp_inserta_titulacion_sep  
         @a_cuenta = :al_cuenta,   
         @a_cve_carrera = :al_cve_carrera,   
         @a_cve_plan = :al_cve_plan  USING gtr_sce;

EXECUTE sp_calcula_titulo;
			
FETCH sp_calcula_titulo INTO :ldttm_fecha_inicio, :ldttm_fecha_terminacion;

//--FETCH @fecha_inicio,  @fecha_terminacion 
//INTO :ldttm_fecha_inicio, ldttm_fecha_terminacion;

CLOSE sp_calcula_titulo;
...
if li_codigo_sql = -1 then
	...
	MessageBox("Error al invocar sp_calcula_titulo:"+string(al_cuenta)+"-"+string(al_cve_carrera)+"- "+ string(al_cve_plan),  ls_mensaje_sql)
...
public function string of_obten_ruta_archivo (string as_id_doc);//of_obten_ruta_archivo
// Recibe
//		as_id_doc			string
//Ejecuta el query que devuelve la ruta del documento para almacenar los XML's

string ls_mensaje_sql, ls_ruta
integer li_codigo_sql, li_resultado
datetime ldttm_fecha_inicio, ldttm_fecha_terminacion
...
if li_codigo_sql = -1 then
	rollback using gtr_sce;
	MessageBox("Error al consultar la tabla archivo: as_id_doc-"+as_id_doc,  ls_mensaje_sql)
	RETURN ""
elseif li_codigo_sql=100 then
	MessageBox("No existe er renglón de la tabla archivo: as_id_doc-"+as_id_doc,  ls_mensaje_sql)
	RETURN ""
end if
...
public function string of_obten_nivel_alumno (long al_cuenta);//of_obten_nivel_alumno
// Recibe
//		al_cuenta			long
//Obtiene el nivel del alumno al que se le va a generar el .XML

string ls_mensaje_sql, ls_ruta, ls_nivel
integer li_codigo_sql, li_resultado


SELECT  c.nivel
INTO :ls_nivel 
FROM titulacion_sep ts, titulacion t, carreras c
WHERE ts.cuenta = t.cuenta
AND ts.cve_carrera = t.cve_carrera
AND ts.cve_plan = t.cve_plan
AND t.cuenta = :al_cuenta
AND t.cve_carrera = c.cve_carrera
USING gtr_sce;
...
if li_codigo_sql = -1 then
	rollback using gtr_sce;
	MessageBox("Error al consultar la tabla titulacion: cuenta-"+string(al_cuenta),  ls_mensaje_sql)
	RETURN ""
elseif li_codigo_sql=100 then
	MessageBox("No existe el renglón de la tabla titulacion: cuenta-"+string(al_cuenta),  ls_mensaje_sql)
	RETURN ""
end if
...
on uo_genera_xml_cuenta.create
call super::create
TriggerEvent( this, "constructor" )
end on

on uo_genera_xml_cuenta.destroy
