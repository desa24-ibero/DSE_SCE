$PBExportHeader$uo_nombre_carrera_cats.sru
forward
global type uo_nombre_carrera_cats from UserObject
end type
type em_digito from editmask within uo_nombre_carrera_cats
end type
type st_2 from statictext within uo_nombre_carrera_cats
end type
type st_1 from statictext within uo_nombre_carrera_cats
end type
type r_3 from rectangle within uo_nombre_carrera_cats
end type
type cbx_nuevo from checkbox within uo_nombre_carrera_cats
end type
type vsb_dw_nombre_alumno from vscrollbar within uo_nombre_carrera_cats
end type
type cb_2 from commandbutton within uo_nombre_carrera_cats
end type
type cb_1 from commandbutton within uo_nombre_carrera_cats
end type
type em_cuenta from editmask within uo_nombre_carrera_cats
end type
type rb_actual from radiobutton within uo_nombre_carrera_cats
end type
type rb_anterior from radiobutton within uo_nombre_carrera_cats
end type
type gb_2 from groupbox within uo_nombre_carrera_cats
end type
type gb_1 from groupbox within uo_nombre_carrera_cats
end type
type dw_nombre_alumno from datawindow within uo_nombre_carrera_cats
end type
type cbx_navega from checkbox within uo_nombre_carrera_cats
end type
end forward
...
global uo_nombre_carrera_cats uo_nombre_carrera_cats

type variables
w_cat_carreras_ant ventana
menu menu
...
on uo_nombre_carrera_cats.create
...
on uo_nombre_carrera_cats.destroy
...
ventana.dw_catalogo. event carga(ventana.uo_nombre.em_cuenta.Text,&
...
ventana.dw_catalogo. event ue_actualiza()
...
/*Si el número de cuenta entrado fué un 0*/
	if em_cuenta.text='0' then
		/*Deshabilita la opción nuevo del menú y habilita la de actualiza y borraregistro*/
  		m_ancestro_menu_catalogos.m_registro.m_nuevo.disable ( )
		m_ancestro_menu_catalogos.m_registro.m_actualizar.enable ( )
		m_ancestro_menu_catalogos.m_registro.m_borraregistro.enable ( )
		m_ancestro_menu_catalogos.m_registro.m_primero.enable ( )
		m_ancestro_menu_catalogos.m_registro.m_ltimo.enable ( )
		m_ancestro_menu_catalogos.m_registro.m_siguiente.enable ( )
		m_ancestro_menu_catalogos.m_registro.m_anterior.enable ( )
...
////		rb_anterior.enabled=FALSE
		...
		if ventana.dw_catalogo. event carga('0','0','0')<>0 then
			...
			ventana.uo_nombre.em_cuenta.event busqueda_simple(string(&
				ventana.dw_catalogo.Object.hist_carreras_cuenta[ventana.dw_catalogo.GetRow()]))
		end if
	else
		/*Si el número de cuenta entrado no fue 0...*/
		/*Apaga la bandera de "navega"*/
		parent.cbx_navega.checked=FALSE
		m_ancestro_menu_catalogos.m_registro.m_primero.disable ( )
		m_ancestro_menu_catalogos.m_registro.m_ltimo.disable ( )
		m_ancestro_menu_catalogos.m_registro.m_siguiente.disable ( )
		m_ancestro_menu_catalogos.m_registro.m_anterior.disable ( )
		...
		elseif cbx_nuevo.checked = false then
				if gtr_sce.sqlcode = 100 then
					...
					if dw_nombre_alumno.retrieve(0) = 0 then
			  			dw_nombre_alumno.insertrow(0)
						  //ventana.dw_catalogo.enabled = false
						ventana.dw_catalogo.visible = false
//						ventana.vsb_dw_certificado.visible=false
					end if
					goto fin
				end if
				em_cuenta.text = string(cuenta)
...
//if cbx_nuevo.checked = false then
	...
	if gtr_sce.sqlerrtext = "Select returned more than one row" then
		...
		if dw_nombre_alumno.retrieve(cuenta) = 0 then
			dw_nombre_alumno.insertrow(0)
			ventana.dw_catalogo.InsertRow(0) /**/
			//ventana.dw_catalogo.enabled = false /**/
		else 
			em_cuenta.text=string(cuenta)
...
if parent.cbx_navega.checked then
	...
	if rowcount=0 then
		/*Si no se bajaron datos, deshabilita algunas opciones del menú,*/
  		m_ancestro_menu_catalogos.m_registro.m_nuevo.disable ( )
		m_ancestro_menu_catalogos.m_registro.m_actualizar.disable ( )
		m_ancestro_menu_catalogos.m_registro.m_borraregistro.disable ( )
		/*Inserta un renglón en el DataWindow dw_catalogo pero deshabilítalo,*/
//		ventana.dw_catalogo.InsertRow(0)
		//ventana.dw_catalogo.enabled = false
...
////		rb_anterior.enabled=FALSE
		...
		/*Habilita algunas opciones del menú,*/
  		m_ancestro_menu_catalogos.m_registro.m_nuevo.enable ( )
		m_ancestro_menu_catalogos.m_registro.m_actualizar.enable ( )
		m_ancestro_menu_catalogos.m_registro.m_borraregistro.enable ( )
