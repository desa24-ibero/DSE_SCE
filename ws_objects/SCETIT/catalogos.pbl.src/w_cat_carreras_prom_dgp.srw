$PBExportHeader$w_cat_carreras_prom_dgp.srw
forward
global type w_cat_carreras_prom_dgp from w_catalogo_ancestro
end type
end forward
...
integer width = 4896
integer height = 1784
string title = "Catalogo de Promedios Mención y DGP Carreras"
string menuname = "m_cat_carreras_ant_nvo"
boolean resizable = false
long backcolor = 16777215
end type
global w_cat_carreras_prom_dgp w_cat_carreras_prom_dgp

on w_cat_carreras_prom_dgp.create
call super::create
if IsValid(this.MenuID) then destroy(this.MenuID)
if this.MenuName = "m_cat_carreras_ant_nvo" then this.MenuID = create m_cat_carreras_ant_nvo
end on

on w_cat_carreras_prom_dgp.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
end on

event open;call super::open;//m_cat_carreras_ant_nvo.m_registro.m_cargaregistro.enabled=false
//
...
type dw_catalogo from w_catalogo_ancestro`dw_catalogo within w_cat_carreras_prom_dgp
...
integer width = 4800
integer height = 1556
string dataobject = "dw_cat_carrera_prom_menc_dgp"
...
event type integer dw_catalogo::carga(string as_cuenta, string as_cve_carrera, string as_cve_plan);
this.Retrieve()

return 0
end event

event dw_catalogo::itemchanged;call super::itemchanged;//DataWindowChild cve_plan, cve_subsistema
//DataWindowChild cve_plan_actual, cve_subsistema_actual
//string ls_filtro, ls_filtro_1, ls_filtro_2, ls_carrera, ls_columna, ls_plan
//string ls_filtro_actual, ls_filtro_1_actual, ls_filtro_2_actual, ls_carrera_actual, ls_columna_actual, ls_plan_actual
//integer rtncode
