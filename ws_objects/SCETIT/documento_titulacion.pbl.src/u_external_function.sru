...
public function string uf_get_codigo (long an_titulo, integer an_acta, integer an_libro, integer an_hoja, date af_examen, date af_exped, string a_nombres, string a_a_pat, string a_a_mat, integer a_id_plantel, integer a_id_univ, integer a_id_titulo, integer a_id_carrera)
end prototypes
...
if RegistryGet ( sRegKey + "\Code Examples", "ExampleDir", RegString!, ls_ExampleDir ) <> 1 then
	if RegistryGet ( sPBKey, "", RegString!, ls_PBDir ) = 1 Then
		//  Parse the exe name from the path to get the PB Directory
		li_Pos = Pos ( Lower ( ls_PBDir ), Lower ( sPBEXE ) )
		if li_Pos > 0 then ls_PBDir = Left ( ls_PBDir, li_Pos - 1 ) 
		If Left ( ls_PBDir, 1 ) = "\" Then
			ls_ExampleDir = ls_PBDir + "Code Examples\Example App"
		Else
			ls_ExampleDir = ls_PBDir + "\Code Examples\Example App"
		End If
	end if
...
if not FileExists ( ls_exampledir + "\example.bmp" ) then
...
public function string uf_get_codigo (long an_titulo, integer an_acta, integer an_libro, integer an_hoja, date af_examen, date af_exped, string a_nombres, string a_a_pat, string a_a_mat, integer a_id_plantel, integer a_id_univ, integer a_id_titulo, integer a_id_carrera);////uf_get_codigo(
//Devuelve String
//an_titulo As Long, 
//an_acta As Double, 
//an_libro As Double, 
...
//a_a_pat As String, 
//a_a_mat As String, 
//a_id_plantel As Double, 
//a_id_univ As Double, 
//a_id_titulo As Double, 
//a_id_carrera As Double) 
