$PBExportHeader$w_doc_ole_titulacion_tsu.srw
forward
global type w_doc_ole_titulacion_tsu from w_main
end type
type uo_datos_tsu_i from uo_datos_tsu within w_doc_ole_titulacion_tsu
end type
type cb_seleccion from commandbutton within w_doc_ole_titulacion_tsu
end type
type st_3 from statictext within w_doc_ole_titulacion_tsu
end type
type uo_tipo_documento from uo_clase_documento within w_doc_ole_titulacion_tsu
end type
type ole_documento from u_oc_impresion_word within w_doc_ole_titulacion_tsu
end type
type dw_documento_titulacion from u_dw within w_doc_ole_titulacion_tsu
end type
end forward
...
integer width = 3305
integer height = 1676
string title = "Titulación Individual TSU"
string menuname = "m_genera_documento_alumno"
uo_datos_tsu_i uo_datos_tsu_i
...
global w_doc_ole_titulacion_tsu w_doc_ole_titulacion_tsu
...
type variables
uo_atencion_tsu iuo_atencion_tsu
end variables
...
this.Control[iCurrent+1]=this.uo_datos_tsu_i
...
on w_doc_ole_titulacion_tsu.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.uo_datos_tsu_i)
...
ll_rows= dw_documento_titulacion.Retrieve(22)

x=1
y=1

iuo_atencion_tsu = CREATE uo_atencion_tsu
end event

event close;call super::close;if isvalid(iuo_atencion_tsu) THEN
	DESTROY iuo_atencion_tsu
end if
end event
...
on uo_datos_tsu_i.destroy
call uo_datos_tsu::destroy
end on
...
string binarykey = "w_doc_ole_titulacion_tsu.win"
end type
...
ll_cuenta = uo_datos_tsu_i.of_obten_cuenta()
ll_cve_carrera = uo_datos_tsu_i.of_obten_cve_carrera()
ll_cve_plan= uo_datos_tsu_i.of_obten_cve_plan(ll_cuenta)

lb_procede_tramite = uo_datos_tsu_i.of_procede_tramite()
lb_alumno_susceptible =iuo_atencion_tsu.of_alumno_susceptible(ll_cuenta, ll_cve_carrera, ll_cve_plan)
...
08w_doc_ole_titulacion_tsu.bin 
...
18w_doc_ole_titulacion_tsu.bin 
