$PBExportHeader$uo_nombre_carrera_ant_estprof_tsu.sru
forward
global type uo_nombre_carrera_ant_estprof_tsu from userobject
end type
type em_digito from editmask within uo_nombre_carrera_ant_estprof_tsu
end type
type st_2 from statictext within uo_nombre_carrera_ant_estprof_tsu
end type
type st_1 from statictext within uo_nombre_carrera_ant_estprof_tsu
end type
type r_3 from rectangle within uo_nombre_carrera_ant_estprof_tsu
end type
type cbx_nuevo from checkbox within uo_nombre_carrera_ant_estprof_tsu
end type
type vsb_dw_nombre_alumno from vscrollbar within uo_nombre_carrera_ant_estprof_tsu
end type
type cb_2 from commandbutton within uo_nombre_carrera_ant_estprof_tsu
end type
type cb_1 from commandbutton within uo_nombre_carrera_ant_estprof_tsu
end type
type em_cuenta from editmask within uo_nombre_carrera_ant_estprof_tsu
end type
type rb_actual from radiobutton within uo_nombre_carrera_ant_estprof_tsu
end type
type rb_anterior from radiobutton within uo_nombre_carrera_ant_estprof_tsu
end type
type gb_2 from groupbox within uo_nombre_carrera_ant_estprof_tsu
end type
type gb_1 from groupbox within uo_nombre_carrera_ant_estprof_tsu
end type
type dw_nombre_alumno from datawindow within uo_nombre_carrera_ant_estprof_tsu
end type
type cbx_navega from checkbox within uo_nombre_carrera_ant_estprof_tsu
end type
end forward
...
global uo_nombre_carrera_ant_estprof_tsu uo_nombre_carrera_ant_estprof_tsu
...
on uo_nombre_carrera_ant_estprof_tsu.create
...
on uo_nombre_carrera_ant_estprof_tsu.destroy
...
w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura. event carga(long(w_antecedentesestadiaprofesional_tsu.uo_nombrealumno.em_cuenta.Text),&
...
w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura. event actualiza()
...
/*Si el número de cuenta entrado fué un 0*/
	if em_cuenta.text='0' then
		...
		if w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura. event carga(0,0,0)<>0 then
			...
			w_antecedentesestadiaprofesional_tsu.uo_nombrealumno.em_cuenta.event busqueda_simple(string(&
				w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura.Object.cuenta[w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura.GetRow()]))
		end if
	else
		...
		elseif cbx_nuevo.checked = false then
				if gtr_sce.sqlcode = 100 then
					...
					if dw_nombre_alumno.retrieve(0) = 0 then
			  			dw_nombre_alumno.insertrow(0)
						  //w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura.enabled = false
						w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura.visible = false
//						w_antecedentesestadiaprofesional_tsu.vsb_dw_1=false
					end if
					goto fin
				end if
				em_cuenta.text = string(cuenta)
				if em_digito.text = "" then
					...
					if digito = dig then	
					  ...
					  //w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura.enabled = false
   					w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura.visible = false
//						w_antecedentesestadiaprofesional_tsu.vsb_dw_certificado.visible=false
...
//if cbx_nuevo.checked = false then
	...
	if gtr_sce.sqlerrtext = "Select returned more than one row" then
		...
		if dw_nombre_alumno.retrieve(cuenta) = 0 then
			dw_nombre_alumno.insertrow(0)
			w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura.InsertRow(0) /**/
			//w_antecedentesestadiaprofesional_tsu.dw_servicio_social_captura.enabled = false /**/
		else 
			em_cuenta.text=string(cuenta)
