$PBExportHeader$w_titulacion_eti2x.srw
$PBExportComments$Esta ventana ofrece etiquetas de los alumnos titulados con Reconocimiento
forward
global type w_titulacion_eti2x from w_master_main_rep
end type
type tab_1 from tab within w_titulacion_eti2x
end type
...
integer width = 3776
integer height = 2656
string title = "Impresion de Etiquetas de los Alumnos con Reconocimiento"
string menuname = "m_titulacion"
boolean resizable = true
tab_1 tab_1
end type
global w_titulacion_eti2x w_titulacion_eti2x

type variables
int carrerax
end variables

on w_titulacion_eti2x.create
int iCurrent
call super::create
...
event clicked;/*Se validan las fechas, y si son validas se mandan al datawindow */
if isdate(em_1.text) and isdate(em_2.text) then
	if date(em_2.text)>=date(em_1.text) then
			if (carrerax = 999) then
				carrerax=0
			end if	
			dw_1.retrieve(date(em_1.text),date(em_2.text),carrerax)
...
integer height = 1380
integer taborder = 5
string dataobject = "dw_etiqueta_tit2_nvo"
...
integer height = 1436
integer taborder = 4
string dataobject = "dw_etiqueta_tit2x_nvo"
boolean hscrollbar = true
boolean vscrollbar = true
end type
