$PBExportHeader$w_titulacion_repo9x.srw
$PBExportComments$Esta ventana ofrece el reporte de Titulos enviados a Profesiones
forward
global type w_titulacion_repo9x from w_master_main_rep
end type
type tab_1 from tab within w_titulacion_repo9x
end type
...
integer width = 4023
integer height = 2728
string title = "Reporte de los Titulos Enviados a Profesiones"
string menuname = "m_titulacion2"
boolean resizable = true
tab_1 tab_1
end type
global w_titulacion_repo9x w_titulacion_repo9x

type variables
int carrerax
end variables

on w_titulacion_repo9x.create
int iCurrent
call super::create
...
on w_titulacion_repo9x.destroy
call super::destroy
...
type st_sistema from w_master_main_rep`st_sistema within w_titulacion_repo9x
end type

type p_ibero from w_master_main_rep`p_ibero within w_titulacion_repo9x
end type
...
if (rb_3.checked=TRUE) then
   dw_1.setsort("profesiones_fecha_envio_prof A")
	dw_1.sort()
	if (cbx_2.checked=TRUE) then
      dw_1.setsort("carreras_carrera A, profesiones_fecha_envio_prof A")
		dw_1.sort()
	end if	
end if
...
integer height = 364
integer taborder = 5
string dataobject = "dw_tit_cuenta9"
...
integer height = 1292
integer taborder = 5
string dataobject = "dw_titulacion_rep9"
boolean resizable = true
end type
...
if rowcount>=1 then
   ...
   /*Se verifica que cumplan con los parametros de diseño */
	select tiempo
	into :Tiempo
   from param_titulacion
	where param_titulacion.nombre_tramite like "%rofesion%" using gtr_sce;

	if gtr_sce.SQLCode = 100 then
		/* Not found*/
		messagebox("Aviso"," No se pudo encontrar el parámetro: Profesiones ~r~ "+&
		                   "Por lo que se mostraran los registros requeridos ~r~ "+&
								 " SIN tomar en cuenta el parámetro especificado",Exclamation!)
   	/*Si es así, haz visible el VerticalScrollBar*/
	   ...
	   st_1.text='Total : '+Cont      
		...
		if gtr_sce.SQLCode = 0 then
		   /* Found*/
			/* Se eliminan los renglones que aun esten en rango*/
		   DO UNTIL Contador = rowcount()
		     Contador ++
		     fecha_x=date(this.object.profesiones_fecha_envio_prof[Contador])
           fecha_y=relativedate(fecha_x,Tiempo)
	        if fecha_y >= today() then
					 deleterow(Contador)
					 Contador=0
		     end if
         LOOP
...
integer height = 204
integer taborder = 4
string dataobject = "dw_tit_cuenta9x"
boolean livescroll = true
end type
...
if (rb_3x.checked=TRUE) then
   dw_1x.setsort("profesiones_fecha_envio_prof A")
	dw_1x.sort()
	if (cbx_2x.checked=TRUE) then
      dw_1x.setsort("carreras_carrera A, profesiones_fecha_envio_prof A")
		dw_1x.sort()
	end if	
end if
...
integer height = 1292
integer taborder = 4
string dataobject = "dw_titulacion_rep9x"
boolean resizable = true
end type
