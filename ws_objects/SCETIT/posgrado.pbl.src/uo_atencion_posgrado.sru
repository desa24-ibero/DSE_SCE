$PBExportHeader$uo_atencion_posgrado.sru
forward
global type uo_atencion_posgrado from nonvisualobject
end type
end forward

global type uo_atencion_posgrado from nonvisualobject
end type
global uo_atencion_posgrado uo_atencion_posgrado
...
public function integer of_baja_laboratorio (long al_cuenta)
public function integer of_baja_biblioteca (long al_cuenta)
public function integer of_baja_disciplina (long al_cuenta)
public function integer of_adeuda_finanzas (long al_cuenta)
public function boolean of_alumno_susceptible_vent (long al_cuenta, long al_cve_carrera, long al_cve_plan)
public function boolean of_alumno_susceptible_tit (long al_cuenta, long al_cve_carrera, long al_cve_plan)
public function integer of_inserta_hoja_libro_carrera (long al_cuenta, long al_cve_carrera, long al_cve_plan)
public function integer of_inserta_hoja_libro_posg_rg (long al_cuenta, long al_cve_carrera, long al_cve_plan)
public function integer of_actualiza_datos_tramites ()
public function integer of_existe_hoja_libro_posgrado (long al_cuenta, long al_cve_carrera, long al_cve_plan)
public function integer of_existe_hoja_libro_posg_rg (long al_cuenta, long al_cve_carrera, long al_cve_plan)
public function integer of_inserta_hoja_libro_posgrado (long al_cuenta, long al_cve_carrera, long al_cve_plan)
end prototypes
...
public function integer of_baja_laboratorio (long al_cuenta);//of_baja_laboratorio
//Revisa si el alumno esta dado de baja por adeudo de laboratorio
//Recibe
//al_cuenta			long
...
public function integer of_baja_biblioteca (long al_cuenta);//of_baja_biblioteca
//Revisa si el alumno esta dado de baja por biblioteca
//Recibe
//al_cuenta			long
...
INTO :li_bandera
FROM dbo.banderas b
WHERE b.cve_flag_biblioteca in (3)
AND	b.cuenta = :al_cuenta
USING gtr_sce;
...
public function integer of_baja_disciplina (long al_cuenta);//of_baja_disciplina
//Revisa si el alumno esta dado de baja por disciplina
//Recibe
//al_cuenta			long
...
public function boolean of_alumno_susceptible_vent (long al_cuenta, long al_cve_carrera, long al_cve_plan);// of_alumno_susceptible_vent
//Recibe 	al_cuenta	long
//			al_cve_carrera long
//			al_cve_plan
//Revisión para otorgar el trámite en ventanilla


integer li_adeuda_finanzas, li_baja_biblioteca, li_baja_laboratorio, li_baja_disciplina
integer li_docs_nacim, li_docs_estudios, li_docs_rev_est
integer li_sql_code=0
string ls_mensaje
long ll_cuenta
boolean lb_titulado_anterior

li_adeuda_finanzas = of_adeuda_finanzas(al_cuenta)
li_baja_biblioteca = of_baja_biblioteca(al_cuenta)
li_baja_laboratorio = of_baja_laboratorio(al_cuenta)
li_baja_disciplina = of_baja_disciplina(al_cuenta)
lb_titulado_anterior = of_titulado_anterior(al_cuenta, al_cve_carrera, al_cve_plan)
...
public function boolean of_alumno_susceptible_tit (long al_cuenta, long al_cve_carrera, long al_cve_plan);// of_alumno_susceptible_tit
//Recibe 	al_cuenta	long
//			al_cve_carrera long
//			al_cve_plan
//Revisión para otorgar el trámite en titulación
...
public function integer of_inserta_hoja_libro_carrera (long al_cuenta, long al_cve_carrera, long al_cve_plan);//Función que inserta un egresado en el libro de posgrado
//
...
public function integer of_inserta_hoja_libro_posg_rg (long al_cuenta, long al_cve_carrera, long al_cve_plan);//Función que inserta un egresado en el libro de posgrado
//
//of_inserta_hoja_libro_posg_rg
//
//Parámetros:		al_cuenta	
//						al_cve_carrera
//						al_cve_plan
...
CHOOSE CASE ls_cve_grado
	CASE "M"
		ls_cve_grupo_grado = "EM"
	CASE "E"
		ls_cve_grupo_grado = "EM"
	CASE "D"
		ls_cve_grupo_grado = "D"
	CASE ELSE
		ls_cve_grupo_grado = ""
END CHOOSE
...
if li_codigo_sql = -1 then
	ls_mensaje_aux="Error al consultar en hoja_libro_posgrado_rg : "
	MessageBox(ls_mensaje_aux+string(ll_cuenta)+":"+string(ll_cve_carrera),  ls_mensaje_sql, StopSign!)
	return -1
end if
...
SELECT max(folio)
INTO	:ll_max_folio
FROM	libro_posgrado_rg
WHERE cve_grupo_grado = :ls_cve_grupo_grado
AND   incorporado_sep = :ll_incorporado_sep
AND   folio < 250
USING gtr_sce;
...
if lb_nueva_insercion then
	...
	ls_mensaje_aux="Error al insertar en libro_posgrado_rg en el libro: "

	INSERT INTO	libro_posgrado_rg
	( cve_grupo_grado,	   incorporado_sep,	   libro,  	  folio )
	VALUES
	(:ls_cve_grupo_grado,  :ll_incorporado_sep,  :ll_libro, :ll_folio_siguiente)
	USING gtr_sce;
else

	ls_mensaje_aux="Error al actualizar en libro_posgrado_rg en el libro: "

	UPDATE libro_posgrado_rg
	SET    folio = :ll_folio_siguiente
	FROM   libro_posgrado_rg
	WHERE  cve_grupo_grado = :ls_cve_grupo_grado
   AND   incorporado_sep = :ll_incorporado_sep
...
if li_codigo_sql = -1 then
	rollback using gtr_sce;
	MessageBox(ls_mensaje_aux+ls_cve_grupo_grado+"/"+string(ll_incorporado_sep)+"/"+string(ll_libro)+"/"+string(ll_folio_siguiente),  ls_mensaje_sql, StopSign!)
	return 	li_codigo_sql
...
if li_existente=0 then
	...
	INSERT INTO hoja_libro_posgrado_rg
		(cve_grupo_grado,      cve_grado,     incorporado_sep,	 cuenta,      cve_carrera,	    cve_plan,     libro,     folio) 		
	VALUES
		(:ls_cve_grupo_grado, :ls_cve_grado, :ll_incorporado_sep, :ll_cuenta, :ll_cve_carrera, :ll_cve_plan, :ll_libro, :ll_folio_siguiente)
	USING gtr_sce;
...
public function integer of_existe_hoja_libro_posgrado (long al_cuenta, long al_cve_carrera, long al_cve_plan);//Función que inserta un egresado en el libro de carreras
//
//of_existe_hoja_libro_posgrado
//
//Parámetros:		al_cuenta	
//						al_cve_carrera
//						al_cve_plan
...
public function integer of_existe_hoja_libro_posg_rg (long al_cuenta, long al_cve_carrera, long al_cve_plan);//Función que inserta un egresado en el libro de carreras
//
//of_existe_hoja_libro_posg_rg
//
//Parámetros:		al_cuenta	
//						al_cve_carrera
//						al_cve_plan
...
if li_codigo_sql = -1 then
	MessageBox("Error al contar alumnos en libro RG:"+string(ll_cve_carrera),  ls_mensaje_sql)
	return -1
end if
...
elseif ll_num_veces = 1 then
	return 1
elseif ll_num_veces > 1 then
	MessageBox("Error al contar alumnos en libro RG:"+string(ll_cve_carrera)+"~n *>1*",  ls_mensaje_sql)
	return -1	
end if
...
public function integer of_inserta_hoja_libro_posgrado (long al_cuenta, long al_cve_carrera, long al_cve_plan);//Función que inserta un egresado en el libro de posgrado
//
//of_inserta_hoja_libro_posgrado
//
//Parámetros:		al_cuenta	
//						al_cve_carrera
//						al_cve_plan
...
if li_codigo_sql = -1 then
	ls_mensaje_aux="Error al consultar en hoja_libro_posgrado : "
	MessageBox(ls_mensaje_aux+string(ll_cuenta)+":"+string(ll_cve_carrera),  ls_mensaje_sql, StopSign!)
	return -1
end if
...
if lb_nueva_insercion then
	...
	ls_mensaje_aux="Error al insertar en libro_posgrado en el libro: "

	INSERT INTO	libro_posgrado
	( cve_grupo_grado,	   incorporado_sep,	   libro,  	  folio )
	VALUES
	(:ls_cve_grupo_grado,  :ll_incorporado_sep,  :ll_libro, :ll_folio_siguiente)
	USING gtr_sce;
	...
	ls_mensaje_aux="Error al actualizar en libro_posgrado en el libro: "

	UPDATE libro_posgrado
	SET    folio = :ll_folio_siguiente
	FROM   libro_posgrado
	WHERE  cve_grupo_grado = :ls_cve_grupo_grado
   AND   incorporado_sep = :ll_incorporado_sep
...
if li_existente=0 then
	...
	INSERT INTO hoja_libro_posgrado
		(cve_grupo_grado,      cve_grado,     incorporado_sep,	 cuenta,      cve_carrera,	    cve_plan,     libro,     folio) 		
	VALUES
		(:ls_cve_grupo_grado, :ls_cve_grado, :ll_incorporado_sep, :ll_cuenta, :ll_cve_carrera, :ll_cve_plan, :ll_libro, :ll_folio_siguiente)
	USING gtr_sce;
...
on uo_atencion_posgrado.create
call super::create
TriggerEvent( this, "constructor" )
end on

on uo_atencion_posgrado.destroy
