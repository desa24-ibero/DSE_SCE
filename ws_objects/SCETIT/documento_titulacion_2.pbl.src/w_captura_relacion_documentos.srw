$PBExportHeader$w_captura_relacion_documentos.srw
forward
global type w_captura_relacion_documentos from w_main
end type
type uo_1 from uo_datos_opc_cero_sin_rev2 within w_captura_relacion_documentos
end type
type dw_2 from u_dw_captura within w_captura_relacion_documentos
end type
type cb_1 from commandbutton within w_captura_relacion_documentos
end type
type st_1 from statictext within w_captura_relacion_documentos
end type
type em_cve_relacion from editmask within w_captura_relacion_documentos
end type
type dw_1 from u_dw_captura within w_captura_relacion_documentos
end type
end forward
...
integer width = 3122
integer height = 1852
string title = "Relacion de Documentos enviados a Diseño"
string menuname = "m_menu_relacion_documentos"
event metodo01 ( )
uo_1 uo_1
dw_2 dw_2
cb_1 cb_1
...
global w_captura_relacion_documentos w_captura_relacion_documentos
...
on w_captura_relacion_documentos.create
int iCurrent
call super::create
if this.MenuName = "m_menu_relacion_documentos" then this.MenuID = create m_menu_relacion_documentos
...
on w_captura_relacion_documentos.destroy
call super::destroy
...
IF NOT ISNUMBER(ls_cve_relacion) THEN
	MessageBox("Número Inválido", "Favor de escribir un número positivo", StopSign!)
ELSE
	ll_row_buscado = dw_1.Find("cve_relacion = "+ls_cve_relacion, 1, dw_1.RowCount())
	IF ll_row_buscado= 0 THEN
		MessageBox("Número No Encontrado", "No se encontró la relacion ["+ls_cve_relacion+"]", StopSign!)
	ELSE
...
integer height = 604
integer taborder = 10
string dataobject = "d_relacion_documentos_01"
end type
