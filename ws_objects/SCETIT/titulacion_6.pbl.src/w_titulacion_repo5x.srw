$PBExportHeader$w_titulacion_repo5x.srw
$PBExportComments$Esta ventana ofrece el reporte de enviados a diseño
forward
global type w_titulacion_repo5x from w_master_main_rep
end type
type tab_1 from tab within w_titulacion_repo5x
end type
...
integer width = 4160
integer height = 2540
string title = "Reporte de los Titulos enviados a Diseño"
string menuname = "m_titulacion2"
boolean resizable = true
tab_1 tab_1
end type
global w_titulacion_repo5x w_titulacion_repo5x

type variables
int carrerax
end variables

on w_titulacion_repo5x.create
int iCurrent
call super::create
...
on w_titulacion_repo5x.destroy
call super::destroy
...
type st_sistema from w_master_main_rep`st_sistema within w_titulacion_repo5x
end type

type p_ibero from w_master_main_rep`p_ibero within w_titulacion_repo5x
end type
...
if (rb_3.checked=TRUE) then
   dw_1.setsort("dise_o_titulacion_fecha_envio A")
	dw_1.sort()
	if (cbx_2.checked=TRUE) then
      dw_1.setsort("carreras_carrera A, dise_o_titulacion_fecha_envio A")
		dw_1.sort()
	end if	
end if
...
integer height = 364
integer taborder = 5
string dataobject = "dw_tit_cuenta5"
...
integer height = 1496
integer taborder = 5
string dataobject = "dw_titulacion_rep5"
end type
...
if rowcount>=1 then
   ...
   from param_titulacion
	where param_titulacion.nombre_tramite like "%iseño%" using gtr_sce;

	if gtr_sce.SQLCode = 100 then
		/* Not found*/
		messagebox("Aviso","  No se pudo encontrar el parámetro: Diseño ~r~ "+&
	   ...
	   st_1.text='Total : '+Cont      
		...
		if gtr_sce.SQLCode = 0 then
		   ...
		   DO UNTIL Contador = rowcount()
		     Contador ++
		     fecha_x=date(this.object.dise_o_titulacion_fecha_envio[Contador])
...
integer height = 204
integer taborder = 4
string dataobject = "dw_tit_cuenta5x"
boolean livescroll = true
end type
...
if (rb_3x.checked=TRUE) then
   dw_1x.setsort("dise_o_titulacion_fecha_envio A")
	dw_1x.sort()
	if (cbx_2x.checked=TRUE) then
      dw_1x.setsort("carreras_carrera A, dise_o_titulacion_fecha_envio A")
		dw_1x.sort()
	end if	
end if
...
integer height = 1292
integer taborder = 4
string dataobject = "dw_titulacion_rep5x"
end type
