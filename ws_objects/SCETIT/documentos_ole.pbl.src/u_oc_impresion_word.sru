...
public function long of_inserta_folio_documento_tit (long al_cuenta, long al_cve_carrera, long al_cve_plan, integer ai_cve_documento)
public function integer of_impresion_masiva (long al_cve_documento, boolean ab_en_sitio, boolean ab_permite_edicion, integer ai_marca_inicial)
end prototypes
...
//	this.object.application.document.PrintOut(Background, Append, Range,    OutputFileName, From, To, Item, Copies, Pages, PageType       , PrintToFile, Collate, FileName, ActivePrinterMacGX, ManualDuplexPrint, PrintZoomColumn, PrintZoomRow, PrintZoomPaperWidth, PrintZoomPaperHeight)
	ll_folio_insertado= this.of_inserta_folio_documento_tit(al_valor, al_valor, al_valor, al_cve_documento)
...
integer li_num_cols_unico, li_num_tipo_dato_unico, li_num_valores, li_indice_where, li_desplazamiento=0
string ls_marca_folio_documento = "folio_documento", ls_folio_insertado
long ll_folio_insertado
integer li_query_con_union
SetPointer(HourGlass!)
...
if not(li_num_cols_unico=li_num_tipo_dato_unico) then
   //AND li_num_tipo_dato_unico= li_num_valores) then
...
FOR li_indice_where= 1 TO li_num_valores 
	IF upper(ls_tipos_dato_valor_unico[li_indice_where - li_desplazamiento])="INT" or &
		upper(ls_tipos_dato_valor_unico[li_indice_where - li_desplazamiento])="INTEGER" or & 
		upper(ls_tipos_dato_valor_unico[li_indice_where - li_desplazamiento])="LONG" THEN
		IF as_valor_01[li_indice_where] <>"-1" THEN
			IF POS(upper(ls_query_definicion),"WHERE") > 0  THEN
				ls_condicion_where = ls_condicion_where +  " AND "+ls_columnas_valor_unico[li_indice_where - li_desplazamiento]+&
			...
			ELSE
				IF POS(upper(ls_condicion_where),"WHERE") > 0 THEN			
					ls_condicion_where = ls_condicion_where + " AND "+ls_columnas_valor_unico[li_indice_where - li_desplazamiento]+&
												" = "+as_valor_01[li_indice_where]
				ELSE
					ls_condicion_where = ls_condicion_where + " WHERE "+ls_columnas_valor_unico[li_indice_where - +li_desplazamiento]+&
												" = "+as_valor_01[li_indice_where]
				END IF
			
			END IF
		ELSE
			li_desplazamiento = li_desplazamiento +1			
		END IF			
			
	ELSEIF  upper(ls_tipos_dato_valor_unico[li_indice_where - li_desplazamiento])="STRING" or &
			upper(ls_tipos_dato_valor_unico[li_indice_where - li_desplazamiento])="STR" THEN
		IF as_valor_01[li_indice_where] <>"*" THEN			
...
//	this.object.application.document.PrintOut(Background, Append, Range,    OutputFileName, From, To, Item, Copies, Pages, PageType       , PrintToFile, Collate, FileName, ActivePrinterMacGX, ManualDuplexPrint, PrintZoomColumn, PrintZoomRow, PrintZoomPaperWidth, PrintZoomPaperHeight)
	...
	ll_folio_insertado= this.of_inserta_folio_documento_tit(long(as_valor_01[1]), long(as_valor_01[2]),long(as_valor_01[3]),al_cve_documento)
	if	ll_folio_insertado <> -1 then
		ls_folio_insertado = string(ll_folio_insertado)
		IF this.object.application.ActiveDocument.Bookmarks.Exists(ls_marca_folio_documento) THEN
			//Si existe la marca, sustituirá la marca por el contenido encontrado en el query
...
public function long of_inserta_folio_documento_tit (long al_cuenta, long al_cve_carrera, long al_cve_plan, integer ai_cve_documento);//Función que registra el documento generado
//
//of_inserta_folio_documento_tit
//
//Parámetros:		al_cuenta		
...
ll_cve_concepto= of_obten_cve_concepto(ai_cve_documento)
...
if lb_nueva_insercion then
	...
	INSERT INTO	dbo.folio_documento_tit
	(dbo.folio_documento_tit.folio, dbo.folio_documento_tit.cuenta, dbo.folio_documento_tit.cve_documento, dbo.folio_documento_tit.cve_carrera, dbo.folio_documento_tit.cve_plan)
	VALUES
	(:ll_folio_siguiente, :ll_cuenta, :li_cve_documento, :al_cve_carrera, :al_cve_plan)
	USING gtr_sce;
else
	...
	SET dbo.folio_documento_tit.folio = :ll_folio_siguiente,
	    dbo.folio_documento_tit.cuenta = :ll_cuenta,
		 dbo.folio_documento_tit.cve_documento = :li_cve_documento,
	    dbo.folio_documento_tit.cve_carrera = :al_cve_carrera,
		 dbo.folio_documento_tit.cve_plan = :al_cve_plan
	FROM dbo.folio_documento_tit
	USING gtr_sce;
end if
...
if li_codigo_sql = -1 then
	...
	IF ll_cve_concepto=0 THEN
		commit using gtr_sce;
	ELSEIF ll_cve_concepto>0 THEN
		ll_folio_mov_alumnos= of_valida_pago (al_cuenta, al_cve_carrera, al_cve_plan,ai_cve_documento)
		IF ll_folio_mov_alumnos>0 THEN
			ls_mensaje_aux="Error al actualizar en pago_tramite_titulacion en la constancia: "
			
			UPDATE pago_tramite_titulacion
			SET cve_concepto = :ll_cve_concepto,
				 folio_mov_alumnos = :ll_folio_mov_alumnos
			...
			ELSE
				commit using gtr_sce;				
			END IF				
		ELSEIF ll_folio_mov_alumnos<=0 THEN
			rollback using gtr_sce;
			MessageBox("No es posible imprimir Documento", "No se encontro un pago disponible")			
			return -1		
		END IF
	END IF
...
public function integer of_impresion_masiva (long al_cve_documento, boolean ab_en_sitio, boolean ab_permite_edicion, integer ai_marca_inicial);//of_impresion_masiva
//Recibe		al_cve_documento long
//				as_valor_01[]  string
//				ab_permite_edicion boolean
//				ab_en_sitio	boolean
...
blob lblob_ole
long ll_campos, ll_row, ll_rows, ll_tamanio_arreglo, ll_indice_arreglo
integer iDDEHandle, li_remote, li_ret_activate, li_cve_tipo_dato
string ls_filename, ls_valor_02, ls_valor_03
string ls_condicion_cuenta, ls_condicion_datastore,ls_ret_activate
datastore lds_datastore
string ls_presentacion, ls_error, ls_resultado_syntax, ls_marca_actual, ls_columna_actual
string ls_columna_valor_unico, ls_query_definicion, ls_columnas_marcas[], ls_query_datos
...
string ls_marca_folio_documento = "folio_documento", ls_folio_insertado, ls_valor_01[]
long ll_folio_insertado
integer li_confirmacion,li_indice_columnas
integer li_num_pausa=500
SetPointer(HourGlass!)
//expresión.PrintOut(Background, Append, Range, OutputFileName, From, To, Item, Copies, Pages, PageType, PrintToFile, Collate, FileName, ActivePrinterMacGX, ManualDuplexPrint, PrintZoomColumn, PrintZoomRow, PrintZoomPaperWidth, PrintZoomPaperHeight)
//Background   Variant opcional. True para que la macro continúe mientras Microsoft Word imprime el documento.
//Append   Variant opcional. True para adjuntar el documento seleccionado al nombre de archivo especificado por el argumento OutputFileName. False para sobrescribir el contenido de OutputFileName.
...
//expresión.Move(Unit/WdUnits, Count/numerico entero)
//Valores posibles de WdUnits: 
constant integer wdCharacter = 1
constant integer wdWord      = 2
constant integer wdSentence  = 3
constant integer wdParagraph = 4
constant integer wdSection   = 8
constant integer wdStory     = 6
constant integer wdCell      =12
constant integer wdColumn    = 9
constant integer wdRow       =10
constant integer wdTable     =15

//expresión.EndKey(Unit/WdUnits, Extend/WdMovementType)
//Valores posibles de WdUnits
//constant integer wdStory  = 6
//constant integer wdColumn = 9 
...
//Valores posibles de WdMovementType:
...
elseif ll_rows = -1 then
	...
	return -1
elseif ll_rows > 1 then
	li_confirmacion= MessageBox("Confirmación","¿Desea Generar los ["+string(ll_rows)+"] documentos de Word?",Question!, YesNo!)
...
//Recorre todos los registros	
	FOR ll_row = 1 to ll_rows
		...
		IF mod(ll_row, li_num_pausa)= 0 THEN
			MessageBox("Pausa","Favor de esperar que el spool de impresión esté vacío para continuar",Information!)	
		END IF		
	...
	//Recorre toda la lista de las marcas
		FOR ll_indice_arreglo= 1 to ll_tamanio_arreglo
			...
			IF this.object.application.ActiveDocument.Bookmarks.Exists(ls_marca_actual) THEN
				//Si existe la marca, sustituirá la marca por el contenido encontrado en el query
				this.object.application.Selection.Goto(TRUE,0,0,ls_marca_actual)
				this.object.application.Selection.SelectCell()
				this.object.application.Selection.Delete(wdCharacter,1)
				ls_columna_actual = lds_datastore.GetItemString(ll_row,ll_indice_arreglo + ai_marca_inicial)
				this.object.application.Selection.TypeText(ls_columna_actual)
			END IF
		NEXT	
...
//li_num_tipo_dato_unico = upperbound(ls_tipos_dato_valor_unico)
	...
	FOR li_indice_columnas = 1 to ai_marca_inicial
		ls_valor_01[li_indice_columnas]= lds_datastore.GetItemString(ll_row,li_indice_columnas)	
	NEXT

		ll_folio_insertado= this.of_inserta_folio_documento_tit(long(ls_valor_01[1]), long(ls_valor_01[2]),long(ls_valor_01[3]),al_cve_documento)

		if	ll_folio_insertado <> -1 then
			ls_folio_insertado = string(ll_folio_insertado)
//Evitar el foliado del documento			
//			IF this.object.application.ActiveDocument.Bookmarks.Exists(ls_marca_folio_documento) THEN
//				//Si existe la marca, sustituirá la marca por el contenido encontrado en el query
//				this.object.application.Selection.Goto(TRUE,0,0,ls_marca_folio_documento)
//				this.object.application.Selection.TypeText(ls_folio_insertado)
