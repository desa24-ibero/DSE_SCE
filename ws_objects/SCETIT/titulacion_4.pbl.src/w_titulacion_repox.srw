$PBExportHeader$w_titulacion_repox.srw
$PBExportComments$Esta ventana ofrece el reporte de alumnos titulados
forward
global type w_titulacion_repox from w_master_main_rep
end type
type tab_1 from tab within w_titulacion_repox
end type
...
type uo_nivel_3 from uo_nivel_2013 within tabpage_opcion
end type
...
integer width = 4069
integer height = 3056
string title = "Reportes de Alumnos Titulados"
string menuname = "m_titulacion2"
boolean resizable = true
tab_1 tab_1
end type
global w_titulacion_repox w_titulacion_repox
...
public function integer wf_obtiene_niveles ()
end prototypes
...
public function integer wf_obtiene_niveles ();//long li_numtot
//integer li_pos_ini , li_pos_fin, iIndex, iPosini, iPosfin
//string sAsterisco, sCadena, sResultado
//        
//sCadena = tab_1.tabpage_general.uo_1.of_obtiene_seleccion( )
//sAsterisco = '|'
//iIndex = 1
//iPosini  = 1
//	 
...
on w_titulacion_repox.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.tab_1)
end on

event open;gtr_sce.DBParm = "DBTextLimit=32000"

/*Cuando se abra la ventana w_certificados...*/
...
tab_1.tabpage_opcion.uo_nivel_3.of_carga_control(gtr_sce)
end event

type st_sistema from w_master_main_rep`st_sistema within w_titulacion_repox
end type

type p_ibero from w_master_main_rep`p_ibero within w_titulacion_repox
end type
...
integer height = 292
integer taborder = 5
string dataobject = "dw_tit_cuenta"
boolean resizable = true
...
If UpperBound(ls_array_aux[]) > 1 Then
	MessageBox(" Error ","No puede seleccionar más de un nivel",StopSign!)
	return -1
End If
...
integer height = 1636
integer taborder = 5
string dataobject = "dw_titulacion_rep"
...
integer height = 300
integer taborder = 4
string dataobject = "dw_tit_cuentax"
boolean livescroll = true
end type
...
li_result = uo_nivel_2.of_carga_arreglo_nivel( )
uo_nivel_2.of_obtiene_array(ls_array_aux[])
...
li_result = uo_nivel_2.of_carga_arreglo_nivel( )
ls_array_aux[] = uo_nivel_2.is_array_nivel
...
integer height = 1464
integer taborder = 4
string dataobject = "dw_titulacion_repx"
boolean vscrollbar = true
end type
...
integer height = 432
integer taborder = 6
string dataobject = "dw_tit_cuenta11"
boolean vscrollbar = true
boolean livescroll = true
end type
...
li_result = uo_nivel_3.of_carga_arreglo_nivel( )
uo_nivel_3.of_obtiene_array(ls_array_aux[])
...
if isdate(em_1w.text) and isdate(em_2w.text) then
	if date(em_2w.text)>=date(em_1w.text) then
			...
			dw_1w.retrieve(date(em_1w.text),date(em_2w.text),carrera2,opcion, ls_array_aux[])
			dw_3w.retrieve(date(em_1w.text),date(em_2w.text),carrera2,opcion, ls_array_aux[])
/* Se ordenan al terminar de traer los datos */
...
integer taborder = 5
boolean bringtotop = true
string dataobject = "dw_opc_titulacion"
boolean border = false
end type
...
integer height = 1596
integer taborder = 1
string dataobject = "dw_titulacion_rep11"
boolean vscrollbar = true
end type
