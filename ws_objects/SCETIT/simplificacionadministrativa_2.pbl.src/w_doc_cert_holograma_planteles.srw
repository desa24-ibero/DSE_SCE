$PBExportHeader$w_doc_cert_holograma_planteles.srw
forward
global type w_doc_cert_holograma_planteles from Window
end type
type st_plantel1 from statictext within w_doc_cert_holograma_planteles
end type
type st_plantel from statictext within w_doc_cert_holograma_planteles
end type
type st_1 from statictext within w_doc_cert_holograma_planteles
end type
type st_folio_acta from statictext within w_doc_cert_holograma_planteles
end type
type uo_1 from uo_orden_cobro within w_doc_cert_holograma_planteles
end type
type dw_1 from uo_dw_captura_holograma within w_doc_cert_holograma_planteles
end type
end forward
...
string Title="Captura de Hologramas / Certificación (PLANTELES)"
...
global w_doc_cert_holograma_planteles w_doc_cert_holograma_planteles

type variables
long il_folio_acta, il_ultimo_folio, il_numeros[]
integer ii_cve_doc_control_sep, ii_cve_plantel
...
FOR ll_row_actual = 1 to ll_num_rows
	ll_numero= dw_1.GetItemNumber(al_num_row, "numero")
	If (ll_row_actual <>  al_num_row)  then		
		ll_num_holograma= dw_1.GetItemNumber(ll_row_actual, "num_holograma")
...
on w_doc_cert_holograma_planteles.destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.st_plantel1)
...
int Height=1363
int TabOrder=10
string DataObject="d_doc_cert_holograma_planteles"
end type
...
ll_numero= this.GetItemNumber(ai_num_registro, "numero")
ll_num_holograma= this.GetItemNumber(ai_num_registro, "num_holograma")

if f_existe_holograma_repetido(il_numeros[], ll_num_holograma, true) & 
...
integer li_cve_doc_control_sep[], li_respuesta, li_cve_plantel, li_tamanio_lista, li_indice
...
if rowcount > 0 then
	...
	FOR ll_row_actual= 1 TO rowcount
		ll_numero= this.GetItemNumber(ll_row_actual,"numero")
		il_numeros[ll_row_actual]= ll_numero
	NEXT

	FOR ll_row_actual= 1 TO rowcount
		ll_folio_acta = this.GetItemNumber(ll_row_actual,"folio_acta")
		li_cve_plantel = this.GetItemNumber(ll_row_actual,"planteles_cve_plantel")
		ls_plantel = this.GetItemString(ll_row_actual,"planteles_plantel")
		li_cve_doc_control_sep[ll_row_actual]= this.GetItemNumber(ll_row_actual,"cve_doc_control_sep")
	...
	ii_cve_plantel = li_cve_plantel
	ii_cve_doc_control_sep = li_cve_doc_control_sep[1]
	
//	if lb_acta_nula then		
		
		il_ultimo_folio= f_obten_folio_acta(ii_cve_doc_control_sep,ii_cve_plantel)		
//	end if
...
/*Función que solo actualiza*/
	if IsNull(il_folio_acta) or il_folio_acta= 0 then
		il_folio_acta= f_incrementa_folio_acta(ii_cve_doc_control_sep, ii_cve_plantel,  il_ultimo_folio)
	end if
	st_folio_acta.text= string(il_folio_acta)
