$PBExportHeader$uo_nombre_alumno_plant.sru
forward
global type uo_nombre_alumno_plant from UserObject
end type
type em_digito from editmask within uo_nombre_alumno_plant
end type
type st_2 from statictext within uo_nombre_alumno_plant
end type
type st_1 from statictext within uo_nombre_alumno_plant
end type
type r_2 from rectangle within uo_nombre_alumno_plant
end type
type dw_nombre_alumno from datawindow within uo_nombre_alumno_plant
end type
type r_3 from rectangle within uo_nombre_alumno_plant
end type
type cbx_nuevo from checkbox within uo_nombre_alumno_plant
end type
type r_1 from rectangle within uo_nombre_alumno_plant
end type
type em_cuenta from editmask within uo_nombre_alumno_plant
end type
end forward
...
global uo_nombre_alumno_plant uo_nombre_alumno_plant
...
cbx_nuevo.checked = False
 ...
 SELECT min(control_sep_planteles.cuenta )
    INTO :ll_cuenta  
    FROM control_sep_planteles
	 USING gtr_sce;
...
SELECT min(control_sep_planteles.cuenta  )
  INTO :ll_cuenta  
  FROM control_sep_planteles  
 WHERE control_sep_planteles.cuenta > :ll_cuenta
 USING gtr_sce;
...
SELECT max(control_sep_planteles.cuenta )
  INTO :ll_cuenta  
  FROM control_sep_planteles  
 WHERE control_sep_planteles.cuenta < :ll_cuenta
 USING gtr_sce;
...
on uo_nombre_alumno_plant.create
...
on uo_nombre_alumno_plant.destroy
...
int Height=243
int TabOrder=30
string DataObject="dw_nombre_alumno_plant"
boolean Border=false
end type
	...
	CHOOSE CASE ls_columna
		CASE "nombre_completo"
		     ls_nombre ="%"+dw_nombre_alumno.gettext()+"%"
			  ls_apat="%--%"
			  ls_amat="%--%"
			  SELECT control_sep_planteles.cuenta  
	    		 INTO :ll_cuenta  
	    	    FROM control_sep_planteles  
	   		WHERE control_sep_planteles.nombre_completo like :ls_nombre
				USING gtr_sce;
	END CHOOSE	
	
	if gtr_sce.sqlerrtext = "Select returned more than one row" then
		origen = entrega_ventana()
		openwithparm(w_alumnosnombre_plant,origen)
		if w_alumnosnombre_plant.dw_nombre.retrieve(ls_nombre)  = 0 then
			messagebox("Aviso","No existe ningun "+mid(ls_nombre,2,len(ls_nombre)-2)+"  dado de alta.")
...
event getfocus;if isvalid(w_alumnosnombre_plant) then
	close(w_alumnosnombre_plant)
end if
end event
