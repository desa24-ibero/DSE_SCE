$PBExportHeader$w_reporte_cert_sep.srw
forward
global type w_reporte_cert_sep from Window
end type
type cb_guardar from commandbutton within w_reporte_cert_sep
end type
type dw_reporte_sep from datawindow within w_reporte_cert_sep
end type
type cb_imprimir from commandbutton within w_reporte_cert_sep
end type
type cb_nuevo from commandbutton within w_reporte_cert_sep
end type
type cb_eliminar from commandbutton within w_reporte_cert_sep
end type
type cb_agregar from commandbutton within w_reporte_cert_sep
end type
type lb_orden_cobro from uolb_datastorenum within w_reporte_cert_sep
end type
type lb_reporte_sep_con_orden from uolb_datastorenum within w_reporte_cert_sep
end type
type lb_reporte_sep_sin_orden from uolb_datastorenum within w_reporte_cert_sep
end type
end forward
...
string Title="Reporte escrito de las listas capturadas (Certificación)"
...
global w_reporte_cert_sep w_reporte_cert_sep

event open;X=1
Y=1
string ls_null
SetNull(ls_null)
lb_reporte_sep_sin_orden.llena("d_reporte_sep_cert",gtr_sce,3,0,'0')
lb_orden_cobro.llena("d_orden_cobro_cert",gtr_sce,1,1,ls_null)

end event

on w_reporte_cert_sep.create
...
int Height=35
int TabOrder=40
string DataObject="d_reporte_sep_cert"
...
if lb_reporte_sep_con_orden.SelectedIndex() > 0 then
	...
	lb_reporte_sep_sin_orden.llena("d_reporte_sep_cert",gtr_sce,3,0,'0')
	lb_reporte_sep_con_orden.llena("d_reporte_sep_cert",gtr_sce,3,0,lb_orden_cobro.Text(lb_orden_cobro.SelectedIndex()))
	lb_orden_cobro.llena("d_orden_cobro_cert",gtr_sce,1,1,ls_null)
else
...
event selectionchanged;call super::selectionchanged;lb_reporte_sep_con_orden.llena("d_reporte_sep_cert",gtr_sce,3,0,text(index))
end event
