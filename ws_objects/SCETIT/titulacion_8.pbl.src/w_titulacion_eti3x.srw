$PBExportHeader$w_titulacion_eti3x.srw
$PBExportComments$Esta ventana ofrece etiquetas de los alumnos titulados
forward
global type w_titulacion_eti3x from w_master_main_rep
end type
type tab_1 from tab within w_titulacion_eti3x
end type
type tabpage_general from userobject within tab_1
end type
type uo_1 from uo_nivel_2013 within tabpage_general
end type
type rb_examena from radiobutton within tabpage_general
end type
type rb_envio_a_profesiones from radiobutton within tabpage_general
end type
type gb_criterio_fecha from groupbox within tabpage_general
end type
...
type tabpage_general from userobject within tab_1
uo_1 uo_1
rb_examena rb_examena
rb_envio_a_profesiones rb_envio_a_profesiones
gb_criterio_fecha gb_criterio_fecha
hsb_1 hsb_1
rb_1 rb_1
rb_2 rb_2
rb_3 rb_3
...
integer width = 3808
integer height = 2648
string title = "Impresion de Etiquetas de los Alumnos Aprobados en su Examen Profesional"
string menuname = "m_titulacion"
boolean resizable = true
tab_1 tab_1
end type
global w_titulacion_eti3x w_titulacion_eti3x
...
forward prototypes
public subroutine wf_cambia_dw (string as_nom_datawindow)
end prototypes
...
if isnull(tab_1.tabpage_general.dw_1.DataObject) then
	MessageBox("Nombre de DataWindow Inválido","El DataWindow :"+as_nom_datawindow+" no existe")	
	return 
end if

tab_1.tabpage_general.dw_1.DataObject =  as_nom_datawindow

tab_1.tabpage_general.dw_1.SetTransObject(gtr_sce)

m_titulacion.dw = tab_1.tabpage_general.dw_1
...
tab_1.tabpage_general.uo_1.of_carga_control(gtr_sce) 

end event

type st_sistema from w_master_main_rep`st_sistema within w_titulacion_eti3x
end type

type p_ibero from w_master_main_rep`p_ibero within w_titulacion_eti3x
end type
...
if newindex = 2 then
	 m_titulacion.dw = tab_1.tabpage_individual.dw_1x
...
this.rb_examena=create rb_examena
this.rb_envio_a_profesiones=create rb_envio_a_profesiones
this.gb_criterio_fecha=create gb_criterio_fecha
...
this.rb_examena,&
this.rb_envio_a_profesiones,&
this.gb_criterio_fecha,&
this.hsb_1,&
this.rb_1,&
this.rb_2,&
this.rb_3,&
...
destroy(this.uo_1)
destroy(this.rb_examena)
destroy(this.rb_envio_a_profesiones)
destroy(this.gb_criterio_fecha)
destroy(this.hsb_1)
destroy(this.rb_1)
destroy(this.rb_2)
destroy(this.rb_3)
...
event clicked;wf_cambia_dw('dw_etiqueta_tit3_nvo_fechex')
is_criterio = "E"
end event
...
long textcolor = 33554432
long backcolor = 12639424
string text = "Envío Profesiones"
boolean checked = true
boolean lefttext = true
end type

event clicked;wf_cambia_dw('dw_etiqueta_tit3_nvo')
is_criterio = "P"
end event
...
event clicked;string ls_nivel, ls_array_aux[]
SetNull(ls_nivel)
integer li_result


li_result = tab_1.tabpage_general.uo_1.of_carga_arreglo_nivel( )
tab_1.tabpage_general.uo_1.of_obtiene_array(ls_array_aux[])

If UpperBound(ls_array_aux[]) <= 0 Then
	MessageBox(" Alerta del Sistema ","Debe seleccionar al menos un nivel",StopSign!)
	return -1
End If
...
if (rb_1.checked=TRUE) then
   dw_1.setsort("alumnos_apaterno A, alumnos_amaterno A, alumnos_nombre A, ccarrera A")
	dw_1.sort()
   if (cbx_2.checked=TRUE) then
      dw_1.setsort("ccarrera A, alumnos_apaterno A, alumnos_amaterno A, alumnos_nombre A")
		dw_1.sort()
	end if	
end if
...
if (rb_2.checked=TRUE) then
   dw_1.setsort("titulacion_cuenta A")
	dw_1.sort()
	if (cbx_2.checked=TRUE) then
      dw_1.setsort("ccarrera A, titulacion_cuenta A")
		dw_1.sort()
	end if	
...
if (rb_3.checked=TRUE) then
	if is_criterio = "P" then
  	   dw_1.setsort("cfecha_envio_prof A")
	elseif is_criterio = "E" then
  	   dw_1.setsort("cfecha_examen A")		
	end if
   dw_1.sort()
	if (cbx_2.checked=TRUE) then
		if is_criterio = "P" then
   	   dw_1.setsort("ccarrera A, cfecha_envio_prof A")
		elseif is_criterio = "E" then
   	   dw_1.setsort("ccarrera A, cfecha_examen A")		
		end if
		dw_1.sort()
	end if	
end if
...
integer height = 1084
integer taborder = 5
string dataobject = "dw_etiqueta_tit3_nvo"
...
event lineleft;dw_1x.ScrollPriorPage ( ) 
end event

event lineright;dw_1x.ScrollNextPage ( ) 
...
if (rb_1x.checked=TRUE) then
   dw_1x.setsort("alumnos_apaterno A, alumnos_amaterno A, alumnos_nombre A, ccarrera A")
	dw_1x.sort()
   if (cbx_2x.checked=TRUE) then
      dw_1x.setsort("ccarrera A, alumnos_apaterno A, alumnos_amaterno A, alumnos_nombre A")
		dw_1x.sort()
	end if	
end if
...
if (rb_2x.checked=TRUE) then
   dw_1x.setsort("titulacion_cuenta A")
	dw_1x.sort()
	if (cbx_2x.checked=TRUE) then
      dw_1x.setsort("ccarrera A, titulacion_cuenta A")
		dw_1x.sort()
	end if	
...
if (rb_3x.checked=TRUE) then
   dw_1x.setsort("cfecha_examen A")
	dw_1x.sort()
	if (cbx_2x.checked=TRUE) then
      dw_1x.setsort("ccarrera A, cfecha_examen A")
		dw_1x.sort()
	end if	
end if
...
integer height = 1232
integer taborder = 4
string dataobject = "dw_etiqueta_tit3x_nvo"
boolean hscrollbar = true
boolean vscrollbar = true
end type
