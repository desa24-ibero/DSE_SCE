$PBExportHeader$uo_nombre_carr_tit2.sru
$PBExportComments$Objeto para la ventana de diseño
forward
global type uo_nombre_carr_tit2 from userobject
end type
type em_digito from editmask within uo_nombre_carr_tit2
end type
type st_2 from statictext within uo_nombre_carr_tit2
end type
type st_1 from statictext within uo_nombre_carr_tit2
end type
type r_3 from rectangle within uo_nombre_carr_tit2
end type
type cbx_nuevo from checkbox within uo_nombre_carr_tit2
end type
type vsb_dw_nombre_alumno from vscrollbar within uo_nombre_carr_tit2
end type
type cb_2 from commandbutton within uo_nombre_carr_tit2
end type
type cb_1 from commandbutton within uo_nombre_carr_tit2
end type
type rb_actual from radiobutton within uo_nombre_carr_tit2
end type
type rb_anterior from radiobutton within uo_nombre_carr_tit2
end type
type gb_2 from groupbox within uo_nombre_carr_tit2
end type
type gb_1 from groupbox within uo_nombre_carr_tit2
end type
type cbx_navega from checkbox within uo_nombre_carr_tit2
end type
type em_cuenta from editmask within uo_nombre_carr_tit2
end type
type dw_nombre_alumno from datawindow within uo_nombre_carr_tit2
end type
end forward
...
global uo_nombre_carr_tit2 uo_nombre_carr_tit2
...
on uo_nombre_carr_tit2.destroy
...
w_titulacion2.dw_1. event carga(w_titulacion2.uo_nombre.em_cuenta.Text,&
...
//		if w_titulacion2.dw_1. event carga('0', '0', '0')<>0 then
			...
			/*
				w_titulacion2.uo_nombre.em_cuenta.event busqueda_simple(string(&
				w_titulacion2.dw_1.Object.cuenta[w_titulacion2.dw_1.GetRow()]))
			*/
			
			// MALH 10/11/2017 SE CORRIGE BUSQUEDA
			w_titulacion2.uo_nombre.em_cuenta.event busqueda_simple(em_cuenta.text)
//		end if
	else
		...
		elseif cbx_nuevo.checked = false then
				if gtr_sce.sqlcode = 100 then
					...
					if dw_nombre_alumno.retrieve(0) = 0 then
			  			dw_nombre_alumno.insertrow(0)
						w_titulacion2.dw_1.enabled = false
						w_titulacion2.dw_1.visible = false
						w_titulacion2.vsb_dw_certificado.visible=false
					end if
					goto fin
				end if
				em_cuenta.text = string(cuenta)
...
param_obj_nombre origen
	...
	if gtr_sce.sqlerrtext = "Select returned more than one row" then
		...
		if dw_nombre_alumno.retrieve(cuenta) = 0 then
			dw_nombre_alumno.insertrow(0)
			w_titulacion2.dw_1.InsertRow(0) /**/
			w_titulacion2.dw_1.enabled = false /**/
