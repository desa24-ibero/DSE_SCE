$PBExportHeader$uo_nombre_carr_tit.sru
$PBExportComments$Este objeto solo trae el nombre y carrera para el pase de parametros
forward
global type uo_nombre_carr_tit from userobject
end type
type em_digito from editmask within uo_nombre_carr_tit
end type
type st_2 from statictext within uo_nombre_carr_tit
end type
type st_1 from statictext within uo_nombre_carr_tit
end type
type r_3 from rectangle within uo_nombre_carr_tit
end type
type cbx_nuevo from checkbox within uo_nombre_carr_tit
end type
type vsb_dw_nombre_alumno from vscrollbar within uo_nombre_carr_tit
end type
type cb_2 from commandbutton within uo_nombre_carr_tit
end type
type cb_1 from commandbutton within uo_nombre_carr_tit
end type
type rb_actual from radiobutton within uo_nombre_carr_tit
end type
type rb_anterior from radiobutton within uo_nombre_carr_tit
end type
type gb_2 from groupbox within uo_nombre_carr_tit
end type
type gb_1 from groupbox within uo_nombre_carr_tit
end type
type cbx_navega from checkbox within uo_nombre_carr_tit
end type
type em_cuenta from editmask within uo_nombre_carr_tit
end type
type dw_nombre_alumno from datawindow within uo_nombre_carr_tit
end type
end forward
...
global uo_nombre_carr_tit uo_nombre_carr_tit
...
/*Si el número de cuenta entrado fué un 0*/
	if em_cuenta.text='0' then
		...
		if w_titulacion.dw_1. event carga('0','0', '0')<>0 then
			...
			w_titulacion.uo_nombre.em_cuenta.event busqueda_simple(string(&
				w_titulacion.dw_1.Object.cuenta[w_titulacion.dw_1.GetRow()]))
		...
		elseif cbx_nuevo.checked = false then
				if gtr_sce.sqlcode = 100 then
					...
					if dw_nombre_alumno.retrieve(0) = 0 then
						...
						w_titulacion.dw_1.visible = false
						w_titulacion.vsb_dw_certificado.visible=false
					end if
					goto fin
				end if
				em_cuenta.text = string(cuenta)
...
event getfocus;// MALH 10/11/2017 SE COMENTA DEBIDO A QUE CAUSABA ERROR EN LA VENTANA "w_titulacion" AL HACER LA BUSQUEDA DEL 
// ALUMNO USANDO LA VENTANA "w_alumnosnombre". AL CAMBIAR EL FOCO SE CERRABA LA VENTANA Y MAS ADELANTE SE 
// QUERIA HACER USO DE ELLA
/*
