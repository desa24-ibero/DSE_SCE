$PBExportHeader$uo_nombre_carrera_simple.sru
forward
global type uo_nombre_carrera_simple from UserObject
end type
type em_digito from editmask within uo_nombre_carrera_simple
end type
type st_2 from statictext within uo_nombre_carrera_simple
end type
type st_1 from statictext within uo_nombre_carrera_simple
end type
type r_3 from rectangle within uo_nombre_carrera_simple
end type
type cbx_nuevo from checkbox within uo_nombre_carrera_simple
end type
type vsb_dw_nombre_alumno from vscrollbar within uo_nombre_carrera_simple
end type
type cb_2 from commandbutton within uo_nombre_carrera_simple
end type
type cb_1 from commandbutton within uo_nombre_carrera_simple
end type
type em_cuenta from editmask within uo_nombre_carrera_simple
end type
type rb_actual from radiobutton within uo_nombre_carrera_simple
end type
type rb_anterior from radiobutton within uo_nombre_carrera_simple
end type
type gb_2 from groupbox within uo_nombre_carrera_simple
end type
type gb_1 from groupbox within uo_nombre_carrera_simple
end type
type dw_nombre_alumno from datawindow within uo_nombre_carrera_simple
end type
type cbx_navega from checkbox within uo_nombre_carrera_simple
end type
end forward
...
global uo_nombre_carrera_simple uo_nombre_carrera_simple
...
on uo_nombre_carrera_simple.create
...
on uo_nombre_carrera_simple.destroy
...
w_certificados_simples.dw_1. event carga(w_certificados_simples.uo_nombre.em_cuenta.Text,&
...
/*Si el número de cuenta entrado fué un 0*/
	if em_cuenta.text='0' then
		...
		if w_certificados_simples.dw_1. event carga('0','0','0')<>0 then
			...
			w_certificados_simples.uo_nombre.em_cuenta.event busqueda_simple(string(&
				w_certificados_simples.dw_1.Object.cuenta[w_certificados_simples.dw_1.GetRow()]))
		end if
	else
		...
		elseif cbx_nuevo.checked = false then
				if gtr_sce.sqlcode = 100 then
					...
					if dw_nombre_alumno.retrieve(0) = 0 then
			  			dw_nombre_alumno.insertrow(0)
						  //w_certificados_simples.dw_1.enabled = false
						w_certificados_simples.dw_1.visible = false
						w_certificados_simples.vsb_dw_certificado.visible=false
					end if
					goto fin
				end if
				em_cuenta.text = string(cuenta)
...
//if cbx_nuevo.checked = false then
	...
	if gtr_sce.sqlerrtext = "Select returned more than one row" then
		...
		if dw_nombre_alumno.retrieve(cuenta) = 0 then
			dw_nombre_alumno.insertrow(0)
			w_certificados_simples.dw_1.InsertRow(0) /**/
			//w_certificados_simples.dw_1.enabled = false /**/
		else 
			em_cuenta.text=string(cuenta)
