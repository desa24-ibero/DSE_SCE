$PBExportHeader$uo_nombre_carr_tit5.sru
$PBExportComments$Objeto para la ventana de entrega de titulos
forward
global type uo_nombre_carr_tit5 from userobject
end type
type em_digito from editmask within uo_nombre_carr_tit5
end type
type st_2 from statictext within uo_nombre_carr_tit5
end type
type st_1 from statictext within uo_nombre_carr_tit5
end type
type r_3 from rectangle within uo_nombre_carr_tit5
end type
type cbx_nuevo from checkbox within uo_nombre_carr_tit5
end type
type vsb_dw_nombre_alumno from vscrollbar within uo_nombre_carr_tit5
end type
type cb_2 from commandbutton within uo_nombre_carr_tit5
end type
type cb_1 from commandbutton within uo_nombre_carr_tit5
end type
type rb_actual from radiobutton within uo_nombre_carr_tit5
end type
type rb_anterior from radiobutton within uo_nombre_carr_tit5
end type
type gb_2 from groupbox within uo_nombre_carr_tit5
end type
type gb_1 from groupbox within uo_nombre_carr_tit5
end type
type cbx_navega from checkbox within uo_nombre_carr_tit5
end type
type em_cuenta from editmask within uo_nombre_carr_tit5
end type
type dw_nombre_alumno from datawindow within uo_nombre_carr_tit5
end type
end forward
...
global uo_nombre_carr_tit5 uo_nombre_carr_tit5
...
on uo_nombre_carr_tit5.destroy
...
w_titulacion5.dw_1. event carga(w_titulacion5.uo_nombre.em_cuenta.Text,&
...
//Error grave comentado par evitar una busqueda estupida 6/10/2003
//		if w_titulacion5.dw_1. event carga('0','0', '0')<>0 then
			...
			/*
				w_titulacion5.uo_nombre.em_cuenta.event busqueda_simple(string(&
				w_titulacion5.dw_1.Object.cuenta[w_titulacion5.dw_1.GetRow()]))
			*/
			
			// MALH 10/11/2017 SE CORRIGE BUSQUEDA
			w_titulacion5.uo_nombre.em_cuenta.event busqueda_simple(em_cuenta.text)
	      ...
	      WHERE alumnos.cuenta = :cuenta   using gtr_sce;
			...
			if cbx_nuevo.checked = true then
				...
				if dw_nombre_alumno.retrieve(cuenta) = 0 then
			   	dw_nombre_alumno.insertrow(0)
					w_titulacion5.dw_1.enabled = false
					w_titulacion5.dw_1.visible = false
					w_titulacion5.vsb_dw_certificado.visible=false
			end if
...
//if cbx_nuevo.checked = false then
	...
	if gtr_sce.sqlerrtext = "Select returned more than one row" then
		...
		if dw_nombre_alumno.retrieve(cuenta) = 0 then
			dw_nombre_alumno.insertrow(0)
			w_titulacion5.dw_1.InsertRow(0) /**/
			w_titulacion5.dw_1.enabled = false /**/
