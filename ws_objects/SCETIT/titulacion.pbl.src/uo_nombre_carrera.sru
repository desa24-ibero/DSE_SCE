$PBExportHeader$uo_nombre_carrera.sru
$PBExportComments$Objeto que busca al alumno por No. de Cta., por nombre o por apellidos, o efectuando una revisión en la tabla de alumnos y permite seleccionar la carrera en caso de tener varias.
forward
global type uo_nombre_carrera from userobject
end type
type em_digito from editmask within uo_nombre_carrera
end type
type st_2 from statictext within uo_nombre_carrera
end type
type st_1 from statictext within uo_nombre_carrera
end type
type r_3 from rectangle within uo_nombre_carrera
end type
type cbx_nuevo from checkbox within uo_nombre_carrera
end type
type vsb_dw_nombre_alumno from vscrollbar within uo_nombre_carrera
end type
type cb_2 from commandbutton within uo_nombre_carrera
end type
type cb_1 from commandbutton within uo_nombre_carrera
end type
type em_cuenta from editmask within uo_nombre_carrera
end type
type rb_actual from radiobutton within uo_nombre_carrera
end type
type rb_anterior from radiobutton within uo_nombre_carrera
end type
type gb_2 from groupbox within uo_nombre_carrera
end type
type gb_1 from groupbox within uo_nombre_carrera
end type
type dw_nombre_alumno from datawindow within uo_nombre_carrera
end type
type cbx_navega from checkbox within uo_nombre_carrera
end type
end forward
...
global uo_nombre_carrera uo_nombre_carrera
...
w_certificados.dw_1. event carga(w_certificados.uo_nombre.em_cuenta.Text,&
...
//		if w_certificados.dw_1. event carga('0','0','0')<>0 then
			...
			/*
			w_certificados.uo_nombre.em_cuenta.event busqueda_simple(string(&
			w_certificados.dw_1.Object.cuenta[w_certificados.dw_1.GetRow()]))
			*/
			
			// MALH 10/11/2017 SE CORRIGE BUSQUEDA
			w_certificados.uo_nombre.em_cuenta.event busqueda_simple(em_cuenta.text)
		...
		elseif cbx_nuevo.checked = false then
				if gtr_sce.sqlcode = 100 then
					...
					if dw_nombre_alumno.retrieve(0) = 0 then
			  			dw_nombre_alumno.insertrow(0)
						  //w_certificados.dw_1.enabled = false
						w_certificados.dw_1.visible = false
						w_certificados.vsb_dw_certificado.visible=false
					end if
					goto fin
				end if
				em_cuenta.text = string(cuenta)
...
//if cbx_nuevo.checked = false then
	...
	if gtr_sce.sqlerrtext = "Select returned more than one row" then
		...
		if dw_nombre_alumno.retrieve(cuenta) = 0 then
			dw_nombre_alumno.insertrow(0)
			w_certificados.dw_1.InsertRow(0) /**/
			//w_certificados.dw_1.enabled = false /**/
		else 
			em_cuenta.text=string(cuenta)
