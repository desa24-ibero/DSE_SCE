$PBExportHeader$uo_nombre_carr_tit_cat.sru
forward
global type uo_nombre_carr_tit_cat from UserObject
end type
type em_digito from editmask within uo_nombre_carr_tit_cat
end type
type st_2 from statictext within uo_nombre_carr_tit_cat
end type
type st_1 from statictext within uo_nombre_carr_tit_cat
end type
type r_3 from rectangle within uo_nombre_carr_tit_cat
end type
type cbx_nuevo from checkbox within uo_nombre_carr_tit_cat
end type
type vsb_dw_nombre_alumno from vscrollbar within uo_nombre_carr_tit_cat
end type
type cb_2 from commandbutton within uo_nombre_carr_tit_cat
end type
type cb_1 from commandbutton within uo_nombre_carr_tit_cat
end type
type rb_actual from radiobutton within uo_nombre_carr_tit_cat
end type
type rb_anterior from radiobutton within uo_nombre_carr_tit_cat
end type
type gb_2 from groupbox within uo_nombre_carr_tit_cat
end type
type gb_1 from groupbox within uo_nombre_carr_tit_cat
end type
type cbx_navega from checkbox within uo_nombre_carr_tit_cat
end type
type em_cuenta from editmask within uo_nombre_carr_tit_cat
end type
type dw_nombre_alumno from datawindow within uo_nombre_carr_tit_cat
end type
end forward
...
global uo_nombre_carr_tit_cat uo_nombre_carr_tit_cat
...
on uo_nombre_carr_tit_cat.create
...
on uo_nombre_carr_tit_cat.destroy
...
//w_titulacion.dw_1. event carga(w_titulacion.uo_nombre.em_cuenta.Text,&
//	string(dw_nombre_alumno.Object.academicos_cve_carrera[1]))
end event

event constructor;/* Inicializa variables */
enabled=FALSE
visible=FALSE
gb_2.visible=FALSE
...
boolean Border=false
string Mask="##aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
MaskDataType MaskDataType=StringMask!
string DisplayData=""
long BackColor=15793151
int TextSize=-10
...
long cuenta, ll_num_regs, ll_reg_actual, ll_num_rows, ll_cuenta, ll_dw_cuenta, ll_ult_reg
char digito,dig
long ll_renglon_actual
integer li_cve_carrera, li_cve_plan, li_subsistema
long ll_dat_alu_renglon

/*Verifica si hay cambios en el DataWindow dw_1 que no se hayan salvado*/
//w_titulacion.dw_1. event actualiza()
ll_dat_alu_renglon= dw_nombre_alumno.GetRow()
				...
				if em_digito.text = "" then
					...
					if digito = dig then	
						if dw_nombre_alumno.retrieve(cuenta) = 0 then
				  		...
				  		// ventana.triggerevent(doubleclicked!)	
						ll_num_rows = w_cat_carreras_ant.dw_catalogo.RowCount()  
						ll_reg_actual = 1
						ll_cuenta = cuenta
						do while ll_reg_actual <= ll_num_rows and ll_cuenta <> ll_dw_cuenta
						   ll_dw_cuenta = w_cat_carreras_ant.dw_catalogo.GetItemNumber(ll_reg_actual, "hist_carreras_cuenta")			
							ll_reg_actual = ll_reg_actual +1
						loop 
						ll_ult_reg = ll_reg_actual - 1
						if ll_cuenta =ll_dw_cuenta then
							w_cat_carreras_ant.dw_catalogo.ScrollToRow(ll_ult_reg)
							w_cat_carreras_ant.dw_catalogo.triggerevent("ue_filtra_carrera")							
						else
							if w_cat_carreras_ant.dw_catalogo.is_estatus <> 'Nuevo' then
								messagebox("Atención","No existen cambios de carrera para ese numero de cuenta",StopSign!)			
								em_cuenta.text = " "
								em_digito.text=" "
								if dw_nombre_alumno.retrieve(0) = 0 then
			  						dw_nombre_alumno.insertrow(0)
								end if
								ll_renglon_actual =w_cat_carreras_ant.dw_catalogo.GetRow()
								if ll_renglon_actual = 1 then
									w_cat_carreras_ant.dw_catalogo.triggerevent("ue_ultimo")
								else	
									w_cat_carreras_ant.dw_catalogo.triggerevent("ue_primero")
								end if
								goto fin
							else
//								w_cat_carreras_ant.dw_catalogo.SetFocus()
//								w_cat_carreras_ant.dw_catalogo.SetColumn("hist_carreras_cve_carrera_ant")
								li_cve_plan = dw_nombre_alumno.object.academicos_cve_plan[ll_dat_alu_renglon]
								li_cve_carrera =dw_nombre_alumno.object.academicos_cve_carrera[ll_dat_alu_renglon]
								li_subsistema =dw_nombre_alumno.object.academicos_cve_subsistema[ll_dat_alu_renglon]
						   	w_cat_carreras_ant.dw_catalogo.SetItem(ll_ult_reg, "hist_carreras_cuenta", ll_cuenta )			
						   	w_cat_carreras_ant.dw_catalogo.SetItem(ll_ult_reg, "hist_carreras_cve_carrera_act", li_cve_carrera )			
						   	w_cat_carreras_ant.dw_catalogo.SetItem(ll_ult_reg, "hist_carreras_cve_plan_act", li_cve_plan )			
						   	w_cat_carreras_ant.dw_catalogo.SetItem(ll_ult_reg, "hist_carreras_cve_subsistema_act", li_subsistema )																			
							end if
						end if
					else
...
int Height=538
int TabOrder=20
string DataObject="dw_nombre_alumno_carrera_cat"
end type
...
param_obj_nombre origen
	...
	if gtr_sce.sqlerrtext = "Select returned more than one row" then
		...
		if dw_nombre_alumno.retrieve(cuenta) = 0 then
			dw_nombre_alumno.insertrow(0)
//			w_titulacion.dw_1.InsertRow(0) /**/
//			w_titulacion.dw_1.enabled = false /**/
