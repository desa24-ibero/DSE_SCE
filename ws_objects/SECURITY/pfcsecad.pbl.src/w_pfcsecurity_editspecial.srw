$PBExportHeader$w_pfcsecurity_editspecial.srw
forward
global type w_pfcsecurity_editspecial from w_response
end type
type dw_user from u_dw within w_pfcsecurity_editspecial
end type
type cb_ok from u_cb within w_pfcsecurity_editspecial
end type
type cb_cancel from u_cb within w_pfcsecurity_editspecial
end type
end forward
...
global w_pfcsecurity_editspecial w_pfcsecurity_editspecial

type variables
Protected:
s_pfcsecurity_user_special istr_data
end variables

on w_pfcsecurity_editspecial.create
int iCurrent
call super::create
...
on w_pfcsecurity_editspecial.destroy
call super::destroy
...
dw_user.object.user_special[1] = istr_data.user_special
...
choose case istr_data.status // make it look correct 
	case 1 
		...
		dw_user.object.user_special.background.color = 16777215
	case 2
		this.title = 'Edit'
...
if istr_data.user_type = 0 then
	this.title = this.title + ' Special User'
...
elseif istr_data.user_type = 1 then
	this.title = this.title + ' Special Group'
else
...
if dw_user.modifiedcount() = 0 and istr_data.status <> 3 then 
	...
	li_dw = dw_user.object.user_special[1]
	istr_data.user_special = dw_user.object.user_special[1]
	li_str = istr_data.user_special
end if
