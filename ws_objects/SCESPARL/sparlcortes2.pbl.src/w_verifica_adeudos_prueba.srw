$PBExportHeader$w_verifica_adeudos_prueba.srw
forward
global type w_verifica_adeudos_prueba from window
end type
type uo_1 from uo_per_ani within w_verifica_adeudos_prueba
end type
type st_1 from statictext within w_verifica_adeudos_prueba
end type
type cbx_1 from checkbox within w_verifica_adeudos_prueba
end type
type cb_1 from commandbutton within w_verifica_adeudos_prueba
end type
end forward
...
global w_verifica_adeudos_prueba w_verifica_adeudos_prueba

type variables
Datastore dw_band
end variables

on w_verifica_adeudos_prueba.create
...
on w_verifica_adeudos_prueba.destroy
destroy(this.uo_1)
destroy(this.st_1)
destroy(this.cbx_1)
destroy(this.cb_1)
...
dw_band.DataObject = 'd_banderas_adeuda_prueba'
...
for ll_i = 1 to ll_tot
	...
	if ( (tiene_adeudos(ll_cuenta,gtr_scob) = 0) AND &
		( (pago_inscripcion(ll_cuenta, gi_anio ,gi_periodo, gtr_scob) = 1) OR (tiene_beca(ll_cuenta,gi_anio,gi_periodo, ltr_sfeb) = 100) )) then
