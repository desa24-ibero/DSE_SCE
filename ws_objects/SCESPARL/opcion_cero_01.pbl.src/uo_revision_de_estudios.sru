$PBExportHeader$uo_revision_de_estudios.sru
forward
global type uo_revision_de_estudios from nonvisualobject
end type
end forward

global type uo_revision_de_estudios from nonvisualobject
end type
global uo_revision_de_estudios uo_revision_de_estudios
...
ids_cuentas.DataObject = "d_cuenta_revision_estudios"
...
// Se hace el retrieve por cuenta, ya no es necesario hacer el filtro por tipo de periodo. 
...
For ll_i=1 To ids_cuentas.RowCount()
	...
	ls_Nivel = ids_cuentas.GetItemString(ll_i,"nivel")
...
UPDATE dbo.revision_de_estudios
SET  adeuda_finanzas = :li_tiene_adeudos
FROM dbo.revision_de_estudios
...
ELSEIF li_sql_code =100 THEN
	...
	WHERE hc.cve_carrera_ant = c.cve_carrera
	AND	c.nivel = "L" 
	...
	USING gtr_sce;
//hc.cve_carrera_ant not in (9532)
...
if (ai_subsistema<> 0) then
	...
	elseif as_nivel = "P" then
		...
		if li_num_subsistemas >0  and ai_subsistema= 0 then
			in_CredMin[AMeOp] = 1
			in_CredMin[AMeOb] = 1
		end if		
	end if
		...
		if in_tot_mat_a > 0 then
			for in_m=1 to in_tot_mat_a
				...
				if (in_renglon<>0) then
					...
					ids_mat_acred.setitem(in_i,6,ids_historico_area.getitemdecimal(in_m,6))//creditos
...
for in_i = 1 to 12
	...
	if (ids_mat_acred.RowCount()>0) then
		ids_revision.SetItem(in_i,2,ids_mat_acred.getitemdecimal(1,"creditos_cursados"))
...
ids_mat_acred.SetFilter("")
ids_mat_acred.Filter()
ids_mat_acred.SetSort("tipoarea A,periodonum A")
ids_mat_acred.Sort()
ids_mat_acred.GroupCalc()
...
if  al_cve_plan >= 3 AND as_nivel <> "P" then 
	if (ids_revision.getitemnumber(AI1,"cursados")+ids_revision.getitemnumber(AI2,"cursados")+&
...
If f_actualiza_creditos_revision_estudios(al_cuenta, al_cve_carrera, al_cve_plan, in_CredMin , in_TotCred )= -1 THEN
	RETURN FALSE	
else
	return true
end if
...
FROM dbo.historico h, dbo.revision_de_estudios tt
...
UPDATE dbo.revision_de_estudios
SET titulado = 1
FROM dbo.titulacion t, dbo.revision_de_estudios tt
...
UPDATE dbo.revision_de_estudios
SET certificado = 1
FROM dbo.certificado c, dbo.revision_de_estudios tt
...
UPDATE dbo.revision_de_estudios
SET egresado = 1
FROM dbo.academicos a, dbo.revision_de_estudios tt
...
UPDATE dbo.revision_de_estudios
SET egresado = 1
FROM dbo.hist_carreras h, dbo.revision_de_estudios tt
...
UPDATE dbo.revision_de_estudios
SET docs_nacim = 1
FROM dbo.documentos d, dbo.revision_de_estudios tt
...
FROM dbo.banderas b, dbo.revision_de_estudios tt
...
IF al_cve_carrera = 4601 OR al_cve_carrera = 4602 THEN
	...
	FROM revision_de_estudios tt, historico h
...
// Se agrega filtro por tipo de periodo del plan de estudios 
DELETE FROM revision_de_estudios
WHERE EXISTS (SELECT * FROM plan_estudios 
						where plan_estudios.cve_carrera = revision_de_estudios.cve_carrera 
							AND plan_estudios.cve_plan = revision_de_estudios.cve_plan 
...
// Se agrega filtro por tipo de periodo del plan de estudios 
INSERT INTO revision_de_estudios (cuenta, cve_carrera, cve_plan, cve_subsistema, egresado)
select distinct a.cuenta, a.cve_carrera, a.cve_plan, isnull(a.cve_subsistema,0), isnull(a.egresado,0)
from academicos a, carreras c 
where a.cve_carrera not in (0)
and  a.cve_carrera = c.cve_carrera
and exists (SELECT * FROM plan_estudios 
						where plan_estudios.cve_carrera = a.cve_carrera 
							AND plan_estudios.cve_plan = a.cve_plan 
							AND plan_estudios.tipo_periodo = :gs_tipo_periodo)
union
select distinct hc.cuenta, cve_carrera= hc.cve_carrera_ant, cve_plan = hc.cve_plan_ant, cve_subsistema = isnull(hc.cve_subsistema_ant,0), egresado= isnull(hc.egresado_ant,0)
from hist_carreras hc, carreras c
where hc.cve_carrera_ant not in (0)
and	hc.cve_carrera_ant = c.cve_carrera
and exists (SELECT * FROM plan_estudios 
						where plan_estudios.cve_carrera = hc.cve_carrera_ant 
							AND plan_estudios.cve_plan = hc.cve_plan_ant 
							AND plan_estudios.tipo_periodo = :gs_tipo_periodo)
USING gtr_sce;

/*and	c.nivel in ("L","P")   Se elimina esta condición  09-Ene-2017*/
/*and	c.nivel in ("L","P")   Se elimina esta condición  09-Ene-2017*/
...
Error:
	ROLLBACK USING gtr_sce;
	MessageBox("Error al Insertar revision_de_estudios", ls_mensaje,StopSign!)
	return -1
...
IF ge_plantel = 17 THEN 
	...
	/*and	c.nivel = "L"   Condición Original   09-Ene-2017*/
	/*and	c.nivel = "L"   Condición Original   09-Ene-2017*/
	
//	hc.cve_carrera_ant not in (9532)
//	and	hc.cve_plan_ant in (1,2,3,4,6,98)
//	and
//	a.cve_carrera not in (9532)
...
//	hc.cve_plan_ant in (1,2,3,4,6,98)
//	and
//	a.cve_plan in (1,2,3,4,6,98)
//	and	
...
SET prerreq_ingles = 0,
	 ...
	 adeuda_finanzas = 1,
	 baja_disciplina = 0
FROM dbo.revision_de_estudios, dbo.banderas_inscrito 
WHERE dbo.revision_de_estudios.cuenta = dbo.banderas_inscrito.cuenta 
AND EXISTS(SELECT * FROM dbo.plan_estudios
					WHERE dbo.plan_estudios.cve_plan = dbo.revision_de_estudios.cve_plan 
						AND dbo.plan_estudios.cve_carrera = dbo.revision_de_estudios.cve_carrera 
						AND dbo.plan_estudios.tipo_periodo = :gs_tipo_periodo) 
USING gtr_sce;
...
//Actualiza el tramite de titulación indicANDo a los que tienen aprobado el
//prerrequisito de ingles 
// Se toma la clave del ingles de la tabla de materias requisito y se agrega filtro de tipo de periodo.
UPDATE dbo.revision_de_estudios
SET prerreq_ingles = 1
FROM dbo.historico h, dbo.revision_de_estudios tt, dbo.banderas_inscrito bi
WHERE h.cve_mat in (SELECT cve_mat FROM materias_requisito WHERE id_prerrequisito = 'ING')  
AND h.calificacion = "AC"
AND h.cuenta = tt.cuenta
AND h.cve_carrera = tt.cve_carrera
AND h.cve_plan = tt.cve_plan
...
AND EXISTS( SELECT * FROM dbo.plan_estudios
					WHERE dbo.plan_estudios.cve_plan = tt.cve_plan 
						AND dbo.plan_estudios.cve_carrera = tt.cve_carrera 
						AND dbo.plan_estudios.tipo_periodo = :gs_tipo_periodo) 
USING gtr_sce;
...
UPDATE dbo.revision_de_estudios
SET titulado = 1
FROM dbo.titulacion t, dbo.revision_de_estudios tt, dbo.banderas_inscrito bi
WHERE t.cuenta = tt.cuenta
AND t.cve_carrera = tt.cve_carrera
AND t.cve_plan = tt.cve_plan
AND t.aprobado = 1
...
UPDATE dbo.revision_de_estudios
SET certificado = 1
FROM dbo.certificado c, dbo.revision_de_estudios tt, dbo.banderas_inscrito bi
WHERE c.cuenta = tt.cuenta
AND c.cve_carrera = tt.cve_carrera
AND c.cve_plan = tt.cve_plan
AND c.parcial = 0
...
UPDATE dbo.revision_de_estudios
SET egresado = 1
FROM dbo.academicos a, dbo.revision_de_estudios tt, dbo.banderas_inscrito bi
WHERE a.cuenta = tt.cuenta
AND a.cve_carrera = tt.cve_carrera
AND a.cve_plan = tt.cve_plan
AND a.egresado = 1
...
UPDATE dbo.revision_de_estudios
SET egresado = 1
FROM dbo.hist_carreras h, dbo.revision_de_estudios tt, dbo.banderas_inscrito bi
WHERE h.cuenta = tt.cuenta
AND h.cve_carrera_ant = tt.cve_carrera
AND h.cve_plan_ant = tt.cve_plan
AND h.egresado_ant = 1
...
UPDATE dbo.revision_de_estudios
SET docs_nacim = 1
FROM dbo.documentos d, dbo.revision_de_estudios tt, dbo.banderas_inscrito bi
WHERE d.cuenta = tt.cuenta
AND d.cve_flag_documento in (1,2,10,12)
AND d.en_archivo = 0
AND d.cve_documento in (1,14)
...
UPDATE dbo.revision_de_estudios
SET baja_biblioteca = 1
FROM dbo.banderas b, dbo.revision_de_estudios tt, dbo.banderas_inscrito bi
WHERE b.cuenta = tt.cuenta
AND b.cve_flag_biblioteca in (3)
...
UPDATE dbo.revision_de_estudios
SET adeuda_finanzas = 1
WHERE EXISTS( SELECT * FROM dbo.plan_estudios
					WHERE dbo.plan_estudios.cve_plan = dbo.revision_de_estudios.cve_plan 
						AND dbo.plan_estudios.cve_carrera = dbo.revision_de_estudios.cve_carrera 
						AND dbo.plan_estudios.tipo_periodo = :gs_tipo_periodo) 
USING gtr_sce;
...
on uo_revision_de_estudios.create
call super::create
TriggerEvent( this, "constructor" )
end on

on uo_revision_de_estudios.destroy
