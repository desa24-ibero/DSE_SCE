$PBExportHeader$w_extrao_tit_to_historico_2014.srw
forward
global type w_extrao_tit_to_historico_2014 from window
end type
type st_sistema from statictext within w_extrao_tit_to_historico_2014
end type
type p_ibero from picture within w_extrao_tit_to_historico_2014
end type
type uo_1 from uo_per_ani within w_extrao_tit_to_historico_2014
end type
type dw_extrao_tit_sin_calif from datawindow within w_extrao_tit_to_historico_2014
end type
type st_status from statictext within w_extrao_tit_to_historico_2014
end type
type cb_mat_insc_to_hist from commandbutton within w_extrao_tit_to_historico_2014
end type
type cb_poner5na from commandbutton within w_extrao_tit_to_historico_2014
end type
end forward
...
global w_extrao_tit_to_historico_2014 w_extrao_tit_to_historico_2014

on w_extrao_tit_to_historico_2014.create
...
on w_extrao_tit_to_historico_2014.destroy
...
lds_extrao_tit_anio_periodo.DataObject = "d_extrao_tit_anio_periodo_2014"
...
if lds_extrao_tit_anio_periodo.Retrieve(gi_periodo, gi_anio) >= 1 then
	...
	if dw_extrao_tit_sin_calif.Retrieve(gi_periodo, gi_anio) = 0 then
		...
		if li_status = 1 then 
			...
			FROM examen_extrao_titulo et, v_sce_carreras_cursadas a
		...
		if li_status = 3 then
			...
			if ll_mat > 0 then
				for ll_i = 1 to ll_mat
					...
					lds_historico_porcuentaanioper.SetItem(ll_i,"cve_carrera",lds_extrao_tit_anio_periodo.GetItemNumber(1,"v_sce_carreras_cursadas_cve_carrera"))
					lds_historico_porcuentaanioper.SetItem(ll_i,"cve_plan",lds_extrao_tit_anio_periodo.GetItemNumber(1,"v_sce_carreras_cursadas_cve_plan"))
					...
					if ( li_codigo_insert=1 and li_codigo_delete = 1) then
						...
						MessageBox("Atencion",string(lds_extrao_tit_anio_periodo.GetItemNumber(1,"examen_extrao_titulo_cuenta"))+&
										"-"+string(lds_extrao_tit_anio_periodo.GetItemNumber(1,"examen_extrao_titulo_cve_mat"))+&
