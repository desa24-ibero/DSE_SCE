$PBExportHeader$w_preinsctoreinsc.srw
forward
global type w_preinsctoreinsc from window
end type
type rb_pos_primer_ingreso_puntos from radiobutton within w_preinsctoreinsc
end type
type rb_normal_puntos from radiobutton within w_preinsctoreinsc
end type
type st_periodo from statictext within w_preinsctoreinsc
end type
type rb_posgrado_primer_ingreso from radiobutton within w_preinsctoreinsc
end type
type rb_normal from radiobutton within w_preinsctoreinsc
end type
type uo_nombre from uo_nombre_alumno_sparl within w_preinsctoreinsc
end type
type sle_2 from singlelineedit within w_preinsctoreinsc
end type
type sle_1 from singlelineedit within w_preinsctoreinsc
end type
type cb_1 from commandbutton within w_preinsctoreinsc
end type
type st_total from statictext within w_preinsctoreinsc
end type
type cb_preinsctoreinsc from commandbutton within w_preinsctoreinsc
end type
type dw_mat_preinsc from datawindow within w_preinsctoreinsc
end type
type p_uia from picture within w_preinsctoreinsc
end type
type dw_reinsc_mat from datawindow within w_preinsctoreinsc
end type
type dw_horario_mat from datawindow within w_preinsctoreinsc
end type
type dw_plan from datawindow within w_preinsctoreinsc
end type
type dw_ext_h from datawindow within w_preinsctoreinsc
end type
type dw_prerre from datawindow within w_preinsctoreinsc
end type
type dw_cursada from datawindow within w_preinsctoreinsc
end type
type gb_tipo_preinscripcion_reinscripcion from groupbox within w_preinsctoreinsc
end type
type dw_materias from datawindow within w_preinsctoreinsc
end type
type mensaje from structure within w_preinsctoreinsc
end type
end forward
...
rb_pos_primer_ingreso_puntos rb_pos_primer_ingreso_puntos
rb_normal_puntos rb_normal_puntos
...
global w_preinsctoreinsc w_preinsctoreinsc
...
Integer	li_resultado			// Oscar Sánchez, 04-Octubre-2018. La validación de horario debe ser por medio del llamado a un SP
String		ls_mensaje			// Oscar Sánchez, 04-Octubre-2018. La validación de horario debe ser por medio del llamado a un SP
LONG	li_materia_cruce	// Oscar Sánchez, 04-Octubre-2018. La validación de horario debe ser por medio del llamado a un SP
String		ls_grupo_cruce		// Oscar Sánchez, 04-Octubre-2018. La validación de horario debe ser por medio del llamado a un SP
String		ls_sesionado
int d,h, cont,c,verif

verif = dw_horario_mat.retrieve(mat,gpo,G_per,g_anio)

// Oscar Sánchez, 04-Octubre-2018. La validación de horario debe ser por medio del llamado a un SP
...
Return 0
// Oscar Sánchez, 04-Octubre-2018. La validación de horario debe ser por medio del llamado a un SP (FIN de la modificación)
	...
	if dw_horario_mat.rowcount()	> 0 then //Actualización de la estructura del horario 1 si esta ocupado o 0 si las libera
		for ren  = 1 to dw_horario_mat.rowcount()
			...
			for h = dw_horario_mat.getitemnumber(ren,"hora_inicio") to dw_horario_mat.getitemnumber(ren,"hora_final") -1
				...
				//	Oscar Sánchez		 04-Oct-2018 Los horarios deben presentar el grupo y fechas de inicio y fin
...
on w_preinsctoreinsc.create
if this.MenuName = "m_preinsctoreinsc" then this.MenuID = create m_preinsctoreinsc
this.rb_pos_primer_ingreso_puntos=create rb_pos_primer_ingreso_puntos
this.rb_normal_puntos=create rb_normal_puntos
...
this.Control[]={this.rb_pos_primer_ingreso_puntos,&
this.rb_normal_puntos,&
this.st_periodo,&
...
on w_preinsctoreinsc.destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.rb_pos_primer_ingreso_puntos)
destroy(this.rb_normal_puntos)
...
long backcolor = 12639424
string text = "Posgrado Primer Ingreso Puntos"
end type
...
rb_normal_puntos.BackColor = RGB ( 0, 125, 125 )
rb_pos_primer_ingreso_puntos.BackColor = RGB ( 0, 125, 125 )
...
long textcolor = 33554432
long backcolor = 12639424
string text = "Proceso Normal Puntos"
end type
...
rb_normal_puntos.BackColor =  il_back_color
rb_pos_primer_ingreso_puntos.BackColor = il_back_color 
...
elseif rb_normal_puntos.checked then 
	ls_dw_tipo_preinscripcion = "d_preinsc_por_anio_periodo_puntos"
	ls_mensaje_tipo = "¿Desea ejecutar la Preinscripción a Reinscripción para NORMAL POR PUNTOS?"
elseif rb_pos_primer_ingreso_puntos.checked then 
	ls_dw_tipo_preinscripcion = "d_preinsc_pos_prim_ing_puntos"
	ls_mensaje_tipo = "¿Desea ejecutar la Preinscripción a Reinscripción para POSGRADO PRIMER INGRESO POR PUNTOS?"
else
...
IF rb_normal_puntos.CHECKED OR rb_pos_primer_ingreso_puntos.CHECKED THEN 
...
if rowcount()	>	0 then
	...
	if getitemnumber(getrow(),1) > 0 and getitemstring(getrow(),2) <> " " then
		...
		if borra_mat_insc(long(uo_nombre.em_cuenta.text),cvmat,gpo) = 0 then		//borra la materia de materias inscritas del alumno
			...
			dw_ext_h.event Constructor()	//	Oscar Sánchez		 04-Oct-2018 Inicializar la visualizacion de los horarios
			event actualizahorario()			//	Oscar Sánchez		 04-Oct-2018 Refrescar los horarios después de borrar
