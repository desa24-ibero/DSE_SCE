$PBExportHeader$w_eva_extra.srw
forward
global type w_eva_extra from w_ancestral
end type
type cb_4 from commandbutton within w_eva_extra
end type
type cb_3 from commandbutton within w_eva_extra
end type
type dw_extra_tit from datawindow within w_eva_extra
end type
type cb_2 from commandbutton within w_eva_extra
end type
type cb_1 from commandbutton within w_eva_extra
end type
type dw_actas from uo_dw_captura within w_eva_extra
end type
type uo_1 from uo_per_ani within w_eva_extra
end type
end forward
...
integer width = 3163
integer height = 2275
string title = "Lectura de Actas de Evaluación Extraordinario"
...
global w_eva_extra w_eva_extra

on w_eva_extra.create
int iCurrent
call super::create
...
type p_uia from w_ancestral`p_uia within w_eva_extra
end type
...
event clicked;long ll_cont_1, ll_cont_2, ll_retri, ll_clave, ll_pos, ll_inscritos
string ls_calif, ls_grupo

FOR ll_cont_1=1 TO dw_actas.rowcount()
	ll_pos = dw_actas.scrolltorow(ll_cont_1)
	ll_clave = dw_actas.object.clave[ll_cont_1]
	ls_grupo = dw_actas.object.grupo[ll_cont_1]
	ll_retri = dw_extra_tit.retrieve(gi_anio,gi_periodo,dw_actas.object.clave[ll_cont_1],dw_actas.object.grupo[ll_cont_1],2)
	ll_inscritos = dw_actas.object.inscritos[ll_cont_1] 
...
event doubleclicked;call super::doubleclicked;if row>0 then
	dw_extra_tit.retrieve(gi_anio,gi_periodo,dw_actas.object.clave[row],dw_actas.object.grupo[row],2)
end if
end event
...
/*
	messagebox(gi_ls_anio+' '+gi_periodo+' '+ls_clave1+' '+ls_clave2+' '+ls_clave3+' ',ls_clave4+' '+ls_grupo1+' '+&
