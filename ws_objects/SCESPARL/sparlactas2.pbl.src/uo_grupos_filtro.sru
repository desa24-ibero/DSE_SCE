$PBExportHeader$uo_grupos_filtro.sru
forward
global type uo_grupos_filtro from userobject
end type
type cbx_descuenta_sdu_se from checkbox within uo_grupos_filtro
end type
type rb_editar from radiobutton within uo_grupos_filtro
end type
type rb_insertar from radiobutton within uo_grupos_filtro
end type
type dw_1 from datawindow within uo_grupos_filtro
end type
end forward
...
global uo_grupos_filtro uo_grupos_filtro
...
//elseif isvalid(w_tcap_diplomas) then
//	w_ventana  =  w_tcap_diplomas
//end if
if isvalid(w_acta_oficial_impresion) then
	w_ventana  =  w_acta_oficial_impresion
end if
