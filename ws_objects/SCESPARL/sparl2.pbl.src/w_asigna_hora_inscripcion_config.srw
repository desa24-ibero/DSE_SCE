$PBExportHeader$w_asigna_hora_inscripcion_config.srw
forward
global type w_asigna_hora_inscripcion_config from w_main
end type
type rb_puntos from radiobutton within w_asigna_hora_inscripcion_config
end type
type rb_promcrd from radiobutton within w_asigna_hora_inscripcion_config
end type
type rb_crdprom from radiobutton within w_asigna_hora_inscripcion_config
end type
type dw_ordena_fechas_inscripcion from u_dw within w_asigna_hora_inscripcion_config
end type
type cb_establece_folio_sol from commandbutton within w_asigna_hora_inscripcion_config
end type
type em_cuenta_registros from editmask within w_asigna_hora_inscripcion_config
end type
type cb_1 from commandbutton within w_asigna_hora_inscripcion_config
end type
type cb_asigna_horario_inscripcion from commandbutton within w_asigna_hora_inscripcion_config
end type
type cb_limpiar from commandbutton within w_asigna_hora_inscripcion_config
end type
type cb_eliminar from commandbutton within w_asigna_hora_inscripcion_config
end type
type mc_calendar from monthcalendar within w_asigna_hora_inscripcion_config
end type
type cb_establece_criterio from commandbutton within w_asigna_hora_inscripcion_config
end type
type uo_1 from uo_per_ani within w_asigna_hora_inscripcion_config
end type
type dw_cuentas_bloque_horario_insc from u_dw within w_asigna_hora_inscripcion_config
end type
type rb_licenciatura from radiobutton within w_asigna_hora_inscripcion_config
end type
type rb_posgrado from radiobutton within w_asigna_hora_inscripcion_config
end type
type rb_preinscritos from radiobutton within w_asigna_hora_inscripcion_config
end type
type rb_no_preinscritos from radiobutton within w_asigna_hora_inscripcion_config
end type
type rb_todos from radiobutton within w_asigna_hora_inscripcion_config
end type
type rb_ingreso from radiobutton within w_asigna_hora_inscripcion_config
end type
type dw_bloque_horario_insc from u_dw within w_asigna_hora_inscripcion_config
end type
type em_tamanio_bloque_mins from editmask within w_asigna_hora_inscripcion_config
end type
type st_3 from statictext within w_asigna_hora_inscripcion_config
end type
type cb_agregar_horario from commandbutton within w_asigna_hora_inscripcion_config
end type
type st_2 from statictext within w_asigna_hora_inscripcion_config
end type
type st_1 from statictext within w_asigna_hora_inscripcion_config
end type
type em_fin_receso from editmask within w_asigna_hora_inscripcion_config
end type
type em_inicio_receso from editmask within w_asigna_hora_inscripcion_config
end type
type em_hora_fin from editmask within w_asigna_hora_inscripcion_config
end type
type em_hora_ini from editmask within w_asigna_hora_inscripcion_config
end type
type p_final from picture within w_asigna_hora_inscripcion_config
end type
type em_calendar_final from u_em within w_asigna_hora_inscripcion_config
end type
type p_inicial from picture within w_asigna_hora_inscripcion_config
end type
type em_calendar_inicial from u_em within w_asigna_hora_inscripcion_config
end type
type gb_preinscripcion from groupbox within w_asigna_hora_inscripcion_config
end type
type gb_grado from groupbox within w_asigna_hora_inscripcion_config
end type
type lb_dias_inscripcion from listbox within w_asigna_hora_inscripcion_config
end type
type gb_seleccionados from groupbox within w_asigna_hora_inscripcion_config
end type
type gb_1 from groupbox within w_asigna_hora_inscripcion_config
end type
end forward
...
rb_puntos rb_puntos
rb_promcrd rb_promcrd
rb_crdprom rb_crdprom
...
global w_asigna_hora_inscripcion_config w_asigna_hora_inscripcion_config
...
public subroutine wf_orden ()
public function integer wf_carga_cuentas (integer ai_periodo, integer ai_anio, string as_tipo_periodo)
end prototypes
...
/**/
IF ll_rows_bloque = ll_num_bloques_editados THEN 
	ll_rows_bloque = ll_rows_bloque
END IF 	
/**/
...
public subroutine wf_orden ();STRING ls_set_sort 
INTEGER li_set_sort, li_sort

IF rb_crdprom.CHECKED THEN 
	ls_set_sort = "creditos_cursados desc, promedio desc, sem_cursados asc, nombre_completo asc"
ELSEIF rb_promcrd.CHECKED THEN 
	ls_set_sort = "promedio desc, creditos_cursados desc, sem_cursados asc, cuenta asc"
ELSEIF rb_puntos.CHECKED THEN 	
	ls_set_sort = "puntos desc, cuenta asc" 	
END IF
...
lds_paso.DATAOBJECT = dw_cuentas_bloque_horario_insc.DATAOBJECT 

dw_cuentas_bloque_horario_insc.RESET() 

//lds_paso.MODIFY("Datawindow.Table.Select = '" + ls_sql + "'" )
//lds_paso.SETTRANSOBJECT(gtr_sce)
//ll_rows = lds_paso.RETRIEVE() 
//lds_paso.ROWSCOPY(1, lds_paso.ROWCOUNT(), PRIMARY!, dw_cuentas_bloque_horario_insc, dw_cuentas_bloque_horario_insc.ROWCOUNT() + 1, PRIMARY!)  
//

ls_sql = " SELECT cuenta = ac.cuenta, nivel = ac.nivel, preinscrito = 1, " + & 
" creditos_cursados = ac.creditos_cursados, sem_cursados = ac.sem_cursados, promedio = ac.promedio, nombre_completo = al.nombre_completo,  " + & 
" puntos = isnull(puntos.puntos, 0)  " + & 
" FROM academicos ac " + & 
" LEFT OUTER JOIN v_cuenta_puntos_horario puntos ON ac.cuenta = puntos.cuenta " + & 
" , cuentas_cortes cc, alumnos al " + & 
" WHERE ac.egresado = 0 " + & 
" AND ac.cuenta = al.cuenta " + & 
" AND ac.cuenta in (select cuenta from preinsc) " + & 
" AND ac.cuenta = cc.cuenta AND cc.cuenta > 0 "

//dw_cuentas_bloque_horario_insc
//dw_cuentas_bloque_horario_insc.MODIFY("Datawindow.Table.Select = '" + ls_sql + "'" )
//dw_cuentas_bloque_horario_insc.SETTRANSOBJECT(gtr_sce)
//ll_rows = dw_cuentas_bloque_horario_insc.RETRIEVE() 
//dw_cuentas_bloque_horario_insc.ROWSCOPY(1, lds_paso.ROWCOUNT(), PRIMARY!, dw_cuentas_bloque_horario_insc, dw_cuentas_bloque_horario_insc.ROWCOUNT() + 1, PRIMARY!)  

lds_paso.MODIFY("Datawindow.Table.Select = '" + ls_sql + "'" )
lds_paso.SETTRANSOBJECT(gtr_sce)
ll_rows = lds_paso.RETRIEVE(ai_periodo, ai_anio, as_tipo_periodo) 
IF ll_rows < 0 THEN MESSAGEBOX("Error", "Error al recuperar alumnos 1")
lds_paso.ROWSCOPY(1, lds_paso.ROWCOUNT(), PRIMARY!, dw_cuentas_bloque_horario_insc, dw_cuentas_bloque_horario_insc.ROWCOUNT() + 1, PRIMARY!)  

//UNION

ls_sql = " SELECT cuenta = ac.cuenta, nivel = ac.nivel, preinscrito = 0,  " + & 
" creditos_cursados = ac.creditos_cursados, sem_cursados = ac.sem_cursados, promedio = ac.promedio, nombre_completo = al.nombre_completo,  " + & 
" puntos = isnull(puntos.puntos, 0)  " + & 
" FROM academicos ac LEFT OUTER JOIN v_cuenta_puntos_horario puntos ON ac.cuenta = puntos.cuenta " + & 
" , cuentas_cortes cc, alumnos al " + & 
" WHERE ac.egresado = 0 " + & 
" AND ac.cuenta = al.cuenta " + & 
" AND ac.cuenta not in (select cuenta from preinsc) " + & 
" AND ac.cuenta = cc.cuenta AND cc.cuenta > 0 " 

lds_paso.MODIFY("Datawindow.Table.Select = '" + ls_sql + "'" )
lds_paso.SETTRANSOBJECT(gtr_sce)
ll_rows = lds_paso.RETRIEVE(ai_periodo, ai_anio, as_tipo_periodo) 
IF ll_rows < 0 THEN MESSAGEBOX("Error", "Error al recuperar alumnos 2")
lds_paso.ROWSCOPY(1, lds_paso.ROWCOUNT(), PRIMARY!, dw_cuentas_bloque_horario_insc, dw_cuentas_bloque_horario_insc.ROWCOUNT() + 1, PRIMARY!)  

//UNION

ls_sql = " SELECT cuenta = hr.cuenta, nivel = ac.nivel, preinscrito = 1,  " + & 
" creditos_cursados = ac.creditos_cursados, sem_cursados = ac.sem_cursados, promedio = ac.promedio, nombre_completo = al.nombre_completo,  " + & 
" puntos = isnull(puntos.puntos, 0)  " + & 
" FROM academicos ac LEFT OUTER JOIN v_cuenta_puntos_horario puntos ON ac.cuenta = puntos.cuenta " + & 
" , hist_reingreso hr, alumnos al " + & 
" WHERE hr.cuenta = ac.cuenta " + & 
" AND ac.cuenta = al.cuenta " + & 
" AND ac.egresado =  0 " + &
" AND hr.periodo_ing = " + STRING(ai_periodo)  + &
" AND hr.anio_ing =  " + STRING(ai_anio) + & 
" AND ac.cuenta IN (select cuenta from preinsc) " + &
" AND ac.cuenta NOT in (select cuenta from cuentas_cortes) AND ac.cuenta > 0  " 

lds_paso.MODIFY("Datawindow.Table.Select = '" + ls_sql + "'" )
lds_paso.SETTRANSOBJECT(gtr_sce)
ll_rows = lds_paso.RETRIEVE(ai_periodo, ai_anio, as_tipo_periodo) 
IF ll_rows < 0 THEN MESSAGEBOX("Error", "Error al recuperar alumnos 3")
lds_paso.ROWSCOPY(1, lds_paso.ROWCOUNT(), PRIMARY!, dw_cuentas_bloque_horario_insc, dw_cuentas_bloque_horario_insc.ROWCOUNT() + 1, PRIMARY!)  

//UNION

ls_sql = " SELECT cuenta = hr.cuenta, nivel = ac.nivel, preinscrito = 0,  " + &
" creditos_cursados = ac.creditos_cursados, sem_cursados = ac.sem_cursados, promedio = ac.promedio, nombre_completo = al.nombre_completo,  " + &
" puntos = isnull(puntos.puntos, 0)  " + &
" FROM academicos ac LEFT OUTER JOIN v_cuenta_puntos_horario puntos ON ac.cuenta = puntos.cuenta " + &
" ,hist_reingreso hr, alumnos al " + &
...
lds_paso.SETTRANSOBJECT(gtr_sce)
ll_rows = lds_paso.RETRIEVE(ai_periodo, ai_anio, as_tipo_periodo) 
IF ll_rows < 0 THEN MESSAGEBOX("Error", "Error al recuperar alumnos 4")
lds_paso.ROWSCOPY(1, lds_paso.ROWCOUNT(), PRIMARY!, dw_cuentas_bloque_horario_insc, dw_cuentas_bloque_horario_insc.ROWCOUNT() + 1, PRIMARY!)  

//UNION

ls_sql = " SELECT cuenta = ar.cuenta, nivel = ac.nivel,  preinscrito = 0,  " + &
" creditos_cursados = ac.creditos_cursados, sem_cursados = ac.sem_cursados, promedio = ac.promedio, nombre_completo = al.nombre_completo,  " + &
" puntos = isnull(puntos.puntos, 0)  " + &
" FROM academicos ac LEFT OUTER JOIN v_cuenta_puntos_horario puntos ON ac.cuenta = puntos.cuenta " + &
" , aspirantes_revalidacion ar, alumnos al " + &
" WHERE ar.cuenta = ac.cuenta " + &
" AND ac.cuenta = al.cuenta " + &
" AND ac.egresado = 0 " + &
" AND ar.periodo_ing = " + STRING(ai_periodo) + & 
" AND ar.anio_ing = " + STRING(ai_anio) + &
" AND ac.cuenta NOT in (select cuenta from preinsc) " + &
" AND ac.cuenta NOT in (select cuenta from cuentas_cortes) AND ac.cuenta > 0 " 
...
lds_paso.SETTRANSOBJECT(gtr_sce)
ll_rows = lds_paso.RETRIEVE(ai_periodo, ai_anio, as_tipo_periodo) 
IF ll_rows < 0 THEN MESSAGEBOX("Error", "Error al recuperar alumnos 5")
lds_paso.ROWSCOPY(1, lds_paso.ROWCOUNT(), PRIMARY!, dw_cuentas_bloque_horario_insc, dw_cuentas_bloque_horario_insc.ROWCOUNT() + 1, PRIMARY!)  

//UNION

ls_sql = " SELECT cuenta = ar.cuenta, nivel = ac.nivel,  preinscrito = 1,  " + &
" creditos_cursados = ac.creditos_cursados, sem_cursados = ac.sem_cursados, promedio = ac.promedio, nombre_completo = al.nombre_completo,  " + &
" puntos = isnull(puntos.puntos, 0)  " + &
" FROM academicos ac  LEFT OUTER JOIN v_cuenta_puntos_horario puntos ON ac.cuenta = puntos.cuenta " + &
" ,aspirantes_revalidacion ar, alumnos al " + &
...
lds_paso.SETTRANSOBJECT(gtr_sce)
ll_rows = lds_paso.RETRIEVE(ai_periodo, ai_anio, as_tipo_periodo)  
IF ll_rows < 0 THEN MESSAGEBOX("Error", "Error al recuperar alumnos 6")
lds_paso.ROWSCOPY(1, lds_paso.ROWCOUNT(), PRIMARY!, dw_cuentas_bloque_horario_insc, dw_cuentas_bloque_horario_insc.ROWCOUNT() + 1, PRIMARY!)  
...
on w_asigna_hora_inscripcion_config.create
int iCurrent
call super::create
this.rb_puntos=create rb_puntos
this.rb_promcrd=create rb_promcrd
this.rb_crdprom=create rb_crdprom
...
this.Control[iCurrent+1]=this.rb_puntos
this.Control[iCurrent+2]=this.rb_promcrd
this.Control[iCurrent+3]=this.rb_crdprom
this.Control[iCurrent+4]=this.dw_ordena_fechas_inscripcion
this.Control[iCurrent+5]=this.cb_establece_folio_sol
this.Control[iCurrent+6]=this.em_cuenta_registros
this.Control[iCurrent+7]=this.cb_1
this.Control[iCurrent+8]=this.cb_asigna_horario_inscripcion
this.Control[iCurrent+9]=this.cb_limpiar
this.Control[iCurrent+10]=this.cb_eliminar
this.Control[iCurrent+11]=this.mc_calendar
this.Control[iCurrent+12]=this.cb_establece_criterio
this.Control[iCurrent+13]=this.uo_1
this.Control[iCurrent+14]=this.dw_cuentas_bloque_horario_insc
this.Control[iCurrent+15]=this.rb_licenciatura
this.Control[iCurrent+16]=this.rb_posgrado
this.Control[iCurrent+17]=this.rb_preinscritos
this.Control[iCurrent+18]=this.rb_no_preinscritos
this.Control[iCurrent+19]=this.rb_todos
this.Control[iCurrent+20]=this.rb_ingreso
this.Control[iCurrent+21]=this.dw_bloque_horario_insc
this.Control[iCurrent+22]=this.em_tamanio_bloque_mins
this.Control[iCurrent+23]=this.st_3
this.Control[iCurrent+24]=this.cb_agregar_horario
this.Control[iCurrent+25]=this.st_2
this.Control[iCurrent+26]=this.st_1
this.Control[iCurrent+27]=this.em_fin_receso
this.Control[iCurrent+28]=this.em_inicio_receso
this.Control[iCurrent+29]=this.em_hora_fin
this.Control[iCurrent+30]=this.em_hora_ini
this.Control[iCurrent+31]=this.p_final
this.Control[iCurrent+32]=this.em_calendar_final
this.Control[iCurrent+33]=this.p_inicial
this.Control[iCurrent+34]=this.em_calendar_inicial
this.Control[iCurrent+35]=this.gb_preinscripcion
this.Control[iCurrent+36]=this.gb_grado
this.Control[iCurrent+37]=this.lb_dias_inscripcion
this.Control[iCurrent+38]=this.gb_seleccionados
this.Control[iCurrent+39]=this.gb_1
end on

on w_asigna_hora_inscripcion_config.destroy
call super::destroy
destroy(this.rb_puntos)
destroy(this.rb_promcrd)
destroy(this.rb_crdprom)
...
dw_cuentas_bloque_horario_insc.SetTransObject(gtr_sce)
//dw_cuentas_bloque_horario_insc.Retrieve(gi_periodo, gi_anio, gs_tipo_periodo)
...
IF f_foliar_horario_solicitudes_conf(ls_cve_criterio,gi_periodo, gi_anio)	= -1 THEN
...
//   IF lb_dias_inscripcion.State(li_ItemCount) = 1 THEN 
		...
		ELSEIF ls_cond_preinsc = 'P' THEN
			...
			IF ls_grado = "L" THEN 
				ls_filter = "nivel <> 'P' AND preinscrito = "+string(li_preinscrito 	)
			ELSE
				ls_filter = "nivel = '"+ls_grado + "' AND preinscrito = "+string(li_preinscrito 	)
			END IF						
...
// La hora final es la fecha libre del guion hasta la hora correspondiente
		...
		//ll_rows_cuenta = dw_cuentas_bloque_horario_insc.Retrieve(gi_periodo, gi_anio, gs_tipo_periodo)
		wf_carga_cuentas(gi_periodo, gi_anio, gs_tipo_periodo)
		ll_rows_cuenta = dw_cuentas_bloque_horario_insc.ROWCOUNT() 
		...
		// Se pasa el orden a función para que sea según la selección
//		ls_set_sort = "creditos_cursados desc, promedio desc, sem_cursados asc, nombre_completo asc"
//
//		//Ordena las cuentas por creditos, promedio, semestres y nombre
//		li_set_sort = dw_cuentas_bloque_horario_insc.SetSort(ls_set_sort)
...
// El orden se pasa a función y es según la selección
//ls_set_sort = "creditos_cursados desc, promedio desc, sem_cursados asc, nombre_completo asc"
//
//li_set_sort = dw_cuentas_bloque_horario_insc.SetSort(ls_set_sort)
//li_sort = dw_cuentas_bloque_horario_insc.Sort()
...
integer height = 356
integer taborder = 30
string dataobject = "d_cuentas_bloque_horario_insc"
boolean resizable = true
end type
...
long ll_num_alumnos, ll_rows_cuenta 
...
IF dwo.name = "num_alumnos" THEN	
	...
	ll_rows_cuenta = dw_cuentas_bloque_horario_insc.RowCount()
	IF ll_rows_cuenta <= 0 THEN 
		MessageBox("Aviso", "No se ha establecido el criterio de asignación",StopSign!) 
		RETURN 1		
	END IF 
...
//ls_fecha_inicio = em_calendar_inicial.text 
//
//ls_anio = MID(ls_fecha_inicio, 7, 4)
//ls_mes = MID(ls_fecha_inicio, 4, 2)
//ls_dia = MID(ls_fecha_inicio, 1, 2)
//
//ldt_calendar_inicial = date(INTEGER(ls_anio), INTEGER(ls_mes), INTEGER(ls_dia))
//ldt_calendar_inicial = date(ls_anio + "/" + ls_mes + "/" + ls_dia)
