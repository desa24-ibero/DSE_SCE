...
if not isvalid( gtr_sit) then 
	if f_conecta_bd(gtr_sit,"SIT",gs_usuario,gs_password) =0 then 
...
if not isvalid( gtr_sce) then 
	if f_conecta_bd(gtr_sce,"SCE",gs_usuario,gs_password) =0 then 
...
if not isvalid( gtr_sfeb) then 
	if f_conecta_bd(gtr_sfeb,"SFEB",gs_usuario,gs_password) = 0 then 
...
// 
if isvalid( gtr_sce)  then f_desconecta_bd(gtr_sce) 
if isvalid( gtr_sfeb) then f_desconecta_bd(gtr_sfeb)
if isvalid( gtr_sadm) then f_desconecta_bd(gtr_sadm)
...
uo_datastore lds_bandera_inscritos
lds_bandera_inscritos = CREATE uo_datastore
lds_bandera_inscritos.DataObject = "d_bandera_inscri"
lds_bandera_inscritos.settransobject(gtr_sce)

uo_datastore lds_pago_reg
lds_pago_reg = CREATE uo_datastore
lds_pago_reg.DataObject = "d_val_pago_regist"
lds_pago_reg.SetTransObject(gtr_sit)
lds_pago_reg.retrieve(ii_anio,ii_periodo)

uo_datastore lds_concepto_33
lds_concepto_33 = CREATE uo_datastore
lds_concepto_33.DataObject = "d_sdo_concepto33_vista"
lds_concepto_33.SetTransObject(gtr_sit)
lds_concepto_33.retrieve(ii_anio,ii_periodo,33)

uo_datastore lds_becas_al100
lds_becas_al100 = CREATE uo_datastore
lds_becas_al100.DataObject = "d_beca_apoyo_al_100"
lds_becas_al100.SetTransObject(gtr_sfeb)
lds_becas_al100.Retrieve(ii_anio,ii_periodo)

uo_datastore lds_minmax_cta
lds_minmax_cta = CREATE uo_datastore
lds_minmax_cta.DataObject = "d_maxmin_ctas_banderas"
...
uo_datastore lds_adeudos
lds_adeudos = CREATE uo_datastore
lds_adeudos.DataObject = "d_sdo_concep_deu_vista"
lds_adeudos.SetTransObject(gtr_sit)
lds_adeudos.retrieve(ll_cta_min,ll_cta_max)

uo_datastore lds_max_costo_insc
lds_max_costo_insc = CREATE uo_datastore
lds_max_costo_insc.DataObject = "d_max_costo_inscrip"
lds_max_costo_insc.SetTransObject(gtr_sit)

//
//	Extrae la fecha vencimiento costo de inscripción reingreso Lic. 
//	
ll_recupera = f_fechas_vencimiento(0,4,ii_periodo, ii_anio,1, 0, ldt_Fvento_ins_lic, &
...
if ll_recupera <= 0 then
	...
	//	
	ll_recupera = f_fechas_vencimiento(ll_cuenta,6,ii_periodo, ii_anio,1, 0, ldt_Fvento_ins_pos, &
												  ld_tasa_vento, ls_mensaje_err)
	if ll_recupera <= 0 then
...
boolean bringtotop = true
string title = "none"
string dataobject = "d_rep_alumnos_bloq_adeu"
