$PBExportHeader$w_promedio.srw
forward
global type w_promedio from w_ancestral
end type
type dw_cred_75_ss from datawindow within w_promedio
end type
type dw_prom_cred_sem from datawindow within w_promedio
end type
type st_status_2 from statictext within w_promedio
end type
type st_status_1 from statictext within w_promedio
end type
type cb_1 from commandbutton within w_promedio
end type
type dw_cortes from uo_dw_reporte within w_promedio
end type
type uo_1 from uo_per_ani within w_promedio
end type
end forward
...
type p_uia from w_ancestral`p_uia within w_promedio
end type
...
integer height = 432
integer taborder = 30
string dataobject = "d_cred_75_ss_x_periodo"
...
FOR lo_cont_1=1 TO dw_prom_cred_sem.rowcount()
	...
	li_periodo_ing = dw_prom_cred_sem.object.periodo_ing[lo_cont_1]
	li_anio_ing = dw_prom_cred_sem.object.anio_ing[lo_cont_1]	
	
//	if dw_prom_cred_sem.object.academicos_nivel[lo_cont_1]='L' then
	if dw_prom_cred_sem.object.academicos_nivel[lo_cont_1] <> 'P' then
		lo_cont_2=1
//		DO UNTIL ((object.plan_estudios_cve_plan[lo_cont_2]=in_plan &
//						and object.plan_estudios_cve_carrera[lo_cont_2]=in_carrera))
		DO UNTIL ((object.cve_plan[lo_cont_2]=in_plan &
						and object.cve_carrera[lo_cont_2]=in_carrera &
						and (object.periodo_ing_min[lo_cont_2] <= li_periodo_ing and object.anio_ing_min[lo_cont_2] <= li_anio_ing) &
						and (object.periodo_ing_max[lo_cont_2] >= li_periodo_ing and object.anio_ing_max[lo_cont_2] >= li_anio_ing)))
		...
		if isnull(object.puntaje_min[lo_cont_2]) then
			//object.plan_estudios_puntaje_min[lo_cont_2]=0
			object.puntaje_min[lo_cont_2]=0
		end if
...
//		else/*dw_prom_cred_sem.object.academicos_creditos_cursados[lo_cont_1]>=object.plan_estudios_cred_puntaje[lo_cont_2]*/
			...
			st_status_2.text='Terminada la Carga ' + STRING(lo_cuenta) + "-" + STRING(in_carrera) + "-" + STRING(in_plan) 
			
			IF lo_cuenta = 191254 THEN 
				lo_cuenta = lo_cuenta
			END IF 	
...
//						round(object.plan_estudios_puntaje_min[lo_cont_2],1) then
				if round(dw_prom_cred_sem.object.academicos_promedio[lo_cont_1],1)>=&
						round(object.puntaje_min[lo_cont_2],1) then						
...
integer height = 432
integer taborder = 40
string dataobject = "d_prom_cred_sem_per_ing"
boolean vscrollbar = true
boolean livescroll = true
end type
...
dw_cred_75_ss.retrieve(gs_tipo_periodo) 
...
MODIFICACIÓN:
*/
dw_cortes.retrieve('?', gs_tipo_periodo) 
