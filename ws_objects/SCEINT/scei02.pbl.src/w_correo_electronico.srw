$PBExportHeader$w_correo_electronico.srw
forward
global type w_correo_electronico from w_ancestral
end type
type mle_note from multilineedit within w_correo_electronico
end type
type dw_inbox from datawindow within w_correo_electronico
end type
type dw_cons_bolsa from datawindow within w_correo_electronico
end type
type dw_horarios from datawindow within w_correo_electronico
end type
type dw_mat_preinsc from uo_dw_captura within w_correo_electronico
end type
type st_atendidos from statictext within w_correo_electronico
end type
type dw_materias_inscritas from datawindow within w_correo_electronico
end type
type dw_lugar_preinsc from datawindow within w_correo_electronico
end type
end forward
...
global w_correo_electronico w_correo_electronico
...
public subroutine wf_escribe_bitacora (string as_error, long al_cuenta)
end prototypes
...
string ls_nombre_archivo = gs_path+"err_serv.txt"
string ls_
integer li_file_number, li_res_close
string ls_respuesta
boolean lb_con_sce, lb_con_scob, lb_con_sfeb
integer li_rcon_sce, li_rcon_scob, li_rcon_sfeb, li_ejecucion_pgp
string ls_firma_txt_completo, ls_firma_pgp_completo, ls_pif_completo, ls_pgp_completo, ls_run
string ls_err_text_sce, ls_err_text_scob, ls_err_text_sfeb, ls_desconeccion, ls_desconeccion2

uo_periodo_servicios luo_periodo_servicios
luo_periodo_servicios = CREATE uo_periodo_servicios

ls_error_file= ""
ls_desconeccion = "ct_cmd_alloc(): user api layer: external error: The connection has been marked dead."
ls_desconeccion2 = "ct_results(): network packet layer: internal net library error: Net-Library operation terminated due to disconnect"
lb_con_sce = gtr_sce.of_IsConnected()
ls_err_text_sce= gtr_sce.SqlErrText
if NOT lb_con_sce or ls_err_text_sce=ls_desconeccion or ls_err_text_sce=ls_desconeccion2 then
	DESTROY gtr_sce
	if conecta_bd_n_tr(gtr_sce,gs_sce,gs_usuario,gs_password)<>1 then
		MessageBox("Reconeccion Fallida", "El usuario no se pudo conectar a SCE", StopSign!)
		li_rcon_sce = -1
		ls_error_file = ls_error_file+ " CONECCION FALLIDA SCE "
	end if
end if

lb_con_scob = gtr_scob.of_IsConnected()
ls_err_text_scob= gtr_scob.SqlErrText
if NOT lb_con_scob or ls_err_text_scob=ls_desconeccion or ls_err_text_sce=ls_desconeccion2 then
	DESTROY gtr_scob
	if conecta_bd_n_tr(gtr_scob,gs_scob,gs_usuario,gs_password)<>1 then
		MessageBox("Reconeccion Fallida", "El usuario no se pudo conectar a SCOB", StopSign!)
		li_rcon_scob = -1
		ls_error_file = ls_error_file+ " CONECCION FALLIDA SCOB "
	end if
end if

lb_con_sfeb = gtr_sfeb.of_IsConnected()
ls_err_text_sfeb= gtr_sfeb.SqlErrText
if NOT lb_con_sfeb or ls_err_text_sfeb=ls_desconeccion or ls_err_text_sce=ls_desconeccion2 then
	DESTROY gtr_sfeb
	if conecta_bd_n_tr(gtr_sfeb,gs_sfeb,gs_usuario,gs_password)<>1 then
		MessageBox("Reconeccion Fallida", "El usuario no se pudo conectar a SFEB", StopSign!)
		li_rcon_sfeb = -1
		ls_error_file = ls_error_file+ " CONECCION FALLIDA SFEB "
	end if
end if

if li_rcon_sce = -1 or li_rcon_scob = -1 or li_rcon_sfeb = -1 then
...
if linea="" then
	...
	gi_periodo = luo_periodo_servicios.f_recupera_id_periodo_x_desc(gtr_sce, ls_periodo)
...
//	END CHOOSE
	...
	if obten_digito(long(ls_cuenta)) <> is_digito or gtr_sce.SqlCode=100 then
		li_tramite=0
...
//Grabar en un archivo
			mm_respuesta.Subject = 'Error de Comunicacion'
			regreso = 0			
		   ...
		   MessageBox("Error: Problemas al revisar si existe la cuenta",&
			           "Es necesario reiniciar la aplicacion~n"+&
						  "Revise la conexion a la base controlescolar_bd", StopSign!)
      ...
      ls_nip_plano = ls_nip_1
		...
		if ls_nip_1<>ls_nip_2 then
			...
			CASE 'kardex'
				li_tramite=1
				//No exige el pago de ingreso
				if (revisaadeudosinscripcion(il_cuenta,ls_respuesta,0)<>0) then
			...
			CASE 'preinscripcion'
				...
				//Exige el pago de ingreso
				elseif (revisaadeudospreinscripcion(il_cuenta,ls_respuesta,1)=0) then
					if ls_respuesta <> "" then is_respuesta = ls_respuesta
					dw_mat_preinsc.event agrega(linea)
				else
					is_respuesta = ls_respuesta
				end if
				mm_respuesta.Subject = 'Resultado de tu Solicitud de Preinscripcion'
			CASE 'solmat'
				li_tramite=3
				//No exige el pago de ingreso
				if (revisaadeudosinscripcion(il_cuenta,ls_respuesta,0)<>0) then
...
if li_tramite<>-1 then
	...
	if li_tramite = 2 then
		ls_firma_txt_completo = gs_path + "firma.txt"
		li_File_respuesta = FileOpen(ls_firma_txt_completo,LineMode!, Write!, LockWrite!, Replace!)
		...
		ls_firma_pgp_completo =  gs_path + "firma.pgp"
		if FileExists (ls_firma_pgp_completo) then
			FileDelete (ls_firma_pgp_completo)
		end if
		ls_pif_completo = gs_path + "pgps.pif"
//		Run("pgps.pif firma.txt -z "+password)
		ls_run = ls_pif_completo+ " "+ ls_firma_txt_completo+ " -z "
		li_ejecucion_pgp= Run(ls_run+password)
		if li_ejecucion_pgp = -1 then
			MessageBox("Ejecucion Fallida", "No se pudo correr el PGP", StopSign!)
			ls_error_file= " EJECUCION PGP "
			ldttm_fecha_servidor = fecha_servidor(gtr_sce)
			ls_fecha_servidor = string(ldttm_fecha_servidor)
			li_file_number = FileOpen(ls_nombre_archivo, LineMode!, Write!, LockReadWrite!, Append!)
			if li_file_number<>-1 then
		...
		ls_pgp_completo = gs_path+ "firma.pgp"
		DO UNTIL FileExists (ls_pgp_completo)
		LOOP
	
//		mm_respuesta.AttachmentFile[1].Pathname="firma.pgp"
		mm_respuesta.AttachmentFile[1].Pathname=ls_pgp_completo
	end if
	
	mrc_estado = ms_sesion.mailSend ( mm_respuesta )
	IF mrc_estado <> mailReturnSuccess! THEN
		regreso=0
		wf_escribe_bitacora("MailSendFail",il_cuenta)
	else
		regreso=-1
	END IF
else
	regreso=0
	wf_escribe_bitacora("Tramite desconocido",il_cuenta)
end if
...
lugar=string(dw_lugar_preinsc.rowcount(),"00")
...
public subroutine wf_escribe_bitacora (string as_error, long al_cuenta);	string ls_nombre_archivo, ls_cuenta_file, ls_fecha_servidor
	datetime ldttm_fecha_servidor
	int li_file_number, li_res_close
	ls_nombre_archivo = gs_path + "err_serv.txt" 
	ls_cuenta_file = string(al_cuenta)
	ldttm_fecha_servidor = fecha_servidor(gtr_sce)
	ls_fecha_servidor = string(ldttm_fecha_servidor)
	li_file_number = FileOpen(ls_nombre_archivo, LineMode!, Write!, LockReadWrite!, Append!)
	if li_file_number<>-1 then
		FileWrite(li_file_number, ls_cuenta_file +" "+as_error +" "+ls_fecha_servidor)
		li_res_close = FileClose(li_file_number)
	end if
...
is_mail_sce=ProfileString (gs_startupfile, gs_datos, "EMAIL_sce","")
...
FOR lo_n = 1 to UpperBound(ms_sesion.MessageID[])
	...
	if event canaliza(inc(mm_mensaje.NoteText))=-1 then
		mrc_estado = ms_sesion.mailDeleteMessage(ms_sesion.MessageID[lo_n])
		IF mrc_estado <> mailReturnSuccess! THEN
			wf_escribe_bitacora("MailDeleteFail",il_cuenta)
		END IF
	else
		...
		mrc_estado = ms_sesion.mailSend ( mm_respuesta )
			IF mrc_estado = mailReturnSuccess! THEN
				mrc_estado = ms_sesion.mailDeleteMessage(ms_sesion.MessageID[lo_n])
				IF mrc_estado <> mailReturnSuccess! THEN
					wf_escribe_bitacora("Reenvio MailDeleteFail",il_cuenta)
				END IF	
			END IF
	end if
NEXT
...
on w_correo_electronico.create
int iCurrent
call super::create
...
on w_correo_electronico.destroy
call super::destroy
...
type p_uia from w_ancestral`p_uia within w_correo_electronico
end type
...
*/
string ls_clave[10]
string ls_grupo[10], ls_accion[10], ls_nivel, ls_diagnostico
...
is_respuesta=is_respuesta+"Materias Preinscritas ~n"
is_respuesta=is_respuesta+" El diagnostico revisa si existe la materia, si existe el grupo, si pertenece a tu plan de ~n"
is_respuesta=is_respuesta+" estudios, si ya la has cursado y en caso de ser de integración, si ya la puedes cursar. "

is_respuesta += " " + luo_periodo_servicios.f_recupera_desc_periodo(gtr_sce, gi_periodo)
...
FOR ll_renglon=1 TO rowcount()
	if gi_periodo=object.periodo[ll_renglon] and gi_anio=object.anio[ll_renglon] then
		if object.status[ll_renglon]= 63 then
			ls_diagnostico ="OK"
		else
			ls_diagnostico = object.diag[ll_renglon]
		end if
		is_respuesta=is_respuesta+&
