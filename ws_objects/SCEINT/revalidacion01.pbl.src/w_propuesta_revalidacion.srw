$PBExportHeader$w_propuesta_revalidacion.srw
forward
global type w_propuesta_revalidacion from window
end type
type st_2 from statictext within w_propuesta_revalidacion
end type
type st_1 from statictext within w_propuesta_revalidacion
end type
type uo_coordinacion from uo_coordinaciones within w_propuesta_revalidacion
end type
type dw_universidad from datawindow within w_propuesta_revalidacion
end type
type cb_1 from commandbutton within w_propuesta_revalidacion
end type
type dw_errores_perten_prerreq from datawindow within w_propuesta_revalidacion
end type
type cb_revision from commandbutton within w_propuesta_revalidacion
end type
type dw_equiv_mat_revalidacion from uo_dw_captura_base within w_propuesta_revalidacion
end type
type uo_1 from uo_nombre_aspirante_reval within w_propuesta_revalidacion
end type
end forward
...
string title = "Propuesta de Revalidacion"
...
global w_propuesta_revalidacion w_propuesta_revalidacion

type variables
integer ii_itemchanged
boolean ib_imprime_propuesta=false, ib_imprime_errores=false
end variables
...
if not isnumber(ls_folio) then
	MessageBox("Folio inválido",&
				"Favor de seleccionar un folio válido",StopSign!)
	return -1
end if
...
ll_row_coord= this.uo_coordinacion.dw_coordinaciones.GetRow()
li_coordinacion= this.uo_coordinacion.dw_coordinaciones.GetItemNumber(ll_row_coord, "cve_coordinacion")
...
on w_propuesta_revalidacion.create
...
on w_propuesta_revalidacion.destroy
...
string text = "Imprime Solicitud de Equivalencias"
boolean focusrectangle = false
end type

event clicked;cb_revision.event clicked()
end event
...
integer li_res_generacion

li_res_generacion = wf_genera_propuesta(false)

if li_res_generacion = -1 then
	MessageBox("Error de Generación", "No se puede imprimir la propuesta",StopSign!)
else
	ll_num_errores = dw_errores_perten_prerreq.RowCount()
	if ll_num_errores > 0 then
		dw_errores_perten_prerreq.Object.DataWindow.Zoom = 100
...
event clicked;integer li_res_generacion

li_res_generacion = wf_genera_propuesta(true)
...
NewMenu.PopMenu(w_propuesta_revalidacion.PointerX(), w_propuesta_revalidacion.PointerY())
