$PBExportHeader$w_solicitud_materias.srw
$PBExportComments$Reporte de solicitudes de materias. Juan Campos Sánchez. Abril-1998.
forward
global type w_solicitud_materias from window
end type
type rb_avisopos_preinsc from radiobutton within w_solicitud_materias
end type
type rb_avisolic_preinsc from radiobutton within w_solicitud_materias
end type
type cbx_datos_generales from checkbox within w_solicitud_materias
end type
type uo_1 from uo_per_ani within w_solicitud_materias
end type
type gb_5 from groupbox within w_solicitud_materias
end type
type cb_actualiza_avisos from commandbutton within w_solicitud_materias
end type
type tab_1 from tab within w_solicitud_materias
end type
...
type rb_avisopos from radiobutton within w_solicitud_materias
end type
type rb_avisolic from radiobutton within w_solicitud_materias
end type
type dw_avisos from datawindow within w_solicitud_materias
end type
type uo_alumno from uo_nombre_alu_foto within w_solicitud_materias
end type
end forward
...
global w_solicitud_materias w_solicitud_materias
...
// Roberto Novoa Ramírez.		Abr/2016.
//----------------------------------------------
// Modificación a la funcionalidad de periodos.
	...
	For i = 1 To dw_adeuda_cobranzas.RowCount() 
		...
		if	gb_liberacion_vigente then
			li_periodo = dw_adeuda_cobranzas.GetItemNumber(i,"saldos_periodo")
			 ls_periodo=iuo_periodo_servicios.f_recupera_descripcion( li_periodo, "C")
...
public subroutine escribe_encabezado_comp (integer ai_hoja);// Roberto Novoa Ramírez.		Abr/2016
//---------------------------------------------
// Modificaciones a la funcionalidad del periodo escolar
...
ls_linea += iuo_periodo_servicios.f_recupera_descripcion( ids_comprobante.GetItemNumber(1,"mat_inscritas_periodo"), "L")
...
// 	End IF
	...
	If insc_sem_ant = 0 Then 
		...
		FileWrite(NumArchivo,"  NO ESTUVISTE INSCRITO EL " + UPPER(ls_desc_periodo) + " ANTERIOR.")
		
//		if gs_tipo_periodo='S' then
//			FileWrite(NumArchivo,"  NO ESTUVISTE INSCRITO EL SEMESTRE ANTERIOR.")
//		else
//			FileWrite(NumArchivo,"  NO ESTUVISTE INSCRITO EL TRIMESTRE ANTERIOR.")
//		end if			
...
// Roberto Novoa Ramírez.		Abril/2016.
...
on w_solicitud_materias.create
...
on w_solicitud_materias.destroy
...
// Víctor Manuel Iniestra Álvarez.		Octubre-1998. 
integer i,cont
string ls_control_correcto, ls_tipo, ls_desc_periodo
...
ls_tipo = luo_periodo_servicios.f_recupera_tipo_periodo(gtr_sce, gi_periodo)
...
ls_desc_periodo = luo_periodo_tipo_servicios.f_obten_desc_periodo_msg(ls_tipo)
...
if dw_materias_abiertas.retrieve(gi_periodo,gi_anio)=0 then
	messagebox("No hay materias abiertas","para este " + LOWER(ls_desc_periodo) + ", verificalo.")
	return
end if
...
String ls_mensajecomprobante, ls_control_correcto, ls_nombre_datastore, ls_tipo, ls_desc_periodo
...
long ll_pos_ruta, ll_num_archivos = 0
...
if dw_materias_abiertas.retrieve(gi_periodo,gi_anio)=0 then
	messagebox("No hay materias abiertas","para este " + LOWER(ls_desc_periodo) +", verificalo.", StopSign!)
	return
end if
		...
		For Cont = 1 To dw_cuentas.RowCount()
         Cuenta = dw_cuentas.GetItemNumber(Cont,"cuenta") 	
			...
			Select 	cve_carrera, cve_plan, cve_subsistema,nivel,promedio
				Into	:Carrera,:Plan,:Subsistema,:Nivel,:Promedio
				From	academicos 
				Where	cuenta = :Cuenta /*And nivel = :Nivel    SE elimina el filtro de nivel. Una cuenta tiene un academico */ 
			Using gtr_sce;
			If gtr_sce.sqlcode = 0 Then
...
int CarreraIni, CarreraFin
Char  	Texto[]
string ls_cve_criterio, ls_nombre_criterio, ls_tipo, ls_desc_periodo
