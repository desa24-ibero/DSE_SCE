$PBExportHeader$w_solicitud_materias_cuenta.srw
$PBExportComments$Reporte de solicitudes de materias. Juan Campos Sánchez. Abril-1998.
forward
global type w_solicitud_materias_cuenta from window
end type
type rb_avisopos_preinsc from radiobutton within w_solicitud_materias_cuenta
end type
type rb_avisolic_preinsc from radiobutton within w_solicitud_materias_cuenta
end type
type cbx_datos_generales from checkbox within w_solicitud_materias_cuenta
end type
type uo_1 from uo_per_ani within w_solicitud_materias_cuenta
end type
type gb_5 from groupbox within w_solicitud_materias_cuenta
end type
type cb_actualiza_avisos from commandbutton within w_solicitud_materias_cuenta
end type
type tab_1 from tab within w_solicitud_materias_cuenta
end type
...
type cb_genera_solicitud_por_cuenta from commandbutton within tabpage_2
end type
...
type tabpage_2 from userobject within tab_1
cb_genera_solicitud_por_cuenta cb_genera_solicitud_por_cuenta
st_ala st_ala
...
type rb_avisopos from radiobutton within w_solicitud_materias_cuenta
end type
type rb_avisolic from radiobutton within w_solicitud_materias_cuenta
end type
type dw_avisos from datawindow within w_solicitud_materias_cuenta
end type
type uo_alumno from uo_nombre_alu_foto within w_solicitud_materias_cuenta
end type
end forward
...
rb_avisopos_preinsc rb_avisopos_preinsc
rb_avisolic_preinsc rb_avisolic_preinsc
...
global w_solicitud_materias_cuenta w_solicitud_materias_cuenta
...
public subroutine escribe_sugiere_integ_2004 ()
public subroutine escribe_avisos (string aviso)
public subroutine escribe_avisoscontrolescolar ()
public subroutine despliega_materias (integer area, integer k, integer ai_cve_plan)
public subroutine escribe_encabezado1 ()
public subroutine escribe_adeudos_cobranzas ()
public function integer escribe_comprobante (string as_mensaje, integer ai_lineas)
...
public subroutine escribe_sugiere_integ_2004 ();//escribe_sugiere_integ_2004
// Escribe en el archivo de texto las materias que puede inscribir de integración
// Juan Campos Sánchez.		marzo 2005
...
Agrupa[1]=False; Agrupa[2]=False; Agrupa[3]=False
Agrupa[4]=False; Agrupa[5]=False; Agrupa[6]=False
Descrip[1]="  REFLEXION UNIVERSITARIA TEMA FUNDAMENTAL"
Descrip[2]="  REFLEXION UNIVERSITARIA TEMA I"	
Descrip[3]="  REFLEXION UNIVERSITARIA TEMA II"
Descrip[4]="  REFLEXION UNIVERSITARIA TEMA III"	
Descrip[5]="  REFLEXION UNIVERSITARIA TEMA IV"
Descrip[6]="  MATERIAS ADICIONALES"
...
//								"(   )"+Fill(' ',2)+"-----"+Fill(' ',2) + &
//								"----------------------------------------------")
			FileWrite(NumArchivo," 20483" +Fill(' ',2)+ &
							"(   )"+Fill(' ',2)+"RU101"+Fill(' ',2) + &
							"PERSONA Y HUMANISMO")
			ContLinea=ContLinea+1
			FileWrite(NumArchivo,"")
...
FileWrite(NumArchivo,"")
ContLinea=Contlinea + ContAvisos + 2
end subroutine
...
Integer	insc_sem_ant,cve_flag_promedio,baja_3_reprob,baja_4_insc, &
			baja_documentos,cve_flag_prerreq_ingles,cve_flag_serv_social, &
  			cve_flag_biblioteca,cve_flag_diapositeca,Ind, li_preinscrito, li_baja_laboratorio
...
Select	insc_sem_ant,				cve_flag_promedio,		baja_3_reprob,
			...
			cve_flag_diapositeca,	isnull(baja_laboratorio,0)
...
If gtr_sce.sqlcode = 0 Then
	...
	If insc_sem_ant = 0 or 					cve_flag_promedio = 1 or 			&
   	cve_flag_promedio = 2 or			cve_flag_promedio = 3 or 			& 
		...
		li_preinscrito = 0 or li_baja_laboratorio = 1 Then
		If ContLinea >= 55 Then Escribe_Encabezado()
...
// 	End IF
	...
	If insc_sem_ant = 0 Then 
		If ContLinea >= 60 Then Escribe_Encabezado()
...
//*-			FileWrite(NumArchivo,"  UNA SEGUNDA AMONESTACION CONSECUTIVA CAUSA BAJA.")	    
			   FileWrite(NumArchivo,"  ESTAS AMONESTADO , SEGÚN EL REGLAMENTO DE TRÁMITES ESCOLARES ARTÍCULO 3.33 ")	    
			   FileWrite(NumArchivo,"  UNA SEGUNDA AMONESTACIÓN CONSECUTIVA CAUSA BAJA.")	    
			FileWrite(NumArchivo,"")
			ContLinea = ContLinea + 3
		End If			 
...
//*-			FileWrite(NumArchivo,"  REGLAMENTO DE PUNTAJE DE CALIDAD.")		
 			   FileWrite(NumArchivo,"  ESTAS DADO DE BAJA, POR ACUMULAR 2 AMONESTACIONES CONSECUTIVAS, CONSULTA EL")
			   FileWrite(NumArchivo,"  REGLAMENTO DE TRÁMITES ESCOLARES.")		
			FileWrite(NumArchivo,"") 
			ContLinea = ContLinea + 3
		End If			 
//*-Eliminar mensaje
...
//*-		FileWrite(NumArchivo,"  EVALUACIONES , ARTICULO 3.5.")
		   FileWrite(NumArchivo,"  ESTAS DADO DE BAJA POR REPROBAR 3 VECES LA MISMA MATERIA. REGLAMENTO DE ")
		   FileWrite(NumArchivo,"  TRÁMITES ESCOLARES, ARTICULO 4.2a.")
		FileWrite(NumArchivo,"") 	
		ContLinea = ContLinea + 3
	End IF
...
//*-		FileWrite(NumArchivo,"  ARTICULO 3.5 (B) DEL REGLAMENTO.")     		
		FileWrite(NumArchivo,"  ESTAS DADO DE BAJA, POR TENER 4 INSCRIPCIONES A UNA MISMA MATERIA. ")
		FileWrite(NumArchivo,"  REGLAMENTO DE TRÁMITES ESCOLARES, ARTICULO 4.2b.")     		
		FileWrite(NumArchivo,"")
		ContLinea = ContLinea + 3
	End IF
...
//*-			FileWrite(NumArchivo,"  ADEUDA PRERREQUISITO DE INGLES")
			FileWrite(NumArchivo,"  ADEUDAS EL PRERREQUISITO DE INGLÉS")
			FileWrite(NumArchivo,"")
			ContLinea = ContLinea + 2
 		End If
...
//*-			FileWrite(NumArchivo,"  DE 30 CREDITOS.")
   			FileWrite(NumArchivo,"  SI NO ACREDITAS EL PRERREQUISITO DE INGLÉS, EN TU INSCRIPCIÓN AL ")
   			FileWrite(NumArchivo,"  CUARTO SEMESTRE TU CARGA ACADÉMICA SE LIMITARÁ A 48 CRÉDITOS.")			
			FileWrite(NumArchivo,"")
			ContLinea = ContLinea + 3
 		End if
...
//*-			FileWrite(NumArchivo,"  QUE PUEDES INSCRIBIR EN UN SEMESTRE.")
		   FileWrite(NumArchivo,"  YA PUEDES CURSAR TU SERVICIO SOCIAL. TE RECUERDO QUE LOS 16 CRÉDITOS ")
		   FileWrite(NumArchivo,"  DE SERVICIO SOCIAL TAMBIEN SE TOMAN EN CUENTA PARA EL MÁXIMO  DE ")
		   FileWrite(NumArchivo,"  CRÉDITOS QUE PUEDES INSCRIBIR EN UN SEMESTRE.")
			FileWrite(NumArchivo,"")
...
//*-		FileWrite(NumArchivo,"  AMONESTADO POR ADEUDO EN DIAPOSITECA")		
		FileWrite(NumArchivo,"  ADADO DE BAJA POR ADEUDO EN DIAPOSITECA")		
		FileWrite(NumArchivo,"")
		ContLinea = ContLinea + 2
 	End if
...
//*-		FileWrite(NumArchivo,"  SUSPENDIDO POR ADEUDO EN DIAPOSITECA")
	...
	If li_baja_laboratorio = 1 Then	
		If ContLinea >= 60 Then Escribe_Encabezado()
		FileWrite(NumArchivo,"  BLOQUEADO POR ADEUDO DE MATERIAL DE LABORATORIO")
		FileWrite(NumArchivo,"")
		ContLinea = ContLinea + 2
 	End IF
End If	
...
public subroutine despliega_materias (integer area, integer k, integer ai_cve_plan);integer ll_cont_1,cont_materias
string ls_textin
string ls_sigla, ls_sigla_2004
...
if nivel = "P" then
	...
	if (lds_sigla_por_carrera.Retrieve(carrera) = 1) then
		...
		ls_sigla_2004 = lds_sigla_por_carrera.GetItemString(1,"sigla_2004")
	else
		ls_sigla = ""
		ls_sigla_2004 = ""
	end if
...
FOR ll_cont_1=1 TO dw_materiasxcursar.rowcount()
	if genera_individual=0 then
		...
		if ai_cve_plan =6 then
			//if nivel = "L" OR k <> 3 OR Mid(dw_materiasxcursar.object.sigla[ll_cont_1],1,3) = ls_sigla_2004 then
			if nivel <> "P" OR k <> 3 OR Mid(dw_materiasxcursar.object.sigla[ll_cont_1],1,3) = ls_sigla_2004 then
				ll_cve_mat_actual = dw_materiasxcursar.object.cve_mat[ll_cont_1]
				if ll_cve_mat_actual <10000 then
					FileWrite(NumArchivo,+"   "+string(ll_cve_mat_actual)+'  (   )  '+&
					...
					//Pone un espacio menos al principio
					FileWrite(NumArchivo,+"  "+string(ll_cve_mat_actual)+'  (   )  '+&
						string(dw_materiasxcursar.object.sigla[ll_cont_1])+'  '+ls_textin+'  '+&
						string(dw_materiasxcursar.object.creditos[ll_cont_1],"###0.00")+'  '+&
						string(dw_materiasxcursar.object.semestre_ideal[ll_cont_1]))
			...
			//if nivel = "L" OR k <> 3 OR Mid(dw_materiasxcursar.object.sigla_anterior[ll_cont_1],1,3) = ls_sigla then
			if nivel <> "P" OR k <> 3 OR Mid(dw_materiasxcursar.object.sigla_anterior[ll_cont_1],1,3) = ls_sigla then	
				ll_cve_mat_actual = dw_materiasxcursar.object.cve_mat[ll_cont_1]
				if ll_cve_mat_actual <10000 then
					FileWrite(NumArchivo,+"  "+string(ll_cve_mat_actual)+'  (   )  '+&
						string(dw_materiasxcursar.object.sigla_anterior[ll_cont_1])+'  '+ls_textin+'  '+&
						string(dw_materiasxcursar.object.creditos[ll_cont_1],"###0.00")+'  '+&
						string(dw_materiasxcursar.object.semestre_ideal[ll_cont_1]))
				else
					//Pone un espacio menos al principio
		...
		if ai_cve_plan =6 then		  
	   	...
	   	SugiereMatReinsc.SetItem(cont_materias,4,dw_materiasxcursar.object.sigla_anterior[ll_cont_1])
		end if
...
Integer	i,j, li_periodo
String	DesConcepto,Importe,Total, ls_periodo
...
//	If ContLinea >= 48 Then Escribe_Encabezado()
	If ContLinea >= 52 Then Escribe_Encabezado()
   ...
   FileWrite(NumArchivo,"  A VERIFICAR TU PAGO.")
	...
	For i = 1 To dw_adeuda_cobranzas.RowCount() 
		...
		End if	
		If ContLinea >= 58 Then Escribe_Encabezado()
		
		if	gb_liberacion_vigente then
			li_periodo = dw_adeuda_cobranzas.GetItemNumber(i,"saldos_periodo")
			
			ls_periodo = luo_periodo_servicios.f_recupera_desc_corta_periodo(gtr_sce, li_periodo)
...
//			END CHOOSE
			...
			ls_periodo = dw_adeuda_cobranzas.GetItemString(i,"saldos_periodo")
		end if
		FileWrite(NumArchivo,Fill(" ",4)+ls_periodo + &
...
public function integer escribe_comprobante (string as_mensaje, integer ai_lineas);int li_i, li_res, li_numlinea =0, li_hoja, li_lineas_mensaje, li_lineas_encabezado = 13
long ll_cve_mat
string ls_linea
//ic_crlf[1] = char(13)
//ic_crlf[2] = char(10)
...
if ( li_res >= 0 ) then
	if ( li_res > 0 ) then
		li_numlinea = 0
		li_lineas_mensaje = ai_lineas
//		ai_lineas = 46 - ai_lineas - 4
		ai_lineas = 47 - ai_lineas - 2
		ll_cve_mat = 0
		li_hoja = 1
		for li_i = 1 to li_res
			...
			if ll_cve_mat <> ids_comprobante.GetItemNumber(li_i,"mat_inscritas_cve_mat") then
				...
				FileWrite(NumArchivo,ls_linea)
				li_numlinea++
				ls_linea = "  "+AjustaPalabra(string(ids_comprobante.GetItemNumber(li_i,"mat_inscritas_cve_mat")),6)+" "+&
			...
			if isnull(ids_comprobante.GetItemNumber(li_i,"horario_hora_inicio")) then
				...
				ls_linea +=	"            "+AjustaPalabra(ids_comprobante.GetItemString(li_i,"horario_cve_salon"),10)+" "+&
		...
		li_numlinea++
		FileWrite(NumArchivo,as_mensaje)
//		li_numlinea= li_numlinea+li_lineas_encabezado+li_lineas_mensaje
	end if
else
...
long ll_row_actual, ll_row_final, ll_rows_por_pagina = 60 
...
ls_linea += " " + UPPER(luo_periodo_servicios.f_recupera_desc_periodo(gtr_sce, ids_comprobante.GetItemNumber(1,"mat_inscritas_periodo"))) + " " 
...
ls_linea +=		" "+string(il_folio,"000000")+ic_crlf+space(54)
ls_linea +=		"FECHA:     "+string(Today( ), "dd/mm/yyyy")+ic_crlf+space(69)+"HOJA "+string(ai_hoja)
ls_linea +=		ic_crlf+ic_crlf+ic_crlf+ic_crlf
ls_linea +=		"  CUENTA: "+string(cuenta)+"-"+obten_digito(cuenta)+ic_crlf+"  NOMBRE: "
ls_linea +=		ids_comprobante.GetItemString(1,"alumnos_nombre_completo")+ic_crlf+"  CARRERA: "
ls_linea +=		ids_comprobante.GetItemString(1,"carreras_carrera")+" PLAN: "
ls_linea +=		ids_comprobante.GetItemString(1,"nombre_plan_nombre_plan")+ic_crlf
ls_linea +=		"                        RELACION DE MATERIAS INSCRITAS                    "+ic_crlf+ic_crlf
ls_linea +=		"  CLAVE GPO MATERIA                                            CRED  HORAS"+ic_crlf
ls_linea +=		"            SALON  DIA        HORARIO"
FileWrite(NumArchivo,ls_linea)
end subroutine
...
if genera_individual=0 then
	...
	IF ContHoja>=1 AND ContLinea <> 0 THEN
		ll_row_final = ll_rows_por_pagina - ContLinea
		FOR ll_row_actual=1 TO ll_row_final
			FileWrite(NumArchivo,"")
		NEXT
	END IF
...
// 21/03/2007 Menos lineas de encabezado	
//	FileWrite(NumArchivo,ic_crlf+ic_crlf+ic_crlf+ic_crlf+ic_crlf)
//	FileWrite(NumArchivo,ic_crlf+ic_crlf)
	FileWrite(NumArchivo,"")
	FileWrite(NumArchivo,"")
	...
	FileWrite(NumArchivo,"--------------------------------------------------------------------------------")
	FileWrite(NumArchivo,"")
	//Aqui se consideran los 3 de la parte superior	ic_crlf
...
//	if conecta_bd_n_tr(gtr_scob,gs_scob,"sie","pruebas")<>1 then
...
on w_solicitud_materias_cuenta.create
this.rb_avisopos_preinsc=create rb_avisopos_preinsc
this.rb_avisolic_preinsc=create rb_avisolic_preinsc
...
this.Control[]={this.rb_avisopos_preinsc,&
this.rb_avisolic_preinsc,&
this.cbx_datos_generales,&
this.uo_1,&
this.gb_5,&
...
on w_solicitud_materias_cuenta.destroy
destroy(this.rb_avisopos_preinsc)
destroy(this.rb_avisolic_preinsc)
...
string facename = "Arial"
long backcolor = 78164112
string text = "Licenciatura Preinsc."
end type
	...
	For i=1 to lb_cuentas.TotalItems( )
		...
		If gtr_sce.sqlcode = 0 Then							 			
			...
			IF iuo_administrador_liberacion.of_liberacion_vigente("SIT") THEN
				IF iuo_administrador_liberacion.of_obten_control_correcto("SIT","scei","dw_adeuda_cobranzas", "datawindow", ls_control_correcto) THEN
					dw_sol_mat.Object.adeuda_cobranzas.DataObject = ls_control_correcto 
					dw_sol_mat.Getchild('adeuda_cobranzas',AdeudaCobranzas)
				END IF
			END IF
         ...
         llena_Avisos_Control_Esc(Cuenta,Puede_Integ,TemaF1,TemaF2,Tema1,Tema2,Tema3,Tema4,Nivel,AvisosControlEscolar,ServSoc,Ingles)
			...
			agrega_materiasxcursar_cuenta(nivel,dw_materiasxcursar,dw_historico,dw_materias_abiertas,cuenta,contlinea,numarchivo,plan,carrera,subsistema,dw_plan_estud,servsoc)
...
on tabpage_2.create
this.cb_genera_solicitud_por_cuenta=create cb_genera_solicitud_por_cuenta
...
this.Control[]={this.cb_genera_solicitud_por_cuenta,&
this.st_ala,&
this.st_delacarrera,&
this.em_nombrefin,&
this.em_nombreini,&
...
destroy(this.cb_genera_solicitud_por_cuenta)
destroy(this.st_ala)
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "Solicitud por Cuenta"
end type

event clicked;// REPORTE GENERAL DE SOLICITUD DE MATERIAS PARA REINSCRIPCIÓN,
// EL CUAL GENERARA SOLICITUDES INDIVIDUALES POR ALUMNO.
// Antonio Pica Ruiz		Marzo-2007

SetPointer(HourGlass!)
DataStore dw_cuentas, lds_folios

String ls_mensajecomprobante, ls_control_correcto, ls_nombre_datastore
int li_lineasmenscomp = 0
long ll_num_folios=0, ll_num_mensajes=0
Integer	Cont,Exito
...
Char  	Texto[]
string ls_cve_criterio, ls_nombre_criterio
string ls_ruta_archivo, ls_nombre_archivo, ls_extension_archivo = ".txt"
long ll_pos_ruta, ll_num_archivos = 0, ll_lineas_comprobante, ll_lineas_encabezado_comp = 13
datetime lddtm_fecha_inicial, lddtm_fecha_final

lddtm_fecha_inicial = fecha_servidor (gtr_sce)

ls_cve_criterio = f_obten_criterio_solicitudes(gi_periodo,gi_anio) 

if ls_cve_criterio="" then
	messagebox("Criterio Inexistente","NO existe criterio de generación para el periodo elegido",StopSign!)
	return	
end if

ls_nombre_criterio = f_obten_nombre_criterio_sol(ls_cve_criterio) 



if dw_materias_abiertas.retrieve(gi_periodo,gi_anio)=0 then
	messagebox("No hay materias abiertas","para este semestre, verificalo.", StopSign!)
	return
end if
...
if rb_avisolic.checked then
	if ls_cve_criterio = "P" then
		messagebox("Criterio Inválido","El criterio definido es~n["+ls_nombre_criterio+"]",StopSign!)
		return			
	end if
	...
	Nivel='L'
elseif rb_avisopos.checked then
	if ls_cve_criterio = "P" then
		messagebox("Criterio Inválido","El criterio definido es~n["+ls_nombre_criterio+"]",StopSign!)
		return			
	end if
	...
	Nivel='P'
elseif rb_avisolic_preinsc.checked then
	if ls_cve_criterio = "T" then
		messagebox("Criterio Inválido","El criterio definido es~n["+ls_nombre_criterio+"]",StopSign!)
		return			
	end if
	dw_cuentas.DataObject = "d_cuentas_inscl_preinsc"
	dw_cuentas.Settransobject(gtr_sce)
	lds_folios.DataObject = "d_folios_lic_preinsc"
	lds_folios.Settransobject(gtr_sce)
	ids_mensaje.DataObject = "d_mensajel"
	rowcuentas = dw_cuentas.Retrieve(NombreIni,NombreFin)
	Nivel='L'
elseif rb_avisopos_preinsc.checked then
	if ls_cve_criterio = "T" then
		messagebox("Criterio Inválido","El criterio definido es~n["+ls_nombre_criterio+"]",StopSign!)
		return			
	end if
	dw_cuentas.DataObject = "d_cuentas_inscp_preinsc"
	dw_cuentas.Settransobject(gtr_sce)
	lds_folios.DataObject = "d_folios_pos_preinsc"
	lds_folios.Settransobject(gtr_sce)
	ids_mensaje.DataObject = "d_mensajep"
	rowcuentas = dw_cuentas.Retrieve(NombreIni,NombreFin,CarreraIni,CarreraFin)
	Nivel='P'
end if
ll_num_folios= lds_folios.Retrieve()
ids_mensaje.SetTransObject(gtr_sce)
ll_num_mensajes= ids_mensaje.Retrieve(gs_tipo_periodo)
...
ls_nombre_datastore = "dw_adeuda_cobranzas"
...
dw_adeuda_cobranzas.DataObject = ls_nombre_datastore
dw_adeuda_cobranzas.SetTransObject(gtr_scob)

/*Verifica el tamaño de los avisos.*/
if rb_avisolic.checked or rb_avisolic_preinsc.checked then
	dw_avisos.retrieve(0)
elseif rb_avisopos.checked or  rb_avisopos_preinsc.checked then
...
MessageBox("Elija un archivo",&
           "A continuación se le solicitará un nombre de archivo, en dicha carpeta se generarán todos los archivos de las solicitudes de materias",&
                 Information!)
If RowCuentas > 0 Then
	Exito = GetFileSaveName("Selecciona Archivo Dummy",Ruta,NombreArch,"TXT","Archivo de texto,*.TXT,")

//Una vez migrado a PB10 debería poder utilizarse esta sintaxis
//	Exito = GetFolder ( "Selecciona Directorio", Ruta )

	If Exito = 1 then
		
		ll_pos_ruta=f_obten_ultima_posicion(Ruta,"\") 
		ls_ruta_archivo =left(Ruta,ll_pos_ruta)
		...
		For Cont = 1 To dw_cuentas.RowCount()
         Cuenta = dw_cuentas.GetItemNumber(Cont,"cuenta") 	
			ls_nombre_archivo =ls_ruta_archivo + string(cuenta) + ls_extension_archivo
			NumArchivo = FileOpen(ls_nombre_archivo,LineMode!,Write!,LockWrite!,Replace!)
			Select 	cve_carrera, cve_plan, cve_subsistema,nivel,promedio
				Into	:Carrera,:Plan,:Subsistema,:Nivel,:Promedio
				From	academicos 
				Where	cuenta = :Cuenta /* Se elimina el filtro de nivel And nivel = :Nivel*/
			Using gtr_sce;
			If gtr_sce.sqlcode = 0 Then
				if cbx_datos_generales.checked = true then escribe_datos_generales()
				//st_3.text = string(Cont)
				ll_lineas_comprobante = escribe_comprobante(ls_mensajecomprobante,li_lineasmenscomp)
				If ll_lineas_comprobante > 0 Then
					ContLinea = ll_lineas_comprobante + li_lineasmenscomp + ll_lineas_encabezado_comp
				else
					ContLinea = 0
				End If
				ContHoja = 0
			...
			ll_num_archivos = ll_num_archivos +1
			il_folio ++
		Next	
	End If
Else
...
lddtm_fecha_final = fecha_servidor (gtr_sce)

MessageBox("Duración del Proceso", "Hora Inicial "+String(lddtm_fecha_inicial)+&
           "~n"+"Hora Final "+String(lddtm_fecha_final)+&
			  "~n"+"Número de Archivos Generados "+String(ll_num_archivos),Information!)
...
ll_num_folios= lds_folios.Retrieve()
ids_mensaje.SetTransObject(gtr_sce)
ll_num_mensajes= ids_mensaje.Retrieve()
ls_mensajecomprobante = convierte_avisos(ids_mensaje.GetItemString(1,"mensaje"))
...
IF RetriEve(0) = 0 Then
	Messagebox("No hay información para los avisos","Favor de verificar")
End if
end event
