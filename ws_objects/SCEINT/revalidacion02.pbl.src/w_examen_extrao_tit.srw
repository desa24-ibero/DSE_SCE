...
event inicia_proceso();int li_res
...
if (ds_academicos.Retrieve(il_cuenta)>0) then
	...
	if li_res = 1 then
		...
		if (ii_tipo = 2 AND li_res = 0) then
			li_res = MessageBox("Importante", "Para un examen extraordinario es necesario estar inscrito en este periodo escolar, ¿es la última materia del alumno?",Question!, YesNo!,2)
		...
		if ((ii_tipo = 2 AND li_res = 1) OR (ii_tipo = 6 AND li_res = 0)) then
			...
			if (ii_banderasalumno[b_bdo] = 0) then
				desconecta_bd_n_tr(gtr_scob);
				if (conecta_bd_n_tr(gtr_scob,gs_scob,gs_usuario,gs_password)=1) then
					li_res = tiene_adeudos_n_tr(il_cuenta,gtr_scob)
					desconecta_bd_n_tr(gtr_scob)
					if (li_res = 1) then
	...
	if li_continua = 1 then
		...
		choose case li_res
			...
			case 100
				li_continua++
			case 101
				if (MessageBox("Atencion","La materia es cursativa, ¿Desea continuar?",Question!,YesNo!,2) = 1)then 
					li_continua++
				end if
			case -1
				Messagebox("Error de Comunicación","Error con la consulta de Base Datos ExisteMateria. Favor de intentar nuevamente",None!)
