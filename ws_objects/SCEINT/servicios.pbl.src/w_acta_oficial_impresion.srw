...
type uo_periodo from uo_periodo_variable within w_acta_oficial_impresion
end type
...
// Se inicializa el objeto de servicios de periodos 
uo_periodo.f_genera_periodo( gs_tipo_periodo, "V", "L", "L", "N", gtr_sce) 
...
elseif rb_departamento.checked then
	...
	ll_rows = dw_estadisticas.Retrieve(ll_cve_coordinacion, li_periodo, li_anio, ll_cve_firma)
elseif rb_acta_nivel.checked then
	MessageBox("Generando Reporte", "Por Acta y Nivel",Information!)
	ll_rows = dw_estadisticas.Retrieve(ll_no_acta, ls_nivel)	
elseif rb_todas_nivel.checked then
	MessageBox("Generando Reporte", "Todas Por Nivel",Information!)
	ll_rows = dw_estadisticas.Retrieve(ll_cve_coordinacion, ls_nivel, li_periodo, li_anio, ll_cve_firma)	
end if
