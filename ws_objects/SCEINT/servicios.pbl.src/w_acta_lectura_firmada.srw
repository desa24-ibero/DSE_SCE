$PBExportHeader$w_acta_lectura_firmada.srw
forward
global type w_acta_lectura_firmada from window
end type
type uo_nivel from uo_nivel_rbutton within w_acta_lectura_firmada
end type
type st_servername from statictext within w_acta_lectura_firmada
end type
type cb_prepara_lector from commandbutton within w_acta_lectura_firmada
end type
type st_periodo_actas from statictext within w_acta_lectura_firmada
end type
type dw_acta_listado_profesor from datawindow within w_acta_lectura_firmada
end type
type cb_consulta_actas_profesor from commandbutton within w_acta_lectura_firmada
end type
type cb_impresion from commandbutton within w_acta_lectura_firmada
end type
type rb_sin_firma from radiobutton within w_acta_lectura_firmada
end type
type rb_con_firma from radiobutton within w_acta_lectura_firmada
end type
type cb_valida_codigo from commandbutton within w_acta_lectura_firmada
end type
type em_codigo_barras from editmask within w_acta_lectura_firmada
end type
type st_1 from statictext within w_acta_lectura_firmada
end type
type rb_todas_nivel from radiobutton within w_acta_lectura_firmada
end type
type rb_acta_nivel from radiobutton within w_acta_lectura_firmada
end type
type st_4 from statictext within w_acta_lectura_firmada
end type
type uoi_coordinaciones from uo_coordinaciones within w_acta_lectura_firmada
end type
type em_no_acta from editmask within w_acta_lectura_firmada
end type
type st_2 from statictext within w_acta_lectura_firmada
end type
type uo_profesor from uo_nombre_profesor within w_acta_lectura_firmada
end type
type uo_grupo from uo_grupos_filtro within w_acta_lectura_firmada
end type
type rb_departamento from radiobutton within w_acta_lectura_firmada
end type
type rb_5 from radiobutton within w_acta_lectura_firmada
end type
type rb_4 from radiobutton within w_acta_lectura_firmada
end type
type rb_3 from radiobutton within w_acta_lectura_firmada
end type
type rb_grupo from radiobutton within w_acta_lectura_firmada
end type
type rb_profesor from radiobutton within w_acta_lectura_firmada
end type
type gb_coordinacion from groupbox within w_acta_lectura_firmada
end type
type st_3 from statictext within w_acta_lectura_firmada
end type
type dw_estadisticas from datawindow within w_acta_lectura_firmada
end type
type cb_lectura from commandbutton within w_acta_lectura_firmada
end type
type dw_1x from datawindow within w_acta_lectura_firmada
end type
type gb_8 from groupbox within w_acta_lectura_firmada
end type
type gb_1 from groupbox within w_acta_lectura_firmada
end type
type gb_2 from groupbox within w_acta_lectura_firmada
end type
type gb_3 from groupbox within w_acta_lectura_firmada
end type
type gb_4 from groupbox within w_acta_lectura_firmada
end type
end forward
...
string title = "Recepción de Actas (Impresión)"
...
global w_acta_lectura_firmada w_acta_lectura_firmada
...
on w_acta_lectura_firmada.create
...
on w_acta_lectura_firmada.destroy
...
if li_proceso_en_captura = 100 then
...
string dataobject = "d_acta_listado_profesor"
boolean maxbox = true
boolean hscrollbar = true
boolean vscrollbar = true
...
ll_rows_profesor = dw_acta_listado_profesor.Retrieve(ll_cve_profesor)  //Tipo de examen 3     3=Ordinario
...
long ll_no_acta
string ls_nivel
integer li_periodo_evento,li_anio_evento,li_evento_baja_total, li_codigo_retorno
...
li_evento_baja_total = 11


f_obten_periodo_tipo(li_periodo_evento,li_anio_evento,li_evento_baja_total, is_tipo_periodo)


li_proceso_en_captura   = f_obten_estatus_proceso_actas(4, li_periodo_evento,li_anio_evento, gtr_sce)
li_proceso_transferidas = f_obten_estatus_proceso_actas(7, li_periodo_evento,li_anio_evento, gtr_sce)
...
//Si ya se tranfirieron Apuntar a SYBASE
	...
	if li_proceso_transferidas = 1 then
		li_actualiza_estatus_acta =	f_actualiza_estatus_acta(ll_no_acta,ls_nivel,li_periodo, li_anio, li_cve_estatus_acta,li_tipo_recepcion, gtr_sce )
	else	
		li_actualiza_estatus_acta =	f_actualiza_estatus_acta(ll_no_acta,ls_nivel,li_periodo, li_anio, li_cve_estatus_acta,li_tipo_recepcion, itr_web )
	end if
...
integer li_periodo_evento,li_anio_evento,li_evento_baja_total
integer li_proceso_en_captura, li_proceso_transferidas
...
li_codigo_retorno = f_lee_codigo_acta(ls_codigo_barras,&
						li_periodo,&		
						li_anio,&
						li_no_acta,&
						ls_nivel)
...
if li_codigo_retorno=-1 then
	...
	ll_retrieve_acta =dw_estadisticas.Retrieve(li_no_acta,ls_nivel,li_periodo, li_anio)
	if ll_retrieve_acta>0 then
		li_cve_estatus_acta_original = dw_estadisticas.GetItemNumber(1,"v_captura_actas_cve_estatus_acta")
		if li_cve_estatus_acta_original<>4 then
			MessageBox("Error","El Acta ["+string(li_no_acta)+" - "+ls_nivel+"] no se encuentra en estatus 4-IMPRESA.~n"+&
			...
			MessageBox("Error","No se encuentra el acta ["+string(li_no_acta)+"-"+ls_nivel+"] o es de Intercambio."+&
							"Por lo tanto no puede Recibirse.",StopSign!)		
			return		
	end if
	//Si ya se tranfirieron Apuntar a SYBASE
	if li_proceso_transferidas = 1 then
		li_actualiza_estatus_acta =	f_actualiza_estatus_acta(li_no_acta,ls_nivel,li_periodo, li_anio, li_cve_estatus_acta,li_tipo_recepcion, gtr_sce )
	else
	//Apuntar a SQL SERVER
		li_actualiza_estatus_acta =	f_actualiza_estatus_acta(li_no_acta,ls_nivel,li_periodo, li_anio, li_cve_estatus_acta,li_tipo_recepcion, itr_web )
	end if
	...
	if li_actualiza_estatus_acta <> -1 then
		...
		dw_estadisticas.Retrieve(li_no_acta,ls_nivel,li_periodo, li_anio)
	end if
	em_codigo_barras.SelectText(1,len(ls_codigo_barras))	
	em_codigo_barras.SetFocus()	
	return	
...
f_obten_periodo(li_periodo_evento,li_anio_evento,li_evento_baja_total)
...
if li_codigo_retorno=-1 then
	...
	//Si ya se tranfirieron Apuntar a SYBASE
	if li_proceso_transferidas = 1 then
		dw_estadisticas.dataobject = 'd_acta_oficial_con_zz'
		dw_estadisticas.SetTransObject(gtr_sce )
	else
	//Apuntar a SQL SERVER
		dw_estadisticas.dataobject = 'd_acta_oficial'
	...
	if li_actualiza_estatus_acta <> -1 then
		dw_estadisticas.Modify("DataWindow.Print.Preview = 'Yes'")
		dw_estadisticas.Modify("DataWindow.Print.Preview.Zoom = '75'")
		dw_estadisticas.Retrieve(ll_no_acta,ls_nivel,li_periodo, li_anio)
	end if
	em_codigo_barras.SelectText(1,len(em_codigo_barras.text))	
	em_codigo_barras.SetFocus()	
	return	
