$PBExportHeader$w_reinscripcion_temporal.srw
forward
global type w_reinscripcion_temporal from window
end type
type cb_1 from commandbutton within w_reinscripcion_temporal
end type
type dw_pase_ingreso from datawindow within w_reinscripcion_temporal
end type
type p_uia from picture within w_reinscripcion_temporal
end type
type st_inic from statictext within w_reinscripcion_temporal
end type
type st_1 from statictext within w_reinscripcion_temporal
end type
type dw_reinsc_mat from datawindow within w_reinscripcion_temporal
end type
type dw_horario_mat from datawindow within w_reinscripcion_temporal
end type
type dw_plan from datawindow within w_reinscripcion_temporal
end type
type uo_nombre from uo_nombre_alumno within w_reinscripcion_temporal
end type
type dw_ext_h from datawindow within w_reinscripcion_temporal
end type
type dw_prerre from datawindow within w_reinscripcion_temporal
end type
type dw_cursada from datawindow within w_reinscripcion_temporal
end type
type dw_labs from datawindow within w_reinscripcion_temporal
end type
type dw_comprobante from datawindow within w_reinscripcion_temporal
end type
type r_1 from rectangle within w_reinscripcion_temporal
end type
type dw_grupos_disponibles from datawindow within w_reinscripcion_temporal
end type
type dw_mat_integ from datawindow within w_reinscripcion_temporal
end type
type dw_grupos_disp from datawindow within w_reinscripcion_temporal
end type
type dw_materias from datawindow within w_reinscripcion_temporal
end type
type dw_mat_pase_ingreso from datawindow within w_reinscripcion_temporal
end type
type mensaje from structure within w_reinscripcion_temporal
end type
end forward
...
global w_reinscripcion_temporal w_reinscripcion_temporal
...
elseif niv = 0 then
	if nivel = 'L' then
		...
		messagebox("No pertenece a Licenciatura","El alumno con número de cuenta "+string(numcta)+"-"+obten_digito(numcta)+" no pertenece a licenciatura. ~rNo puede inscribirse",stopsign!)			
		return 1
	end if
elseif niv = 1 then
	if nivel = 'P' then
			...
			intento1:
			  SELECT area_mat.cve_area  
				 ...
				 WHERE ( mat_inscritas.cve_mat = area_mat.cve_mat ) and  
					...
					if sqlca.sqlcode = 100 or area = 2201 then
							commit;
					elseif sqlca.sqlcode = 0  then
							commit;
...
on w_reinscripcion_temporal.create
...
on w_reinscripcion_temporal.destroy
...
long backcolor = 30976088
boolean enabled = false
string text = "Primavera 2009 10/12/08"
boolean focusrectangle = false
end type
...
string dataobject = "d_mat_pase_ingreso"
boolean resizable = true
boolean border = false
borderstyle borderstyle = stylelowered!
end type
