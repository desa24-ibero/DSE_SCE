...
SELECT      cve_mat = dca_acta_evaluacion_plantilla.cve_mat,   
			...
			nombre_nivel = (SELECT UPPER(desc_corta) FROM controlescolar_bd.dbo.nivel WHERE cve_nivel = dca_acta_evaluacion_plantilla.nivel COLLATE Latin1_General_CI_AS) ,
...
GRANT SELECT ON v_captura_actas TO g_administradores
GO 

GRANT SELECT ON v_captura_actas TO g_control_operativo
GO 

GRANT SELECT ON v_captura_actas TO g_coordinaciones
GO 

GRANT SELECT ON v_captura_actas TO g_intercambio
GO 

GRANT SELECT ON v_captura_actas TO www_sumuia_bd
GO 
