...
IF OBJECT_ID ('dbo.sp_contactos_ce_preu_21') IS NOT NULL
	DROP PROCEDURE dbo.sp_contactos_ce_preu_21
GO

create procedure sp_contactos_ce_preu_21  @a_periodo int, @a_anio int, @a_criterio int 
as
...
--21)->18) El número de aspirantes por carrera, bachillerato y periodo escolar
	SELECT num_contactos = COUNT(*),
			v_sce_carreras.cve_carrera,   
   	   v_sce_carreras.carrera,
			saa_solo_aspirantes.clv_per,
			saa_solo_aspirantes.anio,
			saa_solo_aspirantes.cve_escuela,
			saa_solo_aspirantes.escuela,
			saa_solo_aspirantes.Apaterno_asp,
			saa_solo_aspirantes.Amaterno_asp,
			saa_solo_aspirantes.nombre_asp
...
GRANT EXECUTE ON dbo.sp_contactos_ce_preu_21 TO g_admisionadm
GO
GRANT EXECUTE ON dbo.sp_contactos_ce_preu_21 TO g_inf_admin
GO
