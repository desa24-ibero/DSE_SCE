...
ALTER PROCEDURE [dbo].[sp_srl_qMateriasInscribe_LPI]
 @CUENTA int,
 @CLAVE_MAT int = 0,
 @MATERIA varchar(50) = '',
 @HORAINI int = 0
...
/*Fecha creación 2009-05-14
  ...
  Descripción:	Regresar las materias obligatorias (o la materia específica si el argumento
  				@clave_mat es distinto de cero) del plan que el alumno puede inscribir
...
FROM(
 SELECT	case when ISNULL(SUM(s1_grp.cupo - s1_grp.inscritos),0) <= 0 then 0 else 1 end as cupo,
		...
		0 as es_opt,
		0 as es_reflex
 FROM web_bd.dbo.v_lpi_materias_primer_ingreso mpi
...
--		1 as es_reflex,
--		@a_tf as area_integ
--WHERE @NIVEL = 'L' AND @CLAVE_MAT = 0 AND @puede = 1 AND @PLAN <>6 AND (@tf1 + @tf2) < 2
--UNION
--SELECT  1,
--		0,
...
--		1 as es_reflex,
--		@a_t1 as area_integ
--WHERE @NIVEL = 'L' AND @CLAVE_MAT = 0 AND @puede = 1 AND @t1 = 0
--UNION
--SELECT  1,
--		0,
...
--		1 as es_reflex,
--		@a_t2 as area_integ
--WHERE @NIVEL = 'L' AND @CLAVE_MAT = 0 AND @puede = 1 AND @t2 = 0
--UNION
--SELECT  1,
--		0,
...
--		1 as es_reflex,
--		@a_t3 as area_integ
--WHERE @NIVEL = 'L' AND @CLAVE_MAT = 0 AND @puede = 1 AND @t3 = 0
--UNION
--SELECT  1,
--		0,
...
--		1 as es_reflex,
--		@a_t4 as area_integ
--WHERE @NIVEL = 'L' AND @CLAVE_MAT = 0 AND @puede = 1 AND @t4 = 0
