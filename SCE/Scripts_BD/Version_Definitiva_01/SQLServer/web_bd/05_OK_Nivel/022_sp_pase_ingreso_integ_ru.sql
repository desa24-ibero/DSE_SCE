...
ALTER procedure [dbo].[sp_pase_ingreso_integ_ru] 
@a_cuenta integer,
@a_num_rows integer,
...
--REFLEXION UNIVERSITARIA, HABIENDO CURSADO TEMA I Y FALTA TEMA II
	...
	SELECT  DISTINCT  cuenta = @a_cuenta,
		 ...
		 cve_plan = @ll_cve_plan,
		 cve_mat=  ppm.cve_mat,
	         materia = v_www_materias_optativas.materia,   
	         horas_reales = v_www_materias_optativas.horas_reales,   
	         creditos = v_www_materias_optativas.creditos,   
		 sigla = v_www_materias_optativas.sigla,   
		...
		excede_limite = 0,
		origen = '2RU'
        FROM         v_www_plantilla_plan_materia ppm,
            v_www_materias_optativas, 
            v_www_plan_estudios       
        WHERE     (ppm.cve_plan = @ll_cve_plan) 
            AND (ppm.cve_carrera = @ll_cve_carrera) 	   
            AND ppm.cve_mat = v_www_materias_optativas.cve_mat 
            AND ppm.cve_carrera = v_www_plan_estudios.cve_carrera 
            AND ppm.cve_plan = v_www_plan_estudios.cve_plan 
...
--INSERTA LAS MATERIAS CORRESPONDIENTES AL PLAN IDEAL DE LOS ALUMNOS DE POSGRADO NO APROBADAS POR EL ALUMNO
	  insert into aux_mat_pase_ingreso 
		...
		origen = '2RU'
	        ...
	        WHERE     (v_www_mat_prerreq_pos.cve_plan = @ll_cve_plan) 
	                AND (v_www_mat_prerreq_pos.cve_carrera = @ll_cve_carrera) 
				...
				where h.calificacion in('AC','6','7','8','9','10', 'MB','B','S','E','RE')
				and h.cuenta =@a_cuenta )) 
--		  	AND ( v_www_mat_prerreq_pos.cve_mat in (select g.cve_mat FROM controlescolar_bd.dbo.grupos g, v_www_periodo_mat_inscritas pmi
--				WHERE g.periodo = pmi.periodo
--				AND g.anio =pmi.anio) )
