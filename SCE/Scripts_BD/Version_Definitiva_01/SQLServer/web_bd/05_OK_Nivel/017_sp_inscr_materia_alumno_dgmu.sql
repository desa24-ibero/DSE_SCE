...
ALTER PROCEDURE sp_inscr_materia_alumno_dgmu
 @CUENTA int,
 @MATERIA int,     
 @GRUPO varchar(2),     
 @CARRERA int,
...
if @TMATERIA>0
 begin
  --Obtiene el cupo del grupo e inscritos
  SELECT @CUPO=cupo, @INSCRITOS = inscritos
  FROM dbo.dgmu_grupos, dbo.v_www_periodos_por_procesos  ps
  WHERE dbo.dgmu_grupos.cve_mat = @MATERIA AND dbo.dgmu_grupos.gpo=@GRUPO
  AND dbo.dgmu_grupos.periodo = ps.periodo
  AND dbo.dgmu_grupos.anio = ps.anio
  AND ps.cve_proceso=5
...
--Obtiene el numero de materias de la coordinacion previamente inscritas
  ...
  FROM dbo.dgmu_materias, dbo.dgmu_coordinaciones, dbo.dgmu_mat_inscritas, dbo.v_www_periodos_por_procesos
  ...
  AND dbo.dgmu_mat_inscritas.periodo = dbo.v_www_periodos_por_procesos.periodo
  AND dbo.dgmu_mat_inscritas.anio = dbo.v_www_periodos_por_procesos.anio
  AND  dbo.v_www_periodos_por_procesos.cve_proceso=5
...
--Valida que el grupo tenga cupo mayor a cero, si tiene cero no podra preinscribirse
    ...
    begin
     ...
     FROM dbo.dgmu_mat_inscritas a, dbo.v_www_periodos_por_procesos b
...
--Si es un grupo para otras carreras
         if @termino=0 and @revisa_gpo_bloqueado=1
          ...
          begin
				...
				SELECT @PERIODO=periodo, @ANIO=anio  from dbo.v_www_periodos_por_procesos where dbo.v_www_periodos_por_procesos.cve_proceso = 5   
