...
SELECT      cve_mat = dca_acta_evaluacion_plantilla.cve_mat,   
			...
			nombre_nivel = (SELECT UPPER(n.nivel) FROM controlescolar_bd.dbo.nivel n WHERE n.cve_nivel = dca_acta_evaluacion_plantilla.nivel),
...
SELECT		cve_mat = dca_acta_evaluacion_plantilla.cve_mat,   
			materia = Convert ( Varchar ( 42 ) , dca_materias.materia ),
			gpo = dca_acta_evaluacion_plantilla.gpo,   
			periodo = dca_acta_evaluacion_plantilla.periodo,   
			nombre_periodo = v_sce_periodo.descripcion,   
			anio = dca_acta_evaluacion_plantilla.anio,   
...
GRANT SELECT ON dbo.v_captura_actas TO adminintercambio
GO
GRANT SELECT ON dbo.v_captura_actas TO foraneosadm
GO
GRANT SELECT ON dbo.v_captura_actas TO g_inf_admin
GO
GRANT SELECT ON dbo.v_captura_actas TO g_me_foraneos_aloja
GO
