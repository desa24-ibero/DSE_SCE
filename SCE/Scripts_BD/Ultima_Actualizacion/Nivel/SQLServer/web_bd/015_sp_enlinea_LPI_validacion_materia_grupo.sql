...
/*IF @cursada > 0 AND (@NIVEL = 'L'  OR @NIVEL = 'T')--4, LA MATERIA YA FUÉ CURSADA*/
IF @cursada > 0 AND @NIVEL <> 'P' --4, LA MATERIA YA FUÉ CURSADA 
BEGIN
...
IF @cursadapos > 0 AND @NIVEL = 'P' --4, LA MATERIA YA FUÉ CURSADA
BEGIN
 SET @msg =  'La materia ya fue cursada y aprobada'
 IF @CONMENSAJES <> 0 print @msg
 SET @MENSAJE = @msg
...
IF @retval <> 0
BEGIN
 --SET @msg = 'La materia de integración no se puede inscribir.'
 IF @CONMENSAJES <> 0 print @msg
 SET @MENSAJE = @msg
-- SELECT 140, @msg
-- RETURN 140
